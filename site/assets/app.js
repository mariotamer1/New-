...
\u2022 10" tall`]],at=Date.UTC(2026,8,20),ye=tt.map((e,t)=>{let[a,n,s,i,o,r,c,d,p,m,w]=e,M=(be[a]||[]).map((v,u)=>({sm:`img/p/${a}-${v}-sm.webp`,lg:`img/p/${a}-${v}-lg.webp`,alt:u===0?n:`${n} \u2013 view ${u+1}`}));return{id:"p-"+String(t+1).padStart(3,"0"),slug:a,title:n,brand:s,categoryId:et(i),condition:o,price:r,originalPrice:c,shipping:d,inventory:p,bestDeal:m,published:!0,upc:Xe(t+11),description:w,images:M,createdAt:new Date(at-t*36e5*7).toISOString()}});function we(){throw new Error("Database backend not configured")}var Q="mlgroup-db-v1";function $e(){return{v:1,products:JSON.parse(JSON.stringify(ye)),categories:JSON.parse(JSON.stringify(ve)),settings:{..._},orders:[],offers:[],inquiries:[],customers:[],nextOrder:1001,adminHash:null}}var ke={db:null,async open(){return this.db?this.db:(this.db=await new Promise((e,t)=>{let a=indexedDB.open("mlgroup",1);a.onupgradeneeded=()=>a.result.createObjectStore("kv"),a.onsuccess=()=>e(a.result),a.onerror=()=>t(a.error)}),this.db)},async get(e){let t=await this.open();return new Promise((a,n)=>{let s=t.transaction("kv").objectStore("kv").get(e);s.onsuccess=()=>a(s.result),s.onerror=()=>n(s.error)})},async set(e,t){let a=await this.open();return new Promise((n,s)=>{let i=a.transaction("kv","readwrite");i.objectStore("kv").put(t,e),i.oncomplete=()=>n(),i.onerror=()=>s(i.error)})}};function xe(){let e=null,t=null,a=()=>{clearTimeout(t),t=setTimeout(async()=>{try{await ke.set(Q,e)}catch{T.set(Q,e)}},60)},n=i=>e.products.find(o=>o.id===i),s=(i,o)=>{let r=i||"product",c=2;for(;e.products.some(d=>d.slug===r&&d.id!==o);)r=`${i}-${c++}`;return r};return{kind:"local",async init(){try{e=await ke.get(Q)}catch{e=null}e||(e=T.get(Q,null)),(!e||!e.products)&&(e=$e()),e.adminHash||(e.adminHash=await N("MLGROUP")),a()},catalog(){return{products:e.products,categories:e.categories,settings:e.settings}},async placeOrder(i){let o=[];for(let v of i.items){let u=n(v.id),$=Math.max(1,Math.floor(v.qty));if(!u||!u.published||u.inventory<=0)throw new Error(`${u?u.title:"An item in your cart"} is no longer available.`);if(u.inventory<$)throw new Error(`Only ${u.inventory} left of \u201C${u.title}\u201D. Please lower the quantity.`);o.push({p:u,qty:$})}let r=i.fulfillment==="pickup",c=o.map(({p:v,qty:u})=>({productId:v.id,slug:v.slug,title:v.title,image:v.images[0]||null,price:v.price,qty:u,shipping:r?0:v.shipping})),d=O(c.reduce((v,u)=>v+u.price*u.qty,0)),p=O(c.reduce((v,u)=>v+u.shipping,0));o.forEach(({p:v,qty:u})=>{v.inventory-=u});let m=null,w=(i.customer.email||"").trim().toLowerCase();if(i.createAccount&&i.createAccount.password){let v=e.customers.find(u=>u.email===w);v||(v={id:D(),email:w,name:i.customer.name,phone:i.customer.phone,pw:await N(i.createAccount.password),createdAt:new Date().toISOString()},e.customers.push(v)),m=v.id,T.set("ml-customer",v.id)}else{let v=T.get("ml-customer",null),u=v&&e.customers.find($=>$.id===v&&$.email===w);u&&(m=u.id)}let M={id:D(),number:e.nextOrder++,createdAt:new Date().toISOString(),status:"new",fulfillment:i.fulfillment,payment:i.payment,customer:{name:i.customer.name,email:w,phone:i.customer.phone},address:r?null:i.address,notes:i.notes||"",items:c,subtotal:d,shippingTotal:p,total:O(d+p),customerId:m};return e.orders.unshift(M),a(),M},async getOrder(i){return e.orders.find(o=>o.id===i)||null},async submitOffer(i){let o=n(i.productId);if(!o)throw new Error("This product is no longer available.");e.offers.unshift({id:D(),productId:o.id,productTitle:o.title,productSlug:o.slug,productImage:o.images[0]||null,listPrice:o.price,name:i.name,phone:i.phone,amount:O(i.amount),status:"new",createdAt:new Date().toISOString()}),a()},async submitInquiry(i){e.inquiries.unshift({id:D(),status:"new",createdAt:new Date().toISOString(),...i}),a()},async register({email:i,password:o,name:r,phone:c}){if(i=i.trim().toLowerCase(),e.customers.some(p=>p.email===i))throw new Error("An account with this email already exists. Try signing in.");let d={id:D(),email:i,name:r,phone:c,pw:await N(o),createdAt:new Date().toISOString()};return e.customers.push(d),a(),T.set("ml-customer",d.id),{email:d.email,name:d.name,phone:d.phone}},async login({email:i,password:o}){i=i.trim().toLowerCase();let r=e.customers.find(c=>c.email===i);if(!r||r.pw!==await N(o))throw new Error("Email or password is incorrect.");return T.set("ml-customer",r.id),{email:r.email,name:r.name,phone:r.phone}},async logout(){T.del("ml-customer")},async me(){let i=T.get("ml-customer",null),o=i&&e.customers.find(r=>r.id===i);return o?{email:o.email,name:o.name,phone:o.phone,orders:e.orders.filter(r=>r.customerId===o.id)}:null},admin:{async login(i){if(await N(i)!==e.adminHash)throw new Error("That password is incorrect.");Y.set("ml-admin",1)},async logout(){Y.del("ml-admin")},async session(){return!!Y.get("ml-admin",0)},async changePassword(i){e.adminHash=await N(i),a()},async products(){return e.products},async saveProduct(i){let o=i.id?n(i.id):null,r={...o||{},...i};return r.slug=s(K(i.slug||i.title),r.id),o?Object.assign(o,r):(r.id=D(),r.createdAt=new Date().toISOString(),e.products.unshift(r)),r.updatedAt=new Date().toISOString(),a(),o||r},async deleteProduct(i){e.products=e.products.filter(o=>o.id!==i),a()},async patchProduct(i,o){let r=n(i);return r&&Object.assign(r,o),a(),r},async adjustInventory(i,o){let r=n(i);return r&&(r.inventory=Math.max(0,(r.inventory||0)+o)),a(),r},async orders(){return e.orders},async updateOrder(i,o){let r=e.orders.find(c=>c.id===i);if(r)return o.status==="cancelled"&&r.status!=="cancelled"&&o.restock&&r.items.forEach(c=>{let d=n(c.productId);d&&(d.inventory+=c.qty)}),delete o.restock,Object.assign(r,o),a(),r},async offers(){return e.offers},async updateOffer(i,o){let r=e.offers.find(c=>c.id===i);r&&Object.assign(r,o),a()},async inquiries(){return e.inquiries},async updateInquiry(i,o){let r=e.inquiries.find(c=>c.id===i);r&&Object.assign(r,o),a()},async deleteInquiry(i){e.inquiries=e.inquiries.filter(o=>o.id!==i),a()},async saveCategory(i){if(i.id){let o=e.categories.find(r=>r.id===i.id);Object.assign(o,i,{slug:K(i.slug||i.name)})}else e.categories.push({id:D(),name:i.name,slug:K(i.slug||i.name),sort:e.categories.length+1});a()},async deleteCategory(i){e.categories=e.categories.filter(o=>o.id!==i),e.products.forEach(o=>{o.categoryId===i&&(o.categoryId=null)}),a()},async reorderCategories(i){i.forEach((o,r)=>{let c=e.categories.find(d=>d.id===o);c&&(c.sort=r+1)}),a()},async saveSettings(i){Object.assign(e.settings,i),a()},async uploadImage(i){let[o,r]=await Promise.all([ie(i,1400),ie(i,520)]);return{lg:await ne(o),sm:await ne(r)}},async removeImage(){},async exportData(){return JSON.stringify(e,null,1)},async importData(i){let o=JSON.parse(i);if(!o.products)throw new Error("That file is not an ML Group backup.");e=o,a()},async reset(){let i=e.adminHash;e=$e(),e.adminHash=i,a()}}}}var P=null,W={products:[],categories:[],settings:{..._}},g={get kind(){return P?P.kind:"local"},async init(){let e=S.cfg||{};if(e.supabaseUrl&&e.supabaseKey&&e.routing!=="memory")try{P=we(e),await P.init()}catch(t){console.warn("Store database unreachable, using built-in catalog",t),P=xe(),await P.init(),P.offline=!0}else P=xe(),await P.init();this.refresh()},get offline(){return!!(P&&P.offline)},refresh(){W=P.catalog()},async reload(){P.reload&&await P.reload(),this.refresh()},settings(){return W.settings||_},categories(){return[...W.categories].sort((e,t)=>(e.sort||0)-(t.sort||0))},category(e){return W.categories.find(t=>t.id===e||t.slug===e)||null},products(){return W.products.filter(e=>e.published&&e.inventory>0)},product(e){return this.products().find(t=>t.slug===e)||null},productById(e){return W.products.find(t=>t.id===e)||null},placeOrder:e=>P.placeOrder(e).then(t=>(g.refresh(),t)),getOrder:e=>P.getOrder(e),submitOffer:e=>P.submitOffer(e),submitInquiry:e=>P.submitInquiry(e),account:{register:e=>P.register(e),login:e=>P.login(e),logout:()=>P.logout(),me:()=>P.me()},get admin(){return P.admin}},Se="ml-cart",B={lines(){return T.get(Se,[]).filter(e=>e&&e.id&&e.qty>0)},save(e){T.set(Se,e),window.dispatchEvent(new CustomEvent("cart:change"))},items(){return this.lines().map(e=>{let t=g.productById(e.id);return!t||!t.published||t.inventory<=0?null:{product:t,qty:Math.min(e.qty,t.inventory)}}).filter(Boolean)},count(){return this.items().reduce((e,t)=>e+t.qty,0)},subtotal(){return O(this.items().reduce((e,t)=>e+t.product.price*t.qty,0))},add(e,t=1){let a=g.productById(e);if(!a)return;let n=this.lines(),s=n.find(i=>i.id===e);s?s.qty=Math.min(a.inventory,s.qty+t):n.push({id:e,qty:Math.min(a.inventory,t)}),this.save(n)},set(e,t){let a=g.productById(e),n=this.lines().map(s=>s.id===e?{...s,qty:Math.max(1,Math.min(a?a.inventory:t,t))}:s);this.save(n)},remove(e){this.save(this.lines().filter(t=>t.id!==e))},clear(){this.save([])}};function re({title:e,description:t,path:a="/",image:n,jsonld:s,noindex:i}={}){let o=g.settings(),r=e?`${e} | ${o.storeName||"ML Group"}`:`${o.storeName||"ML Group"} \u2014 Big Savings. Great Products.`;document.title=r;let c=(m,w,M)=>{let v=document.head.querySelector(m);if(!v){v=document.createElement("meta");let[u,$]=m.match(/\[(.+?)="(.+?)"\]/).slice(1);v.setAttribute(u,$),document.head.appendChild(v)}v.setAttribute(w,M)},d=t||"Discounted electronics, home goods, tools, appliances and more for retail and wholesale buyers. Fast shipping and free local pickup.";if(c('meta[name="description"]',"content",d),c('meta[property="og:title"]',"content",r),c('meta[property="og:description"]',"content",d),S.routing==="path"){let m=S.root+a.replace(/^\//,"");c('meta[property="og:url"]',"content",m);let w=document.head.querySelector('link[rel="canonical"]');w||(w=document.createElement("link"),w.rel="canonical",document.head.appendChild(w)),w.href=m.split("?")[0]}n&&c('meta[property="og:image"]',"content",R(n)),c('meta[name="robots"]',"content",i?"noindex":"index,follow");let p=document.getElementById("ld-page");s?(p||(p=document.createElement("script"),p.type="application/ld+json",p.id="ld-page",document.head.appendChild(p)),p.textContent=JSON.stringify(s)):p&&p.remove()}function ee(e,t=""){let a=y(".toasts");a||(a=document.createElement("div"),a.className="toasts",a.setAttribute("role","status"),a.setAttribute("aria-live","polite"),document.body.appendChild(a));let n=document.createElement("div");n.className="toast "+t,n.innerHTML=`${h(t==="err"?"close":"check","icon-sm")}<span>${l(e)}</span>`,a.appendChild(n),setTimeout(()=>{n.style.opacity="0",n.style.transition="opacity .3s",setTimeout(()=>n.remove(),320)},2600)}var st=[["/","Home"],["/products","Products"],["#cat","Categories"],["/wholesale","Wholesale"],["/about","About"],["/contact","Contact"]];function it(){let e={};return g.products().forEach(t=>{e[t.categoryId]=(e[t.categoryId]||0)+1}),e}function Ce(){let e=g.settings(),t=it(),n=g.categories().map(r=>`<a href="${f("/categories/"+r.slug)}">${l(r.name)} <span class="count">${t[r.id]||0}</span></a>`).join(""),s=st.map(([r,c])=>r==="#cat"?`<li class="nav-dd"><button class="nav-btn" type="button" aria-expanded="false" aria-controls="dd-cats" data-dd>Categories ${h("down")}</button>
  ...
  <footer class="site-footer on-ink">
  ...
  </footer>`}function nt(e,{save:t=!1}={}){let a=q(e);return`<div class="price"><span class="now tabnum">${k(e.price)}</span>${a?`<s class="was tabnum">${k(e.originalPrice)}</s>`:""}${t&&a?`<span class="save">Save ${k(e.originalPrice-e.price)}</span>`:""}</div>`}function F(e,{eager:t=!1,sizes:a}={}){let n=q(e),s=f("/products/"+e.slug),i=e.images.length?e.images:[null],o=i.map((r,c)=>`<a href="${s}" ${c?'tabindex="-1" aria-hidden="true"':`aria-label="${l(e.title)}"`}>${A(r,{alt:c?"":e.title,eager:t&&c===0,sizes:a})}</a>`).join("");return`<article class="card" data-pid="${e.id}">
    ...
    <div class="card-body">
      ...
      ${nt(e)}
      <p class="card-ship">${e.shipping>0?`+ ${k(e.shipping)} shipping`:"Free shipping"} \xB7 Pickup available</p>
  ...
  </article>`}function U(e=document){E("[data-carousel]",e).forEach(t=>{if(t._car)return;t._car=!0;let a=y(".track",t),n=E(".dots i",t),s=y(".g-count",t),i=t.closest(".gallery")?E(".g-thumbs button",t.closest(".gallery")):[],o=0,r=()=>{o=0;let c=Math.round(a.scrollLeft/Math.max(1,a.clientWidth));n.forEach((d,p)=>d.classList.toggle("on",p===c)),i.forEach((d,p)=>d.setAttribute("aria-current",p===c?"true":"false")),s&&(s.textContent=`${c+1} / ${a.children.length}`)};a.addEventListener("scroll",()=>{o||(o=requestAnimationFrame(r))},{passive:!0}),t.addEventListener("click",c=>{let d=c.target.closest("[data-car]");if(!d)return;c.preventDefault(),c.stopPropagation();let p=a.children.length,m=a.clientWidth,M=(Math.round(a.scrollLeft/m)+Number(d.dataset.car)+p)%p;a.scrollTo({left:M*m,behavior:matchMedia("(prefers-reduced-motion: reduce)").matches?"auto":"smooth"})}),i.forEach((c,d)=>c.addEventListener("click",()=>a.scrollTo({left:d*a.clientWidth,behavior:"smooth"})))})}function Me(e){return`<form class="search" role="search" data-search action="${f("/products")}">
  ...
  </form>`}var I=e=>String(e||"").toLowerCase().normalize("NFKD").replace(/[̀-ͯ]/g,"");function Z(e,t){let a=l(e);return t.filter(n=>n.length>1).forEach(n=>{a=a.replace(new RegExp("("+n.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+")","ig"),"<mark>$1</mark>")}),a}function le(e,t=99){let a=I(e).split(/\s+/).filter(Boolean);if(!a.length)return[];let n=Object.fromEntries(g.categories().map(i=>[i.id,i.name])),s=[];for(let i of g.products()){let o=I(i.title),r=I(i.brand),c=I(n[i.categoryId]),d=String(i.upc||""),p=`${o} ${r} ${c} ${d} ${I(i.condition)}`;if(!a.every(w=>p.includes(w)))continue;let m=0;a.forEach(w=>{o.startsWith(w)&&(m+=6),new RegExp("\\b"+w.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")).test(o)&&(m+=3),r.startsWith(w)&&(m+=4),d&&d.startsWith(w)&&(m+=8),c.startsWith(w)&&(m+=2)}),s.push({p:i,score:m+(i.bestDeal?1:0)})}return s.sort((i,o)=>o.score-i.score).slice(0,t).map(i=>i.p)}function ot(e,t){let a=y("input",e),n=y(".search-pop",e),s=y("[data-clear]",e),i=-1,o=()=>E('[role="option"]',n),r=()=>{n.classList.remove("open"),a.setAttribute("aria-expanded","false"),i=-1,a.removeAttribute("aria-activedescendant")},c=p=>{let m=o();m.length&&(i=(p+m.length)%m.length,m.forEach((w,M)=>w.classList.toggle("active",M===i)),a.setAttribute("aria-activedescendant",m[i].id),m[i].scrollIntoView({block:"nearest"}))},d=()=>{let p=a.value.trim();if(s.hidden=!p,!p){r();return}let m=I(p).split(/\s+/).filter(Boolean),w=g.categories().filter(b=>m.every(x=>I(b.name).includes(x))),M=[...new Set(g.products().map(b=>b.brand).filter(Boolean))].filter(b=>m.every(x=>I(b).includes(x))).slice(0,4),v=le(p,6),u=a.id,$=0,L="";w.length&&(L+=`<div class="sp-group"><p class="sp-label">Categories</p><div class="sp-chips">${w.map(b=>`<a class="chip" role="option" id="${u}-o${$++}" href="${f("/categories/"+b.slug)}">${Z(b.name,m)}</a>`).join("")}</div></div>`),M.length&&(L+=`<div class="sp-group"><p class="sp-label">Brands</p><div class="sp-chips">${M.map(b=>`<a class="chip" role="option" id="${u}-o${$++}" href="${f("/products?q="+encodeURIComponent(b))}">${Z(b,m)}</a>`).join("")}</div></div>`),v.length&&(L+=`<div class="sp-group"><p class="sp-label">Products</p>${v.map(b=>`<a class="sp-item" role="option" id="${u}-o${$++}" href="${f("/products/"+b.slug)}">${A(b.images[0],{alt:"",sizes:"48px",w:48})}<span><span class="sp-name">${Z(b.title,m)}</span><span class="sp-meta">${l(b.brand||"")} \xB7 ${l(b.condition)}${/^\d{4,}$/.test(p)?` \xB7 UPC ${Z(b.upc,m)}`:""}</span></span><span class="sp-price tabnum">${k(b.price)}${q(b)?`<s>${k(b.originalPrice)}</s>`:""}</span></a>`).join("")}</div>`),L?L+=`<a class="sp-all" role="option" id="${u}-o${$++}" href="${f("/products?q="+encodeURIComponent(p))}">See all results for \u201C${l(p)}\u201D \u2192</a>`:L=`<p class="sp-empty">No matches for \u201C${l(p)}\u201D. Try a brand, category or UPC.</p>`,n.innerHTML=L,n.classList.add("open"),a.setAttribute("aria-expanded","true"),i=-1};a.addEventListener("input",d),a.addEventListener("focus",()=>{a.value.trim()&&d()}),a.addEventListener("keydown",p=>{p.key==="ArrowDown"?(p.preventDefault(),n.classList.contains("open")||d(),c(i+1)):p.key==="ArrowUp"?(p.preventDefault(),c(i-1)):p.key==="Escape"?r():p.key==="Enter"&&i>=0&&(p.preventDefault(),o()[i].click())}),s.addEventListener("click",()=>{a.value="",d(),a.focus()}),e.addEventListener("submit",p=>{p.preventDefault();let m=a.value.trim();r(),t("/products"+(m?"?q="+encodeURIComponent(m):"")),a.blur()}),n.addEventListener("click",p=>{if(p.target.closest("a")){r(),a.blur();let m=document.getElementById("mobile-search");m&&e.closest("#mobile-search")&&(m.hidden=!0,y("[data-msearch]").setAttribute("aria-expanded","false"))}}),document.addEventListener("click",p=>{e.contains(p.target)||r()})}var X=null;function rt(e){let t=y(".sheet.open");if(!t||e.key!=="Tab")return;let a=E('a[href], button:not([disabled]), input, select, textarea, [tabindex]:not([tabindex="-1"])',t).filter(n=>n.offsetParent!==null);a.length&&(e.shiftKey&&document.activeElement===a[0]?(e.preventDefault(),a[a.length-1].focus()):!e.shiftKey&&document.activeElement===a[a.length-1]&&(e.preventDefault(),a[0].focus()))}function oe(e){let t=document.getElementById(e);if(!t)return;X=document.activeElement,z(!0),t.classList.add("open");let a=y(`[data-overlay="${e==="cart-sheet"?"cart":"menu"}"]`);a&&a.classList.add("open"),document.documentElement.style.overflow="hidden",setTimeout(()=>{let n=y("button, a[href]",t);n&&n.focus()},60),E("[data-menu]").forEach(n=>n.setAttribute("aria-expanded",e==="menu-sheet"?"true":"false"))}function z(e){let t=y(".sheet.open");E(".sheet.open, .overlay.open").forEach(a=>a.classList.remove("open")),document.documentElement.style.overflow="",E("[data-menu]").forEach(a=>a.setAttribute("aria-expanded","false")),t&&!e&&X&&document.contains(X)&&X.focus()}function qe(){return`<div class="overlay" data-overlay="cart"></div>
  <aside class="sheet right" id="cart-sheet" role="dialog" aria-modal="true" aria-labelledby="cart-title">
  ...
  </aside>`}function Le(){let e=y("[data-cart-body]"),t=y("[data-cart-foot]"),a=B.items(),n=a.reduce((s,i)=>s+i.qty,0);if(E("[data-cart-count]").forEach(s=>{s.textContent=n,s.hidden=!n}),!!e){if(y("#cart-title").textContent=n?`Your cart (${n})`:"Your cart",!a.length){e.innerHTML=`<div class="cart-empty">${h("bag")}<p><b>Your cart is empty.</b></p><p class="muted">Find something good in today\u2019s deals.</p><a class="btn btn-primary" href="${f("/products?deals=1")}" data-close-cart>Browse Deals</a></div>`,t.innerHTML="";return}e.innerHTML=`<ul class="cart-items">${a.map(({product:s,qty:i})=>`
    <li class="cart-item" data-line="${s.id}">
      ...
      <div>
        <div class="ci-top"><a class="ci-title" href="${f("/products/"+s.slug)}" data-close-cart>${l(s.title)}</a><span class="ci-price tabnum">${k(s.price*i)}</span></div>
        <p class="ci-meta">${l(s.condition)} \xB7 ${k(s.price)} each${s.inventory<=5?` \xB7 ${s.inventory} available`:""}</p>
        <div class="ci-row">
    ...
    </li>`).join("")}</ul>`,t.innerHTML=`<div class="cart-sum"><div class="row total"><span>Subtotal</span><span class="tabnum">${k(B.subtotal())}</span></div></div>
    ...
    <form class="modal-body form" novalidate data-offer-form>
      <div class="offer-item">${A(t.images[0],{alt:"",sizes:"64px"})}<div><b>${l(t.title)}</b><span>Listed at <b class="tabnum" style="display:inline">${k(t.price)}</b> \xB7 ${l(t.condition)}</span></div></div>
    ...
    </form>`;let n=y("[data-offer-form]",a);n.addEventListener("submit",async s=>{s.preventDefault();let i=new FormData(n),o=y("[data-err]",n),r=String(i.get("name")||"").trim(),c=String(i.get("phone")||"").trim(),d=Number(i.get("amount")),p=[];if([["of-name",!r],["of-phone",c.replace(/\D/g,"").length<10],["of-amount",!(d>0)]].forEach(([w,M])=>{y("#"+w).setAttribute("aria-invalid",M?"true":"false"),M&&p.push(w)}),p.length){o.textContent="Please enter your name, a 10-digit phone number and an offer amount.",o.hidden=!1,y("#"+p[0]).focus();return}let m=y('button[type="submit"]',n);m.disabled=!0,m.textContent="Sending\u2026";try{i.get("website")||await g.submitOffer({productId:t.id,name:r,phone:c,amount:d}),n.innerHTML=`<div class="success"><span class="check">${h("check")}</span><h3>Offer sent</h3><p>Thanks, ${l(r.split(" ")[0])}. We received your offer of <b>${k(d)}</b> for <b>${l(t.title)}</b>. We\u2019ll call or text <b>${l(c)}</b> soon.</p><button class="btn btn-primary" type="button" data-close-dialog>Done</button></div>`}catch(w){o.textContent=w.message||"Something went wrong. Please try again.",o.hidden=!1,m.disabled=!1,m.textContent="Submit offer"}}),a.addEventListener("click",s=>{(s.target===a||s.target.closest("[data-close-dialog]"))&&a.close()}),a.showModal?a.showModal():a.setAttribute("open",""),setTimeout(()=>{let s=y("#of-name",a);s&&s.focus()},50)}function Ee(e){E("[data-search]").forEach(a=>ot(a,e)),lt();let t=y("[data-dd]");if(t){let a=document.getElementById(t.getAttribute("aria-controls")),n=t.closest(".nav-dd"),s,i=o=>{t.setAttribute("aria-expanded",o?"true":"false"),a.classList.toggle("open",o)};t.addEventListener("click",()=>i(t.getAttribute("aria-expanded")!=="true")),n.addEventListener("mouseenter",()=>{matchMedia("(hover:hover)").matches&&(clearTimeout(s),i(!0))}),n.addEventListener("mouseleave",()=>{matchMedia("(hover:hover)").matches&&(s=setTimeout(()=>i(!1),160))}),n.addEventListener("keydown",o=>{o.key==="Escape"&&(i(!1),t.focus())}),a.addEventListener("click",o=>{o.target.closest("a")&&i(!1)}),document.addEventListener("click",o=>{n.contains(o.target)||i(!1)})}document.addEventListener("click",a=>{let n=a.target;if(n.closest("[data-open-cart]")){oe("cart-sheet");return}if(n.closest("[data-menu]")){oe("menu-sheet");return}(n.closest("[data-close-cart]")||n.closest("[data-close-menu]")||n.closest(".overlay"))&&z(!!n.closest("a"));let s=n.closest(".menu-toggle");if(s){let c=s.getAttribute("aria-expanded")!=="true";s.setAttribute("aria-expanded",c),document.getElementById(s.getAttribute("aria-controls")).hidden=!c}n.closest(".menu-sheet a")&&z(!0);let i=n.closest("[data-msearch]");if(i){let c=y("#mobile-search"),d=c.hidden;c.hidden=!d,i.setAttribute("aria-expanded",d),d&&setTimeout(()=>y("input",c).focus(),30)}let o=n.closest("[data-add]");o&&(a.preventDefault(),ce(o.dataset.add,Number(o.dataset.qty||1)));let r=n.closest("[data-offer]");r&&(a.preventDefault(),ct(r.dataset.offer))}),document.addEventListener("keydown",a=>{a.key==="Escape"&&y(".sheet.open")&&z(),rt(a)})}function Ae(e){E("[data-nav]").forEach(t=>{let a=t.dataset.nav;(a==="/"?e==="/":e.startsWith(a))?t.setAttribute("aria-current","page"):t.removeAttribute("aria-current")})}var Be=e=>(g.category(e)||{}).name||"Other",de=()=>{let e=g.products(),t=e.filter(n=>n.bestDeal).sort((n,s)=>q(s)-q(n)),a=e.filter(n=>!n.bestDeal).sort((n,s)=>q(s)-q(n));return[...t,...a]};function He(){let e=g.settings(),t=de(),a=d=>g.product(d),s=["portable-bluetooth-cd-boombox","countertop-microwave-1-1-cu-ft","portable-steel-toolbox-20-inch","velvet-accent-chair-mango","vintage-tin-toy-car","robot-vacuum-cleaner"].map(a).filter(Boolean);t.forEach(d=>{s.length<6&&!s.includes(d)&&s.push(d)}),s=s.slice(0,6);let i=g.categories(),o={};g.products().forEach(d=>{o[d.categoryId]=(o[d.categoryId]||0)+1});let r=d=>{let p=t.find(m=>m.categoryId===d.id);return p?p.images[0]:null},c=Math.max(0,...g.products().map(q));return{title:"",description:`Big savings on electronics, home goods, tools, appliances, toys and more \u2014 up to ${c}% off retail. Fast shipping, free local pickup, and wholesale pricing from ML Group.`,html:`
    <section class="hero">
      <div class="wrap hero-grid">
        ...
        <div class="hero-collage" aria-label="Featured deals">
          ${s.map((d,p)=>`<a class="hero-tile ${p===0?"big":""}" href="${f("/products/"+d.slug)}">
            ...
            <span class="tag">${q(d)?`<b>-${q(d)}%</b>`:""}<span class="tabnum">${k(d.price)}</span></span>
          </a>`).join("")}
        </div>
      </div>
    </section>
    ...
    <dialog class="modal" id="filter-dialog" aria-labelledby="filter-title"><div class="modal-head"><h2 id="filter-title">Filters</h2><button class="icon-btn" type="button" aria-label="Close filters" data-close-filters>${h("close")}</button></div><div class="modal-body" data-filters-m></div><div class="modal-body" style="padding-top:0"><button class="btn btn-primary btn-block" type="button" data-close-filters>Show results</button></div></dialog>`,mount(r,{navigateReplace:c}){let d=u=>{let $={};g.products().forEach(b=>{$[b.categoryId]=($[b.categoryId]||0)+1});let L=`${s.min}-${s.max}`;return`
        <div class="f-group"><h3>Category</h3><div class="f-options">
          ...
          ${g.categories().map(b=>`<label class="f-opt"><input type="radio" name="cat${u}" value="${b.slug}" ${s.cat===b.slug?"checked":""}> ${l(b.name)} <span class="count">${$[b.id]||0}</span></label>`).join("")}
        </div></div>
        ...
        <div class="f-group"><h3>Price</h3><div class="f-options">
          ${ut.map(([b,x])=>`<label class="f-opt"><input type="radio" name="range${u}" value="${b}" ${(b===""?!s.min&&!s.max:L===b)?"checked":""}> ${x}</label>`).join("")}
        </div>
        <div class="price-inputs" style="margin-top:10px"><label><span>$</span><input type="number" inputmode="numeric" min="0" placeholder="Min" aria-label="Minimum price" name="min${u}" value="${l(s.min)}"></label><span aria-hidden="true">\u2013</span><label><span>$</span><input type="number" inputmode="numeric" min="0" placeholder="Max" aria-label="Maximum price" name="max${u}" value="${l(s.max)}"></label></div></div>`},p=()=>{let u=s.q?le(s.q):g.products().slice();if(s.cat){let x=g.category(s.cat);u=u.filter(C=>x&&C.categoryId===x.id)}s.deals&&(u=u.filter(x=>x.bestDeal||q(x)>=40));let $=parseFloat(s.min),L=parseFloat(s.max);isNaN($)||(u=u.filter(x=>x.price>=$)),isNaN(L)||(u=u.filter(x=>x.price<=L));let b={deals:(x,C)=>C.bestDeal-x.bestDeal||q(C)-q(x),"price-asc":(x,C)=>x.price-C.price,"price-desc":(x,C)=>C.price-x.price,new:(x,C)=>String(C.createdAt).localeCompare(String(x.createdAt))};return s.q&&s.sort==="deals"||u.sort(b[s.sort]||b.deals),u},m=()=>{let u=new URLSearchParams;s.q&&u.set("q",s.q),s.deals&&u.set("deals","1"),s.min&&u.set("min",s.min),s.max&&u.set("max",s.max),s.sort!=="deals"&&u.set("sort",s.sort);let $=s.cat?"/categories/"+s.cat:"/products";c($+(u.toString()?"?"+u:""))},w=()=>{let u=p();y("[data-count]",r).innerHTML=`<b>${u.length}</b> ${u.length===1?"product":"products"}`;let $=[];s.q&&$.push(["q",`\u201C${s.q}\u201D`]),s.cat&&$.push(["cat",Be((g.category(s.cat)||{}).id)]),s.deals&&$.push(["deals","Best deals"]),(s.min||s.max)&&$.push(["price",`${s.min?"$"+s.min:"$0"} \u2013 ${s.max?"$"+s.max:"any"}`]),y("[data-active]",r).innerHTML=$.map(([b,x])=>`<button class="chip" type="button" data-clear-f="${b}" aria-label="Remove filter ${l(x)}">${l(x)} ${h("close")}</button>`).join("")+($.length>1?'<button class="link" type="button" data-clear-f="all" style="font-size:13px">Clear all</button>':"");let L=y("[data-results]",r);if(!u.length){L.innerHTML='<div class="empty"><h2>No matches</h2><p class="muted">Try removing a filter or searching for something else.</p><button class="btn" type="button" data-clear-f="all">Clear filters</button></div>';return}if(s.view==="scroll"){let b=g.categories().map(C=>[C,u.filter(V=>V.categoryId===C.id)]).filter(([,C])=>C.length),x=u.filter(C=>!g.category(C.categoryId));x.length&&b.push([{name:"More",slug:""},x]),L.innerHTML=`<div class="shelves">${b.map(([C,V])=>`<section><div class="shelf-head"><h3>${l(C.name)}</h3>${C.slug?`<a href="${f("/categories/"+C.slug)}">See all ${V.length}</a>`:""}</div><div class="shelf" tabindex="0" aria-label="${l(C.name)} products, scroll horizontally">${V.map(Ke=>F(Ke,{sizes:"260px"})).join("")}</div></section>`).join("")}</div>`}else{let b={grid4:"cols-4",grid2:"cols-2big",list:"list"}[s.view]||"cols-4",x=s.view==="grid2"?"(min-width:1024px) 38vw, 50vw":s.view==="list"?"220px":"(min-width:1024px) 20vw, 50vw";L.innerHTML=`<div class="grid ${b}">${u.slice(0,s.shown).map(C=>F(C,{sizes:x})).join("")}</div>${u.length>s.shown?`<div class="load-more"><button class="btn btn-lg" type="button" data-more>Show more (${u.length-s.shown} left)</button></div>`:""}`}U(L)},M=()=>{y("[data-filters]",r).innerHTML=d("");let u=y("[data-filters-m]",r);u&&(u.innerHTML=d("-m"))},v=()=>{M(),w(),m()};r.addEventListener("change",u=>{let $=u.target,L=($.name||"").replace(/-m$/,"");if(L==="cat")s.cat=$.value;else if(L==="deals")s.deals=$.checked;else if(L==="range"){let[b,x]=$.value.split("-");s.min=b||"",s.max=x||""}else if(L==="min"||L==="max")s[L]=$.value.trim();else if($.matches("[data-sort]"))s.sort=$.value;else return;s.shown=24,v()}),r.addEventListener("click",u=>{let $=u.target.closest("[data-view]");$&&(s.view=$.dataset.view,T.set("ml-view",s.view),E("[data-view]",r).forEach(b=>b.setAttribute("aria-pressed",b===$)),w());let L=u.target.closest("[data-clear-f]");if(L){let b=L.dataset.clearF;(b==="q"||b==="all")&&(s.q=""),(b==="cat"||b==="all")&&(s.cat=""),(b==="deals"||b==="all")&&(s.deals=!1),(b==="price"||b==="all")&&(s.min="",s.max=""),v()}u.target.closest("[data-more]")&&(s.shown+=24,w()),u.target.closest("[data-open-filters]")&&y("#filter-dialog").showModal(),u.target.closest("[data-close-filters]")&&y("#filter-dialog").close()}),M(),w()}}}function De({params:e}){let t=g.product(e.slug);if(!t)return te("This item is sold out or no longer available.");let a=g.settings(),n=q(t),s=g.category(t.categoryId),i=g.products().filter(p=>p.categoryId===t.categoryId&&p.id!==t.id).slice(0,4),o=t.images.length?t.images:[null],r={New:"NewCondition",Refurbished:"RefurbishedCondition"},c={"@context":"https://schema.org","@type":"Product",name:t.title,sku:t.id,gtin12:t.upc||void 0,image:t.images.map(p=>R(p.lg)),description:t.description.replace(/\n+/g," ").replace(/•/g,""),brand:t.brand?{"@type":"Brand",name:t.brand}:void 0,category:s?s.name:void 0,offers:{"@type":"Offer",url:S.root+"products/"+t.slug,priceCurrency:"USD",price:t.price.toFixed(2),availability:"https://schema.org/InStock",itemCondition:"https://schema.org/"+(r[t.condition]||"UsedCondition"),seller:{"@type":"Organization",name:"ML Group"},shippingDetails:{"@type":"OfferShippingDetails",shippingRate:{"@type":"MonetaryAmount",value:Number(t.shipping).toFixed(2),currency:"USD"},shippingDestination:{"@type":"DefinedRegion",addressCountry:"US"}}}},d=l(t.description).split(/\n\n+/).map(p=>p.trim().startsWith("\u2022")?`<ul>${p.split(`
`).map(m=>`<li>${m.replace(/^•\s*/,"")}</li>`).join("")}</ul>`:`<p>${p.replace(/\n/g,"<br>")}</p>`).join("");return{title:t.title,image:t.images[0]&&t.images[0].lg,jsonld:c,path:"/products/"+t.slug,description:`${t.title} \u2014 ${t.condition}, ${k(t.price)}${n?` (${n}% off)`:""}. Shipping ${k(t.shipping)} or free local pickup at ML Group.`,html:`
    <div class="wrap">
      ...
      <div class="pdp">
        ...
        <div class="pdp-info">
          ...
          <div class="pdp-price">
            <div class="row"><span class="now tabnum">${k(t.price)}</span>${n?`<span class="off">-${n}%</span>`:""}</div>
            ${n?`<div class="row"><span class="was">Retail <s class="tabnum">${k(t.originalPrice)}</s></span><span class="save">You save ${k(t.originalPrice-t.price)}</span></div>`:""}
          </div>
          <ul class="facts">
            <li>${h("truck")}<div><b>Shipping: ${t.shipping>0?k(t.shipping):"Free"}</b><span>Ships in 1\u20132 business days.</span></div></li>
            <li>${h("store")}<div><b>Local Pickup Available</b><span>Free \xB7 ${l(a.city)}, ${l(a.state)} \xB7 ${l(a.pickupHours)}</span></div></li>
          </ul>
          <div class="buy-row">
    ...
    <div class="wrap section" style="padding-top:28px"><div class="prose">${a?`
      ...
      <h2>Start a return</h2><p>Call or text ${l(t.phone)} or email ${l(t.email)} with your order number.</p>`}</div></div>`}}function Fe(){let e=g.settings();return{title:"Checkout",noindex:!0,description:"Secure checkout \u2014 guest checkout, shipping or free local pickup.",html:'<div class="page-head"><div class="wrap"><h1>Checkout</h1><p>No account needed. Choose shipping or free local pickup.</p></div></div><div class="wrap" data-co></div>',mount(t,{navigate:a}){let n=y("[data-co]",t),s={fulfillment:"shipping",payment:"link",...T.get("ml-co",{})},i=()=>{let o=B.items();if(!o.length){n.innerHTML=`<div class="empty" style="margin-block:32px 64px"><h2>Your cart is empty</h2><p class="muted">Add something from today\u2019s deals to check out.</p><a class="btn btn-primary" href="${f("/products?deals=1")}">Browse Deals</a></div>`;return}let r=s.fulfillment==="pickup",c=O(o.reduce((p,m)=>p+m.product.shipping,0)),d=B.subtotal();!r&&s.payment==="pickup"&&(s.payment="link"),n.innerHTML=`<form class="checkout" novalidate data-co-form>
          <div>
            ...
            <section class="co-section"><h2><span class="n">2</span> Delivery</h2><div class="co-body">
              <div class="radio-cards" role="radiogroup" aria-label="Delivery method">
                <label class="radio-card"><input type="radio" name="fulfillment" value="shipping" ${r?"":"checked"}><div><b>Ship to me</b><span>Ships in 1\u20132 business days</span></div><span class="rc-price tabnum">${k(c)}</span></label>
                <label class="radio-card"><input type="radio" name="fulfillment" value="pickup" ${r?"checked":""}><div><b>Local pickup</b><span>${l(e.city)}, ${l(e.state)} \xB7 ready in about 1 business day</span></div><span class="rc-price">Free</span></label>
              </div>
              ${r?`<div class="pickup-box"><b>${h("store","icon-sm")} Pickup location</b><span>${l(e.address1)}, ${l(e.city)}, ${l(e.state)} ${l(e.zip)}</span><span>${l(e.pickupHours)}</span><span class="muted">${l(e.pickupNote)}</span></div>`:`
            ...
            <section class="co-section"><h2><span class="n">3</span> Payment</h2><div class="co-body">
              <div class="radio-cards" role="radiogroup" aria-label="Payment method">
                ...
                ${r?`<label class="radio-card"><input type="radio" name="payment" value="pickup" ${s.payment==="pickup"?"checked":""}><div><b>Pay at pickup</b><span>Cash or card when you collect your order.</span></div>${h("cash")}</label>`:""}
              </div>
          ...
          <aside class="summary" aria-label="Order summary">
            <h2>Order summary</h2>
            <ul class="sum-items">${o.map(({product:p,qty:m})=>`<li class="sum-item"><span class="th">${A(p.images[0],{alt:"",sizes:"56px"})}<span class="q">${m}</span></span><span class="t">${l(p.title)}<small>${l(p.condition)} \xB7 ${m} \xD7 ${k(p.price)}${r?"":` \xB7 ship ${k(p.shipping)}`}</small></span><span class="p tabnum">${k(p.price*m)}</span></li>`).join("")}</ul>
            <div class="sum-totals">
              <div class="row"><span>Subtotal</span><span class="tabnum">${k(d)}</span></div>
              <div class="row"><span>${r?"Local pickup":"Shipping"}</span><span class="tabnum">${r?"Free":k(c)}</span></div>
              <div class="row total"><span>Total</span><span class="tabnum">${k(d+(r?0:c))}</span></div>
              <p class="form-error" data-err hidden></p>
        ...
        </form>`};i(),n.addEventListener("input",o=>{let r=o.target.closest("form");if(!r)return;let c=Object.fromEntries(new FormData(r));Object.assign(s,c,{create:!!c.create}),delete s.password,T.set("ml-co",s)}),n.addEventListener("change",o=>{if(o.target.name==="fulfillment"||o.target.name==="payment"){let r=o.target.closest("form");Object.assign(s,Object.fromEntries(new FormData(r))),delete s.password,T.set("ml-co",s),i()}o.target.matches("[data-create]")&&(y("[data-pw]",n).hidden=!o.target.checked,s.create=o.target.checked)}),n.addEventListener("submit",async o=>{o.preventDefault();let r=o.target,c=Object.fromEntries(new FormData(r)),d=y("[data-err]",r),p=c.fulfillment==="pickup",m=[["co-name",!String(c.name).trim()],["co-email",!/^\S+@\S+\.\S+$/.test(c.email)],["co-phone",String(c.phone).replace(/\D/g,"").length<10]];p||m.push(["co-a1",!String(c.line1).trim()],["co-city",!String(c.city).trim()],["co-state",!/^[A-Za-z]{2}$/.test(String(c.state).trim())],["co-zip",!/^\d{5}(-\d{4})?$/.test(String(c.zip).trim())]),c.create&&m.push(["co-pw",String(c.password||"").length<8]),m.forEach(([v,u])=>{let $=y("#"+v,r);$&&$.setAttribute("aria-invalid",u?"true":"false")});let w=m.find(([,v])=>v);if(w){d.textContent="Please complete the highlighted fields.",d.hidden=!1,y("#"+w[0],r).focus();return}let M=y('button[type="submit"]',r);M.disabled=!0,M.textContent="Placing order\u2026";try{let v=await g.placeOrder({items:B.items().map(u=>({id:u.product.id,qty:u.qty})),customer:{name:c.name.trim(),email:c.email.trim(),phone:c.phone.trim()},fulfillment:c.fulfillment,payment:c.payment,address:p?null:{line1:c.line1.trim(),line2:(c.line2||"").trim(),city:c.city.trim(),state:c.state.trim().toUpperCase(),zip:c.zip.trim()},notes:(c.notes||"").trim(),createAccount:c.create?{password:c.password}:null});B.clear(),T.set("ml-last-order",v.id),a("/order/"+v.id)}catch(v){d.textContent=v.message||"We could not place your order. Please try again.",d.hidden=!1,M.disabled=!1,M.innerHTML=`${h("lock","icon-sm")} Place order`}})}}}function Ue({params:e}){return{title:"Order confirmed",noindex:!0,html:'<div class="wrap" data-order><p style="padding:48px 0" class="muted">Loading your order\u2026</p></div>',async mount(t){let a=y("[data-order]",t),n=await g.getOrder(e.id),s=g.settings();if(!n){a.innerHTML=`<div class="empty" style="margin-block:40px"><h2>Order not found</h2><p class="muted">If you just placed an order, check your email or call ${l(s.phone)}.</p><a class="btn btn-primary" href="${f("/")}">Back to home</a></div>`;return}let i=n.fulfillment==="pickup";a.innerHTML=`<div class="confirm">
        ...
        <dl class="kv">
          ...
          <div><dt>Total</dt><dd class="tabnum">${k(n.total)}</dd></div>
        </dl>
        ...
        <div class="summary" style="position:static"><h2>Items</h2><ul class="sum-items" style="max-height:none">${n.items.map(o=>`<li class="sum-item"><span class="th">${A(o.image,{alt:"",sizes:"56px"})}<span class="q">${o.qty}</span></span><span class="t">${l(o.title)}<small>${o.qty} \xD7 ${k(o.price)}</small></span><span class="p tabnum">${k(o.price*o.qty)}</span></li>`).join("")}</ul>
        <div class="sum-totals"><div class="row"><span>Subtotal</span><span class="tabnum">${k(n.subtotal)}</span></div><div class="row"><span>${i?"Local pickup":"Shipping"}</span><span class="tabnum">${i?"Free":k(n.shippingTotal)}</span></div><div class="row total"><span>Total</span><span class="tabnum">${k(n.total)}</span></div></div></div>
