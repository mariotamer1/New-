...
\u2022 6\u20138 years old
\u2022 10" tall`]],Pe=Date.UTC(2026,8,20),Ae=Ee.map((n,p)=>{let[S,$,d,M,y,m,N,C,c,se,Y]=n,E=(be[S]||[]).map((O,V)=>({sm:`img/p/${S}-${O}-sm.webp`,lg:`img/p/${S}-${O}-lg.webp`,alt:V===0?$:`${$} \u2013 view ${V+1}`}));return{id:"p-"+String(p+1).padStart(3,"0"),slug:S,title:$,brand:d,categoryId:De(M),condition:y,price:m,originalPrice:N,shipping:C,inventory:c,bestDeal:se,published:!0,upc:Le(p+11),description:Y,images:E,createdAt:new Date(Pe-p*36e5*7).toISOString()}});var Qe={products:[],categories:[],settings:{...fe}};function k(n,p=""){let S=g(".toasts");S||(S=document.createElement("div"),S.className="toasts",S.setAttribute("role","status"),S.setAttribute("aria-live","polite"),document.body.appendChild(S));let $=document.createElement("div");$.className="toast "+p,$.innerHTML=`${h(p==="err"?"close":"check","icon-sm")}<span>${i(n)}</span>`,S.appendChild($),setTimeout(()=>{$.style.opacity="0",$.style.transition="opacity .3s",setTimeout(()=>$.remove(),320)},2600)}var Te=[["overview","Overview","chart"],["products","Products","tag"],["orders","Orders","box"],["offers","Offers","cash"],["messages","Messages","msg"],["categories","Categories","layers"],["settings","Settings","gear"]],Oe=["New","Open Box","Like New","Refurbished","Used \u2013 Good","Used \u2013 Fair","Scratch & Dent"],ae=[["new","New"],["processing","Processing"],["ready","Ready for pickup"],["shipped","Shipped"],["completed","Completed"],["cancelled","Cancelled"]],ge=[["new","New"],["contacted","Contacted"],["accepted","Accepted"],["declined","Declined"]],Be=[["new","New"],["read","Read"],["archived","Archived"]];function Ie(n,{store:p,navigate:S,query:$}){let d=p.admin,M=$.get("tab")||"overview",y=$.get("edit")||null,m={products:[],orders:[],offers:[],inquiries:[]},N={q:"",f:"all"},C=new Set,c=null,se=()=>{let t=new URLSearchParams;M!=="overview"&&t.set("tab",M),y&&t.set("edit",y);let e="/admin"+(t.toString()?"?"+t:"");T.routing==="path"?history.replaceState({},"",U(e)):T.routing==="hash"&&history.replaceState(null,"","#"+e)},Y=t=>(p.category(t)||{}).name||"\u2014",E=()=>{p.refresh(),window.dispatchEvent(new CustomEvent("cart:change"))};async function O(){let[t,e,a,s]=await Promise.all([d.products(),d.orders(),d.offers(),d.inquiries()]);m={products:t,orders:e,offers:a,inquiries:s}}async function V(t){n.innerHTML=`<div class="adm-login"><form class="adm-login-card" novalidate data-login>
    ...
    </form></div>`,setTimeout(()=>{let e=g("#adm-pw");e&&e.focus()},30)}function q(t,e,a=""){let s=(L,B="new")=>m[L].filter(o=>o.status===B).length,r={orders:s("orders"),offers:s("offers"),messages:s("inquiries")};return`<div class="admin">
      <aside class="adm-side on-ink">
        <a class="logo" href="${U("/")}" aria-label="View store">${te()}</a>
        <span class="adm-tag">Admin</span>
        <nav aria-label="Admin"><ul>${Te.map(([L,B,o])=>`<li><button type="button" data-tab="${L}" ${M===L?'aria-current="page"':""}>${h(o)}<span>${B}</span>${r[L]?`<b class="adm-badge">${r[L]}</b>`:""}</button></li>`).join("")}</ul></nav>
    ...
    </div>`}async function I(){if(!await d.session()){V();return}if(await O(),se(),M==="products"&&y)return ve();({overview:ie,products:G,orders:Q,offers:le,messages:ce,categories:K,settings:$e}[M]||ie)()}function ie(){let t=m.products,e=t.filter(o=>o.published&&o.inventory>0),a=t.filter(o=>o.inventory<=0),s=t.filter(o=>o.inventory>0&&o.inventory<=2).slice(0,6),r=Date.now()-30*864e5,L=m.orders.filter(o=>o.status!=="cancelled"&&new Date(o.createdAt)>r).reduce((o,x)=>o+x.total,0),B=[["Live listings",e.length,"Published and in stock"],["Units in stock",t.reduce((o,x)=>o+Math.max(0,x.inventory),0),`${a.length} sold out (hidden)`],["New orders",m.orders.filter(o=>o.status==="new").length,`${m.orders.length} total`],["New offers",m.offers.filter(o=>o.status==="new").length,`${m.offers.length} total`],["Sales \xB7 30 days",D(L),"Excludes cancelled"],["Messages",m.inquiries.filter(o=>o.status==="new").length,"Contact + wholesale"]];n.innerHTML=q(`
      <div class="adm-tiles">${B.map(([o,x,l])=>`<div class="adm-tile"><span class="eyebrow">${o}</span><b class="tabnum">${x}</b><span class="muted">${l}</span></div>`).join("")}</div>
      <div class="adm-cols">
        ...
        <section class="adm-card"><div class="adm-card-head"><h2>Latest offers</h2><button class="link" type="button" data-tab="offers">All offers</button></div>
          ${m.offers.length?`<ul class="adm-list">${m.offers.slice(0,6).map(o=>`<li><button type="button" data-tab="offers"><span><b>${D(o.amount)}</b> for ${i(o.productTitle)}<small>${i(o.name)} \xB7 ${i(o.phone)} \xB7 ${W(o.createdAt,!0)}</small></span><span class="status s-${o.status}">${oe(ge,o.status)}</span></button></li>`).join("")}</ul>`:'<p class="muted adm-empty">No offers yet. Offers from the \u201CMake Offer\u201D button land here.</p>'}
        </section>
      ...
      </div>`,"Overview",`<button class="btn btn-primary" type="button" data-new>${h("plus","icon-sm")} Add product</button>`)}let oe=(t,e)=>(t.find(([a])=>a===e)||[e,e])[1];function G(){let t=N.q.toLowerCase(),e=m.products.filter(s=>!t||`${s.title} ${s.brand||""} ${s.upc||""} ${s.slug}`.toLowerCase().includes(t)),a=N.f;a==="live"&&(e=e.filter(s=>s.published&&s.inventory>0)),a==="hidden"&&(e=e.filter(s=>!s.published)),a==="out"&&(e=e.filter(s=>s.inventory<=0)),a==="deal"&&(e=e.filter(s=>s.bestDeal)),C=new Set([...C].filter(s=>m.products.some(r=>r.id===s))),n.innerHTML=q(`
      <div class="adm-toolbar">
        <div class="search-field adm-search">${h("search")}<label class="sr-only" for="pf-q">Search products</label><input id="pf-q" type="search" placeholder="Search title, brand, UPC" value="${i(N.q)}" data-pf-q></div>
      ...
      <div class="table-wrap"><table class="tbl adm-table"><thead><tr>
        ...
        ${e.map(s=>`<tr data-row="${s.id}" class="${s.inventory<=0||!s.published?"dim":""}">
          ...
          <td><div class="adm-prod">${s.images[0]?`<img src="${i(F(s.images[0].sm))}" alt="" width="48" height="48" loading="lazy">`:'<span class="adm-noimg"></span>'}<div><button class="adm-title" type="button" data-edit="${s.id}">${i(s.title)}</button><small>${i(Y(s.categoryId))} \xB7 ${i(s.condition)}${s.upc?" \xB7 "+i(s.upc):""}</small></div></div></td>
          ...
          <td class="adm-row-actions"><button class="btn btn-sm" type="button" data-edit="${s.id}">${h("edit","icon-sm")} Edit</button>${s.published&&s.inventory>0?`<a class="btn btn-sm" href="${U("/products/"+s.slug)}" target="_blank" rel="noopener" aria-label="View ${i(s.title)} on the store">${h("eye","icon-sm")}</a>`:""}</td>
        </tr>`).join("")||'<tr><td colspan="6" class="muted">No products match.</td></tr>'}
      </tbody></table></div>`,"Products",`<button class="btn btn-primary" type="button" data-new>${h("plus","icon-sm")} Add product</button>`)}function ve(){let t=y==="new",e=t?null:m.products.find(r=>r.id===y);if(!t&&!e)return y=null,G();(!c||c._for!==y)&&(c=e?JSON.parse(JSON.stringify(e)):{title:"",slug:"",brand:"",upc:"",condition:"New",categoryId:(p.categories()[0]||{}).id||null,description:"",price:"",originalPrice:"",shipping:9.99,inventory:1,published:!0,bestDeal:!1,images:[]},c._for=y);let a=c,s=a.originalPrice&&a.price&&Number(a.originalPrice)>Number(a.price)?Math.round((1-a.price/a.originalPrice)*100):"";n.innerHTML=q(`
      ...
      </form>`,t?"New product":"Edit product",`<button class="btn" type="button" data-cancel-edit>${h("left","icon-sm")} All products</button>`),ye()}function re(t){return t.images.length?t.images.map((e,a)=>`<figure class="adm-photo" draggable="true" data-ph="${a}">
      ...
      </figcaption></figure>`).join(""):'<p class="muted" style="font-size:14px">No photos yet.</p>'}function ye(){let t=g("[data-editor]"),e=l=>t.elements[l],a=()=>{g("[data-photos]").innerHTML=re(c)},s=()=>{["title","description","brand","upc","slug","condition","categoryId"].forEach(l=>{c[l]=e(l).value}),c.price=e("price").value,c.originalPrice=e("originalPrice").value,c.shipping=e("shipping").value,c.inventory=e("inventory").value,c.published=e("published").checked,c.bestDeal=e("bestDeal").checked};t.addEventListener("input",l=>{let f=l.target.name,v=parseFloat(e("originalPrice").value),A=parseFloat(e("pct").value),u=parseFloat(e("price").value);f==="pct"&&v>0&&A>=0&&(e("price").value=R(v*(1-A/100)).toFixed(2)),f==="price"&&v>0&&u>=0&&(e("pct").value=u<v?Math.round((1-u/v)*100):""),f==="originalPrice"&&v>0&&A>0&&(e("price").value=R(v*(1-A/100)).toFixed(2)),(f==="title"||f==="slug")&&(g("[data-slug-preview]").textContent=ee(e("slug").value||e("title").value)||"product-name"),s()}),t.addEventListener("change",s),t.addEventListener("click",async l=>{let f=l.target.closest("[data-ed-inv]");f&&(e("inventory").value=Math.max(0,(parseInt(e("inventory").value,10)||0)+Number(f.dataset.edInv)),s());let v=l.target.closest("[data-ph-move]");if(v){let u=Number(v.closest("[data-ph]").dataset.ph),b=u+Number(v.dataset.phMove);[c.images[u],c.images[b]]=[c.images[b],c.images[u]],a()}let A=l.target.closest("[data-ph-del]");if(A){let u=Number(A.closest("[data-ph]").dataset.ph),[b]=c.images.splice(u,1);d.removeImage&&d.removeImage(b).catch(()=>{}),a()}l.target.closest("[data-delete-product]")&&(g("[data-confirm]").hidden=!1),l.target.closest("[data-confirm-cancel]")&&(g("[data-confirm]").hidden=!0),l.target.closest("[data-confirm-delete]")&&(await d.deleteProduct(y),E(),k("Product deleted"),y=null,c=null,I())});let r=null,L=g("[data-photos]");L.addEventListener("dragstart",l=>{let f=l.target.closest("[data-ph]");f&&(r=Number(f.dataset.ph),l.dataTransfer.effectAllowed="move",f.classList.add("dragging"))}),L.addEventListener("dragover",l=>{r!==null&&l.preventDefault()}),L.addEventListener("drop",l=>{let f=l.target.closest("[data-ph]");if(r===null||!f)return;l.preventDefault();let v=Number(f.dataset.ph),[A]=c.images.splice(r,1);c.images.splice(v,0,A),r=null,a()}),L.addEventListener("dragend",()=>{r=null,J(".dragging").forEach(l=>l.classList.remove("dragging"))});let B=g("[data-upload-status]"),o=async l=>{if(l=Array.from(l).filter(f=>f.type.startsWith("image/")),!!l.length){for(let f=0;f<l.length;f++){B.textContent=`Uploading ${f+1} of ${l.length}\u2026`;try{let v=await d.uploadImage(l[f]);c.images.push({...v,alt:""}),a()}catch(v){k(`Couldn\u2019t upload ${l[f].name}: ${v.message}`,"err")}}B.textContent=`${l.length} photo${l.length>1?"s":""} added \u2014 remember to save.`}};g("[data-files]").addEventListener("change",l=>{o(l.target.files),l.target.value=""});let x=g("[data-drop]");x.addEventListener("dragover",l=>{l.dataTransfer.types.includes("Files")&&(l.preventDefault(),x.classList.add("over"))}),x.addEventListener("dragleave",()=>x.classList.remove("over")),x.addEventListener("drop",l=>{l.dataTransfer.files.length&&(l.preventDefault(),x.classList.remove("over"),o(l.dataTransfer.files))}),t.addEventListener("submit",async l=>{l.preventDefault(),s();let f=g("[data-err]"),v=parseFloat(c.price),A=parseFloat(c.originalPrice),u=parseFloat(c.shipping||0),b=parseInt(c.inventory,10),w=String(c.upc||"").replace(/\D/g,""),P=[];if(c.title.trim()||P.push(["ed-title","Add a title."]),v>0||P.push(["ed-price","Enter a sale price greater than $0."]),b>=0||P.push(["ed-inv","Inventory must be 0 or more."]),u>=0||P.push(["ed-ship","Shipping must be 0 or more."]),w&&(w.length<8||w.length>14)&&P.push(["ed-upc","UPC should be 8\u201314 digits."]),J("[aria-invalid]",t).forEach(H=>H.removeAttribute("aria-invalid")),P.length){P.forEach(([H])=>g("#"+H).setAttribute("aria-invalid","true")),f.textContent=P.map(H=>H[1]).join(" "),f.hidden=!1,g("#"+P[0][0]).focus();return}let z={id:y==="new"?void 0:y,title:c.title.trim(),slug:c.slug.trim(),brand:c.brand.trim(),upc:w,condition:c.condition,categoryId:c.categoryId||null,description:c.description.trim(),price:R(v),originalPrice:A>v?R(A):null,shipping:R(u),inventory:b,published:c.published,bestDeal:c.bestDeal,images:c.images},j=g('button[type="submit"]',t);j.disabled=!0,j.textContent="Saving\u2026";try{let H=await d.saveProduct(z);return E(),k(y==="new"?"Product created":"Changes saved"),y=null,c=null,M="products",I(),H}catch(H){f.textContent=H.message,f.hidden=!1,j.disabled=!1,j.textContent="Save changes"}})}let Z="all";function Q(){let t=m.orders.filter(e=>Z==="all"||e.status===Z);n.innerHTML=q(`
      <div class="adm-toolbar"><div class="select"><label class="sr-only" for="of-f">Status</label><select id="of-f" data-of>${[["all","All orders"],...ae].map(([e,a])=>`<option value="${e}" ${Z===e?"selected":""}>${a}</option>`).join("")}</select>${h("down")}</div><span class="muted" style="font-size:14px">${t.length} order${t.length===1?"":"s"}</span></div>
      ...
      <dialog class="modal adm-order-dlg" id="order-dlg" aria-labelledby="od-title"></dialog>`,"Orders")}function we(t){let e=m.orders.find(r=>r.id===t);if(!e)return;g("#order-dlg")||(M="orders",Q());let a=g("#order-dlg"),s=e.address?`${i(e.address.line1)}${e.address.line2?", "+i(e.address.line2):""}<br>${i(e.address.city)}, ${i(e.address.state)} ${i(e.address.zip)}`:"Local pickup";a.innerHTML=`<div class="modal-head"><h2 id="od-title">Order #${e.number}</h2><button class="icon-btn" type="button" aria-label="Close" data-close-dlg>${h("close")}</button></div>
      ...
      <div class="adm-msg-actions"><label class="sr-only" for="ms-${t.id}">Status</label><select id="ms-${t.id}" class="adm-status" data-msg-status="${t.id}">${Be.map(([e,a])=>`<option value="${e}" ${t.status===e?"selected":""}>${a}</option>`).join("")}</select><button class="btn btn-sm" type="button" data-msg-del="${t.id}">${h("trash","icon-sm")} Delete</button></div></div>
    </article>`).join("")}</div>`:'<div class="empty"><h2>No messages</h2><p class="muted">Contact form and wholesale requests appear here.</p></div>',"Messages")}function K(){let t=p.categories(),e={};m.products.forEach(a=>{e[a.categoryId]=(e[a.categoryId]||0)+1}),n.innerHTML=q(`<div class="adm-card"><div class="adm-card-body">
