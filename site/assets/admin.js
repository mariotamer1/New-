...
\u2022 6\u20138 years old
\u2022 10" tall`]],Pe=Date.UTC(2026,8,20),Ae=Ee.map((n,p)=>{let[x,$,d,M,y,h,I,C,c,se,Y]=n,E=(be[x]||[]).map((O,V)=>({sm:`img/p/${x}-${O}-sm.webp`,lg:`img/p/${x}-${O}-lg.webp`,alt:V===0?$:`${$} \u2013 view ${V+1}`}));return{id:"p-"+String(p+1).padStart(3,"0"),slug:x,title:$,brand:d,categoryId:De(M),condition:y,price:h,originalPrice:I,shipping:C,inventory:c,bestDeal:se,published:!0,noShipping:x==="live-bonsai-tree",upc:Le(p+11),description:Y,images:E,createdAt:new Date(Pe-p*36e5*7).toISOString()}});var Qe={products:[],categories:[],settings:{...fe}};function k(n,p=""){let x=g(".toasts");x||(x=document.createElement("div"),x.className="toasts",x.setAttribute("role","status"),x.setAttribute("aria-live","polite"),document.body.appendChild(x));let $=document.createElement("div");$.className="toast "+p,$.innerHTML=`${m(p==="err"?"close":"check","icon-sm")}<span>${i(n)}</span>`,x.appendChild($),setTimeout(()=>{$.style.opacity="0",$.style.transition="opacity .3s",setTimeout(()=>$.remove(),320)},2600)}var Te=[["overview","Overview","chart"],["products","Products","tag"],["orders","Orders","box"],["offers","Offers","cash"],["messages","Messages","msg"],["categories","Categories","layers"],["settings","Settings","gear"]],Oe=["New","Open Box","Like New","Refurbished","Used \u2013 Good","Used \u2013 Fair","Scratch & Dent"],ae=[["new","New"],["processing","Processing"],["ready","Ready for pickup"],["shipped","Shipped"],["completed","Completed"],["cancelled","Cancelled"]],ge=[["new","New"],["contacted","Contacted"],["accepted","Accepted"],["declined","Declined"]],Ne=[["new","New"],["read","Read"],["archived","Archived"]];function Be(n,{store:p,navigate:x,query:$}){let d=p.admin,M=$.get("tab")||"overview",y=$.get("edit")||null,h={products:[],orders:[],offers:[],inquiries:[]},I={q:"",f:"all"},C=new Set,c=null,se=()=>{let t=new URLSearchParams;M!=="overview"&&t.set("tab",M),y&&t.set("edit",y);let e="/admin"+(t.toString()?"?"+t:"");T.routing==="path"?history.replaceState({},"",U(e)):T.routing==="hash"&&history.replaceState(null,"","#"+e)},Y=t=>(p.category(t)||{}).name||"\u2014",E=()=>{p.refresh(),window.dispatchEvent(new CustomEvent("cart:change"))};async function O(){let[t,e,a,s]=await Promise.all([d.products(),d.orders(),d.offers(),d.inquiries()]);h={products:t,orders:e,offers:a,inquiries:s}}async function V(t){n.innerHTML=`<div class="adm-login"><form class="adm-login-card" novalidate data-login>
    ...
    </div>`}async function B(){if(!await d.session()){V();return}if(await O(),se(),M==="products"&&y)return ve();({overview:ie,products:G,orders:Q,offers:le,messages:ce,categories:K,settings:$e}[M]||ie)()}function ie(){let t=h.products,e=t.filter(o=>o.published&&o.inventory>0),a=t.filter(o=>o.inventory<=0),s=t.filter(o=>o.inventory>0&&o.inventory<=2).slice(0,6),r=Date.now()-30*864e5,L=h.orders.filter(o=>o.status!=="cancelled"&&new Date(o.createdAt)>r).reduce((o,S)=>o+S.total,0),N=[["Live listings",e.length,"Published and in stock"],["Units in stock",t.reduce((o,S)=>o+Math.max(0,S.inventory),0),`${a.length} sold out (hidden)`],["New orders",h.orders.filter(o=>o.status==="new").length,`${h.orders.length} total`],["New offers",h.offers.filter(o=>o.status==="new").length,`${h.offers.length} total`],["Sales \xB7 30 days",D(L),"Excludes cancelled"],["Messages",h.inquiries.filter(o=>o.status==="new").length,"Contact + wholesale"]];n.innerHTML=q(`
      <div class="adm-tiles">${N.map(([o,S,l])=>`<div class="adm-tile"><span class="eyebrow">${o}</span><b class="tabnum">${S}</b><span class="muted">${l}</span></div>`).join("")}</div>
      <div class="adm-cols">
