...
\u2022 10" tall`]],Ve=Date.UTC(2026,8,20),Ae=We.map((n,p)=>{let[x,C,d,M,S,h,G,E,c,ue,ne]=n,L=(Le[x]||[]).map((O,Q)=>({sm:`img/p/${x}-${O}-sm.webp`,lg:`img/p/${x}-${O}-lg.webp`,alt:Q===0?C:`${C} \u2013 view ${Q+1}`}));return{id:"p-"+String(p+1).padStart(3,"0"),slug:x,title:C,brand:d,categoryId:Ge(M),condition:S,price:h,originalPrice:G,shipping:E,inventory:c,bestDeal:ue,published:!0,noShipping:x==="live-bonsai-tree",upc:Fe(p+11),description:ne,images:L,createdAt:new Date(Ve-p*36e5*7).toISOString()}});var gt={products:[],categories:[],settings:{...Te}};function $(n,p=""){let x=b(".toasts");x||(x=document.createElement("div"),x.className="toasts",x.setAttribute("role","status"),x.setAttribute("aria-live","polite"),document.body.appendChild(x));let C=document.createElement("div");C.className="toast "+p,C.innerHTML=`${u(p==="err"?"close":"check","icon-sm")}<span>${o(n)}</span>`,x.appendChild(C),setTimeout(()=>{C.style.opacity="0",C.style.transition="opacity .3s",setTimeout(()=>C.remove(),320)},2600)}var Qe=[["overview","Overview","chart"],["products","Products","tag"],["orders","Orders","box"],["offers","Offers","cash"],["requests","Requests","calendar"],["messages","Messages","msg"],["categories","Categories","layers"],["settings","Settings","gear"]],Xe=["New","Open Box","Like New","Refurbished","Used \u2013 Good","Used \u2013 Fair","Scratch & Dent"],de=[["new","New"],["processing","Processing"],["ready","Ready for pickup"],["shipped","Shipped"],["completed","Completed"],["cancelled","Cancelled"]],Oe=[["new","New"],["contacted","Contacted"],["accepted","Accepted"],["declined","Declined"]],et=[["new","New"],["read","Read"],["archived","Archived"]];function tt(n,{store:p,navigate:x,query:C}){let d=p.admin,M=C.get("tab")||"overview",S=C.get("edit")||null,h={products:[],orders:[],offers:[],inquiries:[]},G={q:"",f:"all"},E=new Set,c=null,ue=()=>{let s=new URLSearchParams;M!=="overview"&&s.set("tab",M),S&&s.set("edit",S);let e="/admin"+(s.toString()?"?"+s:"");D.routing==="path"?history.replaceState({},"",V(e)):D.routing==="hash"&&history.replaceState(null,"","#"+e)},ne=s=>(p.category(s)||{}).name||"\u2014",L=()=>{p.refresh(),window.dispatchEvent(new CustomEvent("cart:change"))};async function O(){let[s,e,t,a]=await Promise.all([d.products(),d.orders(),d.offers(),d.inquiries()]);h={products:s,orders:e,offers:t,inquiries:a}}async function Q(s){n.innerHTML=`<div class="adm-login"><form class="adm-login-card" novalidate data-login>
    ...
    </div>`}async function U(){if(!await d.session()){Q();return}if(await O(),ue(),M==="products"&&S)return Ie();({overview:ve,products:X,orders:K,offers:pe,requests:me,messages:ke,categories:te,settings:Ne}[M]||ve)()}function ve(){let s=h.products,e=s.filter(r=>r.published&&r.inventory>0),t=s.filter(r=>r.inventory<=0),a=s.filter(r=>r.inventory>0&&r.inventory<=2).slice(0,6),i=Date.now()-30*864e5,v=h.orders.filter(r=>r.status!=="cancelled"&&new Date(r.createdAt)>i).reduce((r,P)=>r+P.total,0),T=[["Live listings",e.length,"Published and in stock"],["Units in stock",s.reduce((r,P)=>r+Math.max(0,P.inventory),0),`${t.length} sold out (hidden)`],["New orders",h.orders.filter(r=>r.status==="new").length,`${h.orders.length} total`],["New offers",h.offers.filter(r=>r.status==="new").length,`${h.offers.length} total`],["Sales \xB7 30 days",k(v),"Excludes cancelled"],["Messages",h.inquiries.filter(r=>r.status==="new").length,"Contact + wholesale"]];n.innerHTML=W(`
      ...
      </form>`,s?"New product":"Edit product",`<button class="btn" type="button" data-cancel-edit>${u("left","icon-sm")} All products</button>`),De()}function we(s){return s.images.length?s.images.map((e,t)=>`<figure class="adm-photo" draggable="true" data-ph="${t}">
