...
export function createSupabaseBackend(cfg) {
  ...
  const backend = {
    ...
    cancelOrder: (id) => raw('POST', '/functions/v1/paypal', { action: 'customer_cancel', orderId: id }),
    ...
    admin: {
      ...
      async setTracking(id, tracking) { await rest('PATCH', `orders?id=eq.${id}`, { tracking }); },
