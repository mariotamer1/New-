...
\u2022 6\u20138 years old
\u2022 10" tall`]],Ve=Date.UTC(2026,8,20),Ae=We.map((n,p)=>{let[x,C,d,M,S,h,G,E,c,ue,ne]=n,L=(Le[x]||[]).map((O,Q)=>({sm:`img/p/${x}-${O}-sm.webp`,lg:`img/p/${x}-${O}-lg.webp`,alt:Q===0?C:`${C} \u2013 view ${Q+1}`}));return{id:"p-"+String(p+1).padStart(3,"0"),slug:x,title:C,brand:d,categoryId:Ge(M),condition:S,price:h,originalPrice:G,shipping:E,inventory:c,bestDeal:ue,published:!0,noShipping:x==="live-bonsai-tree",upc:Fe(p+11),description:ne,images:L,createdAt:new Date(Ve-p*36e5*7).toISOString()}});var gt={products:[],categories:[],settings:{...Te}};function $(n,p=""){let x=b(".toasts");x||(x=document.createElement("div"),x.className="toasts",x.setAttribute("role","status"),x.setAttribute("aria-live","polite"),document.body.appendChild(x));let C=document.createElement("div");C.className="toast "+p,C.innerHTML=`${u(p==="err"?"close":"check","icon-sm")}<span>${o(n)}</span>`,x.appendChild(C),setTimeout(()=>{C.style.opacity="0",C.style.transition="opacity .3s",setTimeout(()=>C.remove(),320)},2600)}var Qe=[["overview","Overview","chart"],["products","Products","tag"],["orders","Orders","box"],["offers","Offers","cash"],["requests","Requests","calendar"],["messages","Messages","msg"],["categories","Categories","layers"],["settings","Settings","gear"]],Xe=["New","Open Box","Like New","Refurbished","Used \u2013 Good","Used \u2013 Fair","Scratch & Dent"],de=[["new","New"],["processing","Processing"],["ready","Ready for pickup"],["shipped","Shipped"],["completed","Completed"],["cancelled","Cancelled"]],Oe=[["new","New"],["contacted","Contacted"],["accepted","Accepted"],["declined","Declined"]],et=[["new","New"],["read","Read"],["archived","Archived"]];function tt(n,{store:p,navigate:x,query:C}){let d=p.admin,M=C.get("tab")||"overview",S=C.get("edit")||null,h={products:[],orders:[],offers:[],inquiries:[]},G={q:"",f:"all"},E=new Set,c=null,ue=()=>{let s=new URLSearchParams;M!=="overview"&&s.set("tab",M),S&&s.set("edit",S);let e="/admin"+(s.toString()?"?"+s:"");I.routing==="path"?history.replaceState({},"",V(e)):I.routing==="hash"&&history.replaceState(null,"","#"+e)},ne=s=>(p.category(s)||{}).name||"\u2014",L=()=>{p.refresh(),window.dispatchEvent(new CustomEvent("cart:change"))};async function O(){let[s,e,t,a]=await Promise.all([d.products(),d.orders(),d.offers(),d.inquiries()]);h={products:s,orders:e,offers:t,inquiries:a}}async function Q(s){n.innerHTML=`<div class="adm-login"><form class="adm-login-card" novalidate data-login>
      <a class="logo adm-logo" href="${V("/")}" aria-label="Back to store">${ce()}</a>
    ...
    </form></div>`,setTimeout(()=>{let e=b("#adm-pw");e&&e.focus()},30)}function W(s,e,t=""){let a=(v,T="new")=>h[v].filter(r=>r.status===T).length,i={orders:a("orders"),offers:a("offers"),requests:h.inquiries.filter(v=>v.type==="pickup"&&v.status==="new").length,messages:h.inquiries.filter(v=>v.type!=="pickup"&&v.status==="new").length};return`<div class="admin">
      <aside class="adm-side on-ink">
        <a class="logo" href="${V("/")}" aria-label="View store">${ce()}</a>
    ...
    </div>`}async function U(){if(!await d.session()){Q();return}if(await O(),ue(),M==="products"&&S)return De();({overview:ve,products:X,orders:K,offers:pe,requests:me,messages:ke,categories:te,settings:Ne}[M]||ve)()}function ve(){let s=h.products,e=s.filter(r=>r.published&&r.inventory>0),t=s.filter(r=>r.inventory<=0),a=s.filter(r=>r.inventory>0&&r.inventory<=2).slice(0,6),i=Date.now()-30*864e5,v=h.orders.filter(r=>r.status!=="cancelled"&&new Date(r.createdAt)>i).reduce((r,P)=>r+P.total,0),T=[["Live listings",e.length,"Published and in stock"],["Units in stock",s.reduce((r,P)=>r+Math.max(0,P.inventory),0),`${t.length} sold out (hidden)`],["New orders",h.orders.filter(r=>r.status==="new").length,`${h.orders.length} total`],["New offers",h.offers.filter(r=>r.status==="new").length,`${h.offers.length} total`],["Sales \xB7 30 days",k(v),"Excludes cancelled"],["Messages",h.inquiries.filter(r=>r.status==="new").length,"Contact + wholesale"]];n.innerHTML=W(`
      ...
      <div class="adm-cols">
        ...
        <section class="adm-card"><div class="adm-card-head"><h2>Latest offers</h2><button class="link" type="button" data-tab="offers">All offers</button></div>
          ${h.offers.length?`<ul class="adm-list">${h.offers.slice(0,6).map(r=>`<li><button type="button" data-tab="offers"><span><b>${k(r.amount)}</b> for ${o(r.productTitle)}<small>${o(r.name)} \xB7 ${o(r.phone)} \xB7 ${F(r.createdAt,!0)}</small></span><span class="status s-${r.status}">${ye(Oe,r.status)}</span></button></li>`).join("")}</ul>`:'<p class="muted adm-empty">No offers yet. Offers from the \u201CMake Offer\u201D button land here.</p>'}
        </section>
        <section class="adm-card"><div class="adm-card-head"><h2>Low stock</h2><button class="link" type="button" data-tab="products">Inventory</button></div>
          ${a.length?`<ul class="adm-list">${a.map(r=>`<li><button type="button" data-edit="${r.id}"><span>${o(r.title)}<small>${ne(r.categoryId)}</small></span><b class="tabnum">${r.inventory} left</b></button></li>`).join("")}</ul>`:'<p class="muted adm-empty">Nothing is running low.</p>'}
        </section>
      </div>`,"Overview",`<button class="btn btn-primary" type="button" data-new>${u("plus","icon-sm")} Add product</button>`)}let ye=(s,e)=>(s.find(([t])=>t===e)||[e,e])[1];function X(){let s=G.q.toLowerCase(),e=h.products.filter(a=>!s||`${a.title} ${a.brand||""} ${a.upc||""} ${a.slug}`.toLowerCase().includes(s)),t=G.f;t==="live"&&(e=e.filter(a=>a.published&&a.inventory>0)),t==="hidden"&&(e=e.filter(a=>!a.published)),t==="out"&&(e=e.filter(a=>a.inventory<=0)),E=new Set([...E].filter(a=>h.products.some(i=>i.id===a))),n.innerHTML=W(`
      <div class="adm-toolbar">
        ...
        <div class="select"><label class="sr-only" for="pf-f">Show</label><select id="pf-f" data-pf-f>${[["all","All products"],["live","Live"],["hidden","Unpublished"],["out","Sold out"]].map(([a,i])=>`<option value="${a}" ${t===a?"selected":""}>${i}</option>`).join("")}</select>${u("down")}</div>
      ...
      <div class="table-wrap"><table class="tbl adm-table"><thead><tr>
        <th><input type="checkbox" aria-label="Select all" data-sel-all ${e.length&&e.every(a=>E.has(a.id))?"checked":""}></th>
        ...
        ${e.map(a=>`<tr data-row="${a.id}" class="${a.inventory<=0||!a.published?"dim":""}">
          <td><input type="checkbox" aria-label="Select ${o(a.title)}" data-sel="${a.id}" ${E.has(a.id)?"checked":""}></td>
          <td><div class="adm-prod">${a.images[0]?`<img src="${o(Y(a.images[0].sm))}" alt="" width="48" height="48" loading="lazy">`:'<span class="adm-noimg"></span>'}<div><button class="adm-title" type="button" data-edit="${a.id}">${o(a.title)}</button><small>${o(ne(a.categoryId))} \xB7 ${o(a.condition)}${a.noShipping?" \xB7 Pick up only":""}${a.upc?" \xB7 "+o(a.upc):""}</small></div></div></td>
          <td class="tabnum"><b>${k(a.price)}</b>${le(a)?`<small><s>${k(a.originalPrice)}</s> \xB7 -${le(a)}%</small>`:""}</td>
          <td><div class="adm-stock"><button type="button" aria-label="Decrease inventory" data-inv="-1" data-id="${a.id}">${u("minus","icon-sm")}</button><b class="tabnum">${a.inventory}</b><button type="button" aria-label="Increase inventory" data-inv="1" data-id="${a.id}">${u("plus","icon-sm")}</button></div>${a.inventory<=0?"<small>Sold out \xB7 hidden</small>":""}</td>
          ...
          <td class="adm-row-actions"><button class="btn btn-sm" type="button" data-edit="${a.id}">${u("edit","icon-sm")} Edit</button>${a.published&&a.inventory>0?`<a class="btn btn-sm" href="${V("/products/"+a.slug)}" target="_blank" rel="noopener" aria-label="View ${o(a.title)} on the store">${u("eye","icon-sm")}</a>`:""}</td>
        </tr>`).join("")||'<tr><td colspan="6" class="muted">No products match.</td></tr>'}
      </tbody></table></div>`,"Products",`<button class="btn btn-primary" type="button" data-new>${u("plus","icon-sm")} Add product</button>`)}function De(){let s=S==="new",e=s?null:h.products.find(i=>i.id===S);if(!s&&!e)return S=null,X();(!c||c._for!==S)&&(c=e?JSON.parse(JSON.stringify(e)):{title:"",slug:"",brand:"",upc:"",condition:"New",categoryId:(p.categories()[0]||{}).id||null,description:"",price:"",originalPrice:"",shipping:9.99,inventory:1,published:!0,bestDeal:!1,images:[]},c._for=S);let t=c,a=t.originalPrice&&t.price&&Number(t.originalPrice)>Number(t.price)?Math.round((1-t.price/t.originalPrice)*100):"";n.innerHTML=W(`
      <form class="adm-editor" novalidate data-editor>
        ...
        <div class="adm-ed-side">
          ...
          <div class="adm-ed-actions">
            ...
            ${s?"":`<button class="btn btn-lg adm-danger" type="button" data-delete-product>${u("trash","icon-sm")} Delete</button>`}
          </div>
      ...
      </form>`,s?"New product":"Edit product",`<button class="btn" type="button" data-cancel-edit>${u("left","icon-sm")} All products</button>`),Ie()}function we(s){return s.images.length?s.images.map((e,t)=>`<figure class="adm-photo" draggable="true" data-ph="${t}">
      <img src="${o(Y(e.sm||e.lg))}" alt="Photo ${t+1}">
      ...
      <figcaption>
        ...
        <button type="button" aria-label="Move photo ${t+1} right" data-ph-move="1" ${t===s.images.length-1?"disabled":""}>${u("right","icon-sm")}</button>
        <button type="button" aria-label="Delete photo ${t+1}" data-ph-del>${u("trash","icon-sm")}</button>
      </figcaption></figure>`).join(""):'<p class="muted" style="font-size:14px">No photos yet.</p>'}function Ie(){let s=b("[data-editor]"),e=l=>s.elements[l],t=()=>{b("[data-photos]").innerHTML=we(c)},a=()=>{["title","description","brand","upc","slug","condition","categoryId"].forEach(l=>{c[l]=e(l).value}),c.price=e("price").value,c.originalPrice=e("originalPrice").value,c.shipping=e("shipping").value,c.inventory=e("inventory").value,c.published=e("published").checked,c.noShipping=e("noShipping").checked};s.addEventListener("input",l=>{let y=l.target.name,w=parseFloat(e("originalPrice").value),A=parseFloat(e("pct").value),D=parseFloat(e("price").value);y==="pct"&&w>0&&A>=0&&(e("price").value=_(w*(1-A/100)).toFixed(2)),y==="price"&&w>0&&D>=0&&(e("pct").value=D<w?Math.round((1-D/w)*100):""),y==="originalPrice"&&w>0&&A>0&&(e("price").value=_(w*(1-A/100)).toFixed(2)),(y==="title"||y==="slug")&&(b("[data-slug-preview]").textContent=se(e("slug").value||e("title").value)||"product-name"),a()}),s.addEventListener("change",a),s.addEventListener("click",async l=>{let y=l.target.closest("[data-ed-inv]");y&&(e("inventory").value=Math.max(0,(parseInt(e("inventory").value,10)||0)+Number(y.dataset.edInv)),a());let w=l.target.closest("[data-ph-move]");if(w){let D=Number(w.closest("[data-ph]").dataset.ph),N=D+Number(w.dataset.phMove);[c.images[D],c.images[N]]=[c.images[N],c.images[D]],t()}let A=l.target.closest("[data-ph-del]");if(A){let D=Number(A.closest("[data-ph]").dataset.ph),[N]=c.images.splice(D,1);d.removeImage&&d.removeImage(N).catch(()=>{}),t()}l.target.closest("[data-delete-product]")&&(b("[data-confirm]").hidden=!1),l.target.closest("[data-confirm-cancel]")&&(b("[data-confirm]").hidden=!0),l.target.closest("[data-confirm-delete]")&&(await d.deleteProduct(S),L(),$("Product deleted"),S=null,c=null,U())});let i=null,v=b("[data-photos]");v.addEventListener("dragstart",l=>{let y=l.target.closest("[data-ph]");y&&(i=Number(y.dataset.ph),l.dataTransfer.effectAllowed="move",y.classList.add("dragging"))}),v.addEventListener("dragover",l=>{i!==null&&l.preventDefault()}),v.addEventListener("drop",l=>{let y=l.target.closest("[data-ph]");if(i===null||!y)return;l.preventDefault();let w=Number(y.dataset.ph),[A]=c.images.splice(i,1);c.images.splice(w,0,A),i=null,t()}),v.addEventListener("dragend",()=>{i=null,H(".dragging").forEach(l=>l.classList.remove("dragging"))});let T=b("[data-upload-status]"),r=async l=>{if(l=Array.from(l).filter(y=>y.type.startsWith("image/")),!!l.length){for(let y=0;y<l.length;y++){T.textContent=`Uploading ${y+1} of ${l.length}\u2026`;try{let w=await d.uploadImage(l[y]);c.images.push({...w,alt:""}),t()}catch(w){$(`Couldn\u2019t upload ${l[y].name}: ${w.message}`,"err")}}T.textContent=`${l.length} photo${l.length>1?"s":""} added \u2014 remember to save.`}};b("[data-files]").addEventListener("change",l=>{r(l.target.files),l.target.value=""});let P=b("[data-drop]");P.addEventListener("dragover",l=>{l.dataTransfer.types.includes("Files")&&(l.preventDefault(),P.classList.add("over"))}),P.addEventListener("dragleave",()=>P.classList.remove("over")),P.addEventListener("drop",l=>{l.dataTransfer.files.length&&(l.preventDefault(),P.classList.remove("over"),r(l.dataTransfer.files))}),s.addEventListener("submit",async l=>{l.preventDefault(),a();let y=b("[data-err]"),w=parseFloat(c.price),A=parseFloat(c.originalPrice),D=parseFloat(c.shipping||0),N=parseInt(c.inventory,10),j=String(c.upc||"").replace(/\D/g,""),q=[];if(c.title.trim()||q.push(["ed-title","Add a title."]),(String(c.price??"").trim()===""||!(w>=0))&&q.push(["ed-price","Enter a sale price ($0 or more)."]),N>=0||q.push(["ed-inv","Inventory must be 0 or more."]),D>=0||q.push(["ed-ship","Shipping must be 0 or more."]),j&&(j.length<8||j.length>14)&&q.push(["ed-upc","UPC should be 8\u201314 digits."]),H("[aria-invalid]",s).forEach(g=>g.removeAttribute("aria-invalid")),q.length){q.forEach(([g])=>b("#"+g).setAttribute("aria-invalid","true")),y.textContent=q.map(g=>g[1]).join(" "),y.hidden=!1,b("#"+q[0][0]).focus();return}let m={id:S==="new"?void 0:S,title:c.title.trim(),slug:c.slug.trim(),brand:c.brand.trim(),upc:j,condition:c.condition,categoryId:c.categoryId||null,description:c.description.trim(),price:_(w),originalPrice:A>w?_(A):null,shipping:_(D),inventory:N,published:c.published,bestDeal:!1,noShipping:!!c.noShipping,images:c.images},f=b('button[type="submit"]',s);f.disabled=!0,f.textContent="Saving\u2026";try{let g=await d.saveProduct(m);return L(),$(S==="new"?"Product created":"Changes saved"),S=null,c=null,M="products",U(),g}catch(g){y.textContent=g.message,y.hidden=!1,f.disabled=!1,f.textContent="Save changes"}})}let oe="all";function K(){let s=h.orders.filter(e=>oe==="all"||e.status===oe);n.innerHTML=W(`
      <div class="adm-toolbar"><div class="select"><label class="sr-only" for="of-f">Status</label><select id="of-f" data-of>${[["all","All orders"],...de].map(([e,t])=>`<option value="${e}" ${oe===e?"selected":""}>${t}</option>`).join("")}</select>${u("down")}</div><span class="muted" style="font-size:14px">${s.length} order${s.length===1?"":"s"}</span></div>
      ...
      ${s.map(e=>`<tr><td class="mono"><b>#${e.number}</b></td><td>${F(e.createdAt,!0)}</td><td>${o(e.customer.name)}<small>${o(e.customer.phone)}</small></td><td>${e.items.reduce((t,a)=>t+a.qty,0)}</td><td>${e.fulfillment==="pickup"?"Pickup":"Ship"}</td><td class="tabnum"><b>${k(e.total)}</b></td>
        <td><label class="sr-only" for="os-${e.id}">Status</label><select id="os-${e.id}" class="adm-status" data-order-status="${e.id}">${de.map(([t,a])=>`<option value="${t}" ${e.status===t?"selected":""}>${a}</option>`).join("")}</select>${e.tracking&&e.tracking.number?`<small class="adm-track">${o(fe(e.tracking.carrier))} \xB7 <a class="mono" href="${o(be(e.tracking.carrier,e.tracking.number))}" target="_blank" rel="noopener">${o(e.tracking.number)}</a></small>`:e.status==="shipped"?`<small><button class="link" type="button" data-track="${e.id}">+ Add tracking</button></small>`:""}</td>
      ...
      <dialog class="modal adm-order-dlg" id="order-dlg" aria-labelledby="od-title"></dialog>`,"Orders")}let ie=s=>(s.paypalRefunds||[]).filter(e=>!["FAILED","CANCELLED"].includes(e.status)),J=s=>s.payment==="paypal"&&s.paidAt&&s.paypalCaptureId?_(s.total-ie(s).reduce((e,t)=>e+Number(t.amount),0)):0,ee=s=>J(s)>0;function $e(s){let e=h.orders.find(v=>v.id===s);if(!e)return;let t=document.getElementById("track-dlg");t||(t=document.createElement("dialog"),t.id="track-dlg",t.className="modal track-dlg",t.setAttribute("aria-labelledby","tk-title"),document.body.appendChild(t));let a=e.tracking||{};t.innerHTML=`<div class="modal-head"><h2 id="tk-title">Tracking package</h2></div>
      <form class="modal-body form" data-track-form="${e.id}" novalidate>
        ...
        <div class="field"><label for="tk-carrier">Carrier</label><select id="tk-carrier" name="carrier">${he.map(([v,T])=>`<option value="${v}" ${a.carrier===v?"selected":""}>${T}</option>`).join("")}</select></div>
        <div class="field"><label class="sr-only" for="tk-number">Tracking number</label><input id="tk-number" name="number" placeholder="Tracking number" autocomplete="off" autocapitalize="characters" spellcheck="false" maxlength="40" value="${o(a.number||"")}"></div>
      ...
      </form>`;let i=t.querySelector("form");t.onclick=v=>{(v.target===t||v.target.closest("[data-track-exit]"))&&t.close()},i.onsubmit=async v=>{v.preventDefault();let T=i.carrier.value,r=i.number.value.trim().replace(/\s+/g,"").toUpperCase(),P=i.querySelector("button[type=submit]");P.disabled=!0,P.textContent="Saving\u2026";try{if(await d.setTracking(e.id,r?{carrier:T,number:r,at:new Date().toISOString()}:null),t.close(),$(r?"Tracking saved":"Saved"),await O(),M==="orders"){let l=b("#order-dlg")&&b("#order-dlg").open;K(),l&&re(e.id)}}catch(l){P.disabled=!1,P.textContent="Save",$(l.message||"Could not save tracking")}},t.showModal()}function re(s){let e=h.orders.find(i=>i.id===s);if(!e)return;b("#order-dlg")||(M="orders",K());let t=b("#order-dlg"),a=e.address?`${o(e.address.line1)}${e.address.line2?", "+o(e.address.line2):""}<br>${o(e.address.city)}, ${o(e.address.state)} ${o(e.address.zip)}`:"Local pickup";t.innerHTML=`<div class="modal-head"><h2 id="od-title">Order #${e.number}</h2><button class="icon-btn" type="button" aria-label="Close" data-close-dlg>${u("close")}</button></div>
      <div class="modal-body">
        ...
        <div class="info-card"><h2>Customer</h2><p><b>${o(e.customer.name)}</b></p><p><a href="${Z(e.customer.phone)}">${o(e.customer.phone)}</a> \xB7 <a href="mailto:${o(e.customer.email)}">${o(e.customer.email)}</a></p><p class="muted">${a}</p>${e.notes?`<p><b>Notes:</b> ${o(e.notes)}</p>`:""}</div>
        ...
        <div class="field"><label for="od-status">Status</label><select id="od-status" data-order-status="${e.id}">${de.map(([i,v])=>`<option value="${i}" ${e.status===i?"selected":""}>${v}</option>`).join("")}</select></div>
        ${e.status==="shipped"||e.tracking&&e.tracking.number?`<div class="info-card od-track"><h2>Tracking package</h2>${e.tracking&&e.tracking.number?`<p>${o(fe(e.tracking.carrier))} \xB7 <a class="mono" href="${o(be(e.tracking.carrier,e.tracking.number))}" target="_blank" rel="noopener">${o(e.tracking.number)}</a></p>`:'<p class="muted">No tracking number yet.</p>'}<p><button class="btn btn-sm" type="button" data-track="${e.id}">${u("truck","icon-sm")} ${e.tracking&&e.tracking.number?"Edit tracking":"Add tracking"}</button></p></div>`:""}
        ${ie(e).length?`<div class="adm-note">${ie(e).map(i=>`<p><b>Refunded ${k(i.amount)}</b> through PayPal on ${F(i.at,!0)}${i.status!=="COMPLETED"?` (${o(String(i.status).toLowerCase())})`:""}.</p>`).join("")}${ie(e).length>1||ee(e)?`<p class="muted">Refunded so far: ${k(_(e.total-J(e)))} of ${k(e.total)}</p>`:""}</div>`:""}
        ...
        <div class="adm-confirm" data-od-confirm="refund" hidden>${ee(e)?`<p><b>Would you like to refund the customer?</b> ${k(J(e))} goes back to their card or PayPal automatically.${e.status!=="cancelled"?" The order is cancelled and items go back in stock.":""}</p><div><button class="btn btn-sm adm-danger" type="button" data-od-refund="${e.id}">Yes, refund ${k(J(e))}</button>${e.status!=="cancelled"?`<button class="btn btn-sm" type="button" data-cancel-order="${e.id}">No, just cancel</button>`:""}<button class="btn btn-sm" type="button" data-od-back>Go back</button></div>`:`<p><b>Cancel order #${e.number}?</b> Items go back in stock.${e.payment==="paypal"?"":" This order wasn\u2019t paid online, so if they already paid you (cash, card link), refund them the same way."}</p><div><button class="btn btn-sm adm-danger" type="button" data-cancel-order="${e.id}">Yes, cancel order</button><button class="btn btn-sm" type="button" data-od-back>Go back</button></div>`}</div>
        ${ee(e)?`<div class="adm-confirm" data-od-confirm="partial" hidden><p><b>Partial refund</b> Enter how much to send back to the customer. Up to ${k(J(e))}. The order stays open and nothing is restocked.</p>
          <div class="field"><label for="od-partial">Refund amount ($)</label><input id="od-partial" type="number" inputmode="decimal" min="0.01" max="${J(e)}" step="0.01" placeholder="0.00" data-od-amount></div>
      ...
      ${h.offers.map(s=>{let e=h.products.find(a=>a.id===s.productId),t=s.listPrice?Math.round(s.amount/s.listPrice*100):0;return`<tr>
        <td>${F(s.createdAt,!0)}</td>
        <td><div class="adm-prod">${s.productImage?`<img src="${o(Y(s.productImage.sm))}" alt="" width="48" height="48">`:""}<div>${e?`<button class="adm-title" type="button" data-edit="${e.id}">${o(s.productTitle)}</button>`:o(s.productTitle)}<small>${e?`${e.inventory} in stock`:"Deleted"}</small></div></div></td>
        <td class="tabnum">${k(s.listPrice)}</td>
        <td class="tabnum"><b>${k(s.amount)}</b><small>${t}% of list</small></td>
        <td>${o(s.name)}<small><a href="${Z(s.phone)}">${o(s.phone)}</a> \xB7 <a href="sms:${o(s.phone.replace(/[^\d+]/g,""))}">Text</a></small></td>
        <td><label class="sr-only" for="ofs-${s.id}">Status</label><select id="ofs-${s.id}" class="adm-status" data-offer-status="${s.id}">${Oe.map(([a,i])=>`<option value="${a}" ${s.status===a?"selected":""}>${i}</option>`).join("")}</select></td>
        <td>${e&&s.status!=="accepted"?`<button class="btn btn-sm" type="button" data-accept-price="${s.id}" title="Set the product price to this offer">Use as price</button>`:""} <button class="btn btn-sm adm-danger" type="button" data-offer-del="${s.id}">${u("trash","icon-sm")} Delete</button></td></tr>`}).join("")}
      </tbody></table></div>`:'<div class="empty"><h2>No offers yet</h2><p class="muted">When a customer taps \u201CMake Offer\u201D, their name, phone and amount appear here.</p></div>',"Offers")}function me(){let s=h.inquiries.filter(t=>t.type==="pickup"),e=t=>{let a=((t.message||"").match(/Pickup request:\s*(.+)/)||[])[1]||"Pickup request",i=((t.message||"").match(/When:\s*(.+)/)||[])[1]||"";return{item:a.trim(),when:i.trim()}};n.innerHTML=W(s.length?`<div class="adm-reqs">${s.map(t=>{let a=e(t),i=String(t.phone||"").replace(/[^\d+]/g,"");return`<details class="adm-req ${t.status==="new"?"unread":""}">
      <summary><span class="adm-req-title"><b>${o(a.item)}</b><small>${u("calendar","icon-sm")} ${o(a.when||F(t.createdAt,!0))}</small></span>${t.status==="new"?'<span class="status s-new">New</span>':`<span class="status s-${t.status}">${t.status==="read"?"Confirmed":"Done"}</span>`}${u("down")}</summary>
      <div class="adm-req-body">
        <dl class="kv"><div><dt>Name</dt><dd>${o(t.name)}</dd></div><div><dt>Phone</dt><dd><a href="${Z(t.phone)}">${o(t.phone)}</a></dd></div><div><dt>Pickup time</dt><dd>${o(a.when||"\u2014")}</dd></div><div><dt>Requested</dt><dd>${F(t.createdAt,!0)}</dd></div></dl>
        <div class="adm-req-actions">
          <a class="btn btn-primary" href="sms:${o(i)}">${u("msg","icon-sm")} Text</a>
          <a class="btn" href="${Z(t.phone)}">${u("phone","icon-sm")} Call</a>
      ...
      <div class="adm-card-body"><p class="adm-msg-meta"><a href="${Z(e.phone)}">${o(e.phone)}</a> \xB7 <a href="sms:${o(String(e.phone).replace(/[^\d+]/g,""))}">Text</a>${e.email?` \xB7 <a href="mailto:${o(e.email)}">${o(e.email)}</a>`:""}${e.company?" \xB7 "+o(e.company):""}</p><p class="adm-msg-text">${o(e.message)}</p>
      <div class="adm-msg-actions"><label class="sr-only" for="ms-${e.id}">Status</label><select id="ms-${e.id}" class="adm-status" data-msg-status="${e.id}">${et.map(([t,a])=>`<option value="${t}" ${e.status===t?"selected":""}>${a}</option>`).join("")}</select><button class="btn btn-sm" type="button" data-msg-del="${e.id}">${u("trash","icon-sm")} Delete</button></div></div>
    </article>`).join("")}</div>`:'<div class="empty"><h2>No messages</h2><p class="muted">Contact form and wholesale requests appear here.</p></div>',"Messages")}function te(){let s=p.categories(),e={};h.products.forEach(t=>{e[t.categoryId]=(e[t.categoryId]||0)+1}),n.innerHTML=W(`<div class="adm-card"><div class="adm-card-body">
      <ul class="adm-cats">${s.map((t,a)=>`<li data-cat="${t.id}">
        ...
        <button class="icon-btn" type="button" aria-label="Move up" data-cat-move="-1" ${a===0?"disabled":""}>${u("left","icon-sm")}</button>
        <button class="icon-btn" type="button" aria-label="Move down" data-cat-move="1" ${a===s.length-1?"disabled":""}>${u("right","icon-sm")}</button>
    ...
    </div></div>`,"Categories")}function Ne(){let s=p.settings(),e=(t,a,i="")=>`<div class="field"><label for="st-${t}">${a}</label><input id="st-${t}" name="${t}" value="${o(s[t]||"")}" ${i}></div>`;n.innerHTML=W(`<div class="adm-cols two">
    ...
    </div>`,"Settings")}let Se=async s=>{let e=s.target,t=e.closest("[data-tab]");if(t){M=t.dataset.tab,S=null,c=null,U();return}if(e.closest("[data-logout]")){await d.logout(),Q();return}if(e.closest("[data-new]")){M="products",S="new",c=null,U();return}let a=e.closest("[data-edit]");if(a){M="products",S=a.dataset.edit,c=null,U();return}if(e.closest("[data-cancel-edit]")){S=null,c=null,M="products",U();return}let i=e.closest("[data-inv]");if(i){let m=await d.adjustInventory(i.dataset.id,Number(i.dataset.inv));if(L(),m){let f=i.closest("tr");b("b",i.parentElement).textContent=m.inventory,f.classList.toggle("dim",m.inventory<=0||!m.published)}return}let v=e.closest("[data-open-order]");if(v){re(v.dataset.openOrder);return}if(e.closest("[data-close-dlg]")||e.matches("dialog.modal")){let m=e.closest("dialog");m&&m.close();return}let T=e.closest("[data-od-ask]");if(T){H("[data-od-confirm]").forEach(g=>{g.hidden=g.dataset.odConfirm!==T.dataset.odAsk});let m=b(`[data-od-confirm="${T.dataset.odAsk}"]`);m.scrollIntoView({block:"nearest"});let f=b("[data-od-amount]",m);f&&f.focus();return}if(e.closest("[data-od-back]")){H("[data-od-confirm]").forEach(m=>{m.hidden=!0});return}let r=e.closest("[data-od-refund], [data-od-partial]");if(r){let m=r.hasAttribute("data-od-partial"),f=r.dataset.odRefund||r.dataset.odPartial,g=h.orders.find(z=>z.id===f),B=g?J(g):0,R=B;if(m){let z=b("[data-od-amount]");if(R=_(Number(z.value)),!(R>0)||R>B){z.setAttribute("aria-invalid","true"),z.focus(),$(`Enter an amount from $0.01 to ${k(B)}`);return}z.setAttribute("aria-invalid","false")}let ae=r.closest("[data-od-confirm]"),He=r.textContent;H("button",ae).forEach(z=>{z.disabled=!0}),r.textContent="Refunding\u2026";try{await d.refundOrder(f,m?R.toFixed(2):null),!m&&g&&g.status!=="cancelled"&&await d.updateOrder(f,{status:"cancelled",restock:!0}),L(),$(`Refunded ${k(R)} to the customer`),await O(),K(),re(f)}catch(z){H("button",ae).forEach(Be=>{Be.disabled=!1}),r.textContent=He,$(z.message||"The refund did not go through.")}return}let P=e.closest("[data-track]");if(P){$e(P.dataset.track);return}let l=e.closest("[data-cancel-order]");if(l){await d.updateOrder(l.dataset.cancelOrder,{status:"cancelled",restock:!0}),L(),$("Order cancelled and items restocked");let m=e.closest("dialog");m&&m.close(),await O(),K();return}let y=e.closest("[data-accept-price]");if(y){let m=h.offers.find(g=>g.id===y.dataset.acceptPrice),f=h.products.find(g=>g.id===m.productId);f&&(await d.saveProduct({...f,originalPrice:f.originalPrice||f.price,price:m.amount}),await d.updateOffer(m.id,{status:"accepted"}),L(),$(`Price set to ${k(m.amount)} and offer accepted`),await O(),pe());return}let w=e.closest("[data-bulk]");if(w){let m=[...E],f=w.dataset.bulk;if(f==="delete"&&!w.dataset.sure){w.dataset.sure="1",w.textContent=`Confirm delete ${m.length}`,w.classList.add("adm-danger");return}for(let g of m){let B=h.products.find(R=>R.id===g);if(B){if(f==="discount"){let R=Math.min(95,Math.max(1,parseInt(b("[data-bulk-pct]").value,10)||0)),ae=B.originalPrice||B.price;await d.saveProduct({...B,originalPrice:ae,price:_(ae*(1-R/100))})}f==="publish"&&await d.patchProduct(g,{published:!0}),f==="unpublish"&&await d.patchProduct(g,{published:!1}),f==="delete"&&await d.deleteProduct(g)}}E.clear(),L(),$("Updated "+m.length+" product"+(m.length>1?"s":"")),await O(),X();return}let A=e.closest("[data-cat-move]");if(A){let m=A.closest("[data-cat]"),f=p.categories().map(R=>R.id),g=f.indexOf(m.dataset.cat),B=g+Number(A.dataset.catMove);[f[g],f[B]]=[f[B],f[g]],await d.reorderCategories(f),L(),te();return}let D=e.closest("[data-cat-save]");if(D){let m=D.closest("[data-cat]"),f=b("[data-cat-name]",m).value.trim();if(!f)return;await d.saveCategory({id:m.dataset.cat,name:f}),L(),$("Category saved"),te();return}let N=e.closest("[data-cat-del]");if(N){if(!N.dataset.sure){N.dataset.sure="1",N.textContent="Confirm";return}await d.deleteCategory(N.closest("[data-cat]").dataset.cat),L(),$("Category deleted"),await O(),te();return}let j=e.closest("[data-offer-del]");if(j){if(!j.dataset.sure){j.dataset.sure="1",j.textContent="Tap again to delete";return}await d.deleteOffer(j.dataset.offerDel),await O(),pe(),$("Offer deleted");return}let q=e.closest("[data-msg-del]");if(q){await d.deleteInquiry(q.dataset.msgDel),await O(),(q.dataset.back==="requests"?me:ke)(),$("Deleted");return}if(e.closest("[data-export]")){let m=await d.exportData(),f=new Blob([m],{type:"application/json"}),g=document.createElement("a");g.href=URL.createObjectURL(f),g.download=`mlgroup-backup-${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(g),g.click(),g.remove(),$("Backup downloaded");return}if(e.closest("[data-reset]")){b("[data-reset-confirm]").hidden=!1;return}if(e.closest("[data-reset-no]")){b("[data-reset-confirm]").hidden=!0;return}e.closest("[data-reset-yes]")&&(await d.reset(),L(),$("Demo data restored"),U())},xe=async s=>{let e=s.target;if(e.matches("[data-pf-f]")&&(G.f=e.value,X()),e.matches("[data-sel]")){e.checked?E.add(e.dataset.sel):E.delete(e.dataset.sel);let t=b("[data-bulk]");t.hidden=!E.size,b("[data-bulk-n]").textContent=`${E.size} selected`}if(e.matches("[data-sel-all]")){H("[data-sel]").forEach(a=>{a.checked=e.checked,e.checked?E.add(a.dataset.sel):E.delete(a.dataset.sel)});let t=b("[data-bulk]");t.hidden=!E.size,b("[data-bulk-n]").textContent=`${E.size} selected`}if(e.matches("[data-pub]")&&(await d.patchProduct(e.dataset.pub,{published:e.checked}),L(),e.nextElementSibling.textContent=e.checked?"Published":"Hidden",$(e.checked?"Published":"Hidden from store")),e.matches("[data-of]")&&(oe=e.value,K()),e.matches("[data-order-status]")){let t=e.dataset.orderStatus;await d.updateOrder(t,{status:e.value}),$("Order status updated"),await O();let a=b("#order-dlg");a&&a.open&&(K(),re(t)),e.value==="shipped"&&$e(t)}if(e.matches("[data-offer-status]")&&(await d.updateOffer(e.dataset.offerStatus,{status:e.value}),$("Offer updated"),await O()),e.matches("[data-msg-status]")&&(await d.updateInquiry(e.dataset.msgStatus,{status:e.value}),await O(),$("Updated"),M==="requests"&&me()),e.matches("[data-import]")&&e.files[0])try{await d.importData(await e.files[0].text()),L(),$("Backup imported"),U()}catch(t){$(t.message,"err")}},Ce=s=>{if(s.target.matches("[data-pf-q]")){G.q=s.target.value;let e=s.target.selectionStart;X();let t=b("[data-pf-q]");t.focus(),t.setSelectionRange(e,e)}},Me=async s=>{let e=s.target;if(e.matches("[data-login]")){s.preventDefault();let t=b('button[type="submit"]',e);t.disabled=!0;try{await d.login(e.elements.pw.value),U()}catch(a){Q(a.message||"Sign in failed.")}}if(e.matches("[data-settings]")){s.preventDefault();let t=Object.fromEntries(new FormData(e));t.state=(t.state||"").toUpperCase(),await d.saveSettings(t),L(),$("Store details saved")}if(e.matches("[data-password]")){s.preventDefault();let t=Object.fromEntries(new FormData(e)),a=b("[data-err]",e);if(t.pw1.length<6){a.textContent="Use at least 6 characters.",a.hidden=!1;return}if(t.pw1!==t.pw2){a.textContent="The passwords don\u2019t match.",a.hidden=!1;return}try{await d.changePassword(t.pw1),e.reset(),a.hidden=!0,$("Password changed")}catch(i){a.textContent=i.message,a.hidden=!1}}if(e.matches("[data-cat-add]")){s.preventDefault();let t=e.elements.name.value.trim();if(!t)return;await d.saveCategory({name:t}),L(),$("Category added"),te()}},qe=()=>H("table.tbl",n).forEach(s=>{let e=H("thead th",s).map(t=>t.textContent.trim());H("tbody tr",s).forEach(t=>H("td",t).forEach((a,i)=>{a.hasAttribute("data-label")||a.setAttribute("data-label",e[i]||"")}))}),Pe=new MutationObserver(qe);return Pe.observe(n,{childList:!0,subtree:!0}),n.addEventListener("click",Se),n.addEventListener("change",xe),n.addEventListener("input",Ce),n.addEventListener("submit",Me),U(),()=>{Pe.disconnect(),n.removeEventListener("click",Se),n.removeEventListener("change",xe),n.removeEventListener("input",Ce),n.removeEventListener("submit",Me)}}window.MLAdmin={mount:tt};})();
