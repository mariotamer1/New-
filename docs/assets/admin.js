...
\u2022 10" tall`]],Oe=Date.UTC(2026,8,20),ye=Ie.map((n,m)=>{let[x,S,d,M,y,h,q,L,c,se,Z]=n,E=(ge[x]||[]).map((I,V)=>({sm:`img/p/${x}-${I}-sm.webp`,lg:`img/p/${x}-${I}-lg.webp`,alt:V===0?S:`${S} \u2013 view ${V+1}`}));return{id:"p-"+String(m+1).padStart(3,"0"),slug:x,title:S,brand:d,categoryId:De(M),condition:y,price:h,originalPrice:q,shipping:L,inventory:c,bestDeal:se,published:!0,noShipping:x==="live-bonsai-tree",upc:Ae(m+11),description:Z,images:E,createdAt:new Date(Oe-m*36e5*7).toISOString()}});var nt={products:[],categories:[],settings:{...ve}};function k(n,m=""){let x=g(".toasts");x||(x=document.createElement("div"),x.className="toasts",x.setAttribute("role","status"),x.setAttribute("aria-live","polite"),document.body.appendChild(x));let S=document.createElement("div");S.className="toast "+m,S.innerHTML=`${p(m==="err"?"close":"check","icon-sm")}<span>${i(n)}</span>`,x.appendChild(S),setTimeout(()=>{S.style.opacity="0",S.style.transition="opacity .3s",setTimeout(()=>S.remove(),320)},2600)}var _e=[["overview","Overview","chart"],["products","Products","tag"],["orders","Orders","box"],["offers","Offers","cash"],["requests","Requests","calendar"],["messages","Messages","msg"],["categories","Categories","layers"],["settings","Settings","gear"]],Re=["New","Open Box","Like New","Refurbished","Used \u2013 Good","Used \u2013 Fair","Scratch & Dent"],ae=[["new","New"],["processing","Processing"],["ready","Ready for pickup"],["shipped","Shipped"],["completed","Completed"],["cancelled","Cancelled"]],we=[["new","New"],["contacted","Contacted"],["accepted","Accepted"],["declined","Declined"]],Ue=[["new","New"],["read","Read"],["archived","Archived"]];function je(n,{store:m,navigate:x,query:S}){let d=m.admin,M=S.get("tab")||"overview",y=S.get("edit")||null,h={products:[],orders:[],offers:[],inquiries:[]},q={q:"",f:"all"},L=new Set,c=null,se=()=>{let s=new URLSearchParams;M!=="overview"&&s.set("tab",M),y&&s.set("edit",y);let e="/admin"+(s.toString()?"?"+s:"");D.routing==="path"?history.replaceState({},"",j(e)):D.routing==="hash"&&history.replaceState(null,"","#"+e)},Z=s=>(m.category(s)||{}).name||"\u2014",E=()=>{m.refresh(),window.dispatchEvent(new CustomEvent("cart:change"))};async function I(){let[s,e,t,a]=await Promise.all([d.products(),d.orders(),d.offers(),d.inquiries()]);h={products:s,orders:e,offers:t,inquiries:a}}async function V(s){n.innerHTML=`<div class="adm-login"><form class="adm-login-card" novalidate data-login>
    ...
    </form></div>`,setTimeout(()=>{let e=g("#adm-pw");e&&e.focus()},30)}function B(s,e,t=""){let a=(w,O="new")=>h[w].filter(r=>r.status===O).length,o={orders:a("orders"),offers:a("offers"),requests:h.inquiries.filter(w=>w.type==="pickup"&&w.status==="new").length,messages:h.inquiries.filter(w=>w.type!=="pickup"&&w.status==="new").length};return`<div class="admin">
    ...
    </div>`}async function N(){if(!await d.session()){V();return}if(await I(),se(),M==="products"&&y)return $e();({overview:oe,products:K,orders:X,offers:ce,requests:ne,messages:de,categories:J,settings:xe}[M]||oe)()}function oe(){let s=h.products,e=s.filter(r=>r.published&&r.inventory>0),t=s.filter(r=>r.inventory<=0),a=s.filter(r=>r.inventory>0&&r.inventory<=2).slice(0,6),o=Date.now()-30*864e5,w=h.orders.filter(r=>r.status!=="cancelled"&&new Date(r.createdAt)>o).reduce((r,C)=>r+C.total,0),O=[["Live listings",e.length,"Published and in stock"],["Units in stock",s.reduce((r,C)=>r+Math.max(0,C.inventory),0),`${t.length} sold out (hidden)`],["New orders",h.orders.filter(r=>r.status==="new").length,`${h.orders.length} total`],["New offers",h.offers.filter(r=>r.status==="new").length,`${h.offers.length} total`],["Sales \xB7 30 days",P(w),"Excludes cancelled"],["Messages",h.inquiries.filter(r=>r.status==="new").length,"Contact + wholesale"]];n.innerHTML=B(`
