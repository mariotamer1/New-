...
\u2022 6\u20138 years old
\u2022 10" tall`]],Ie=Date.UTC(2026,8,20),ye=Oe.map((n,m)=>{let[x,S,d,M,y,h,q,L,c,se,Z]=n,E=(ge[x]||[]).map((O,V)=>({sm:`img/p/${x}-${O}-sm.webp`,lg:`img/p/${x}-${O}-lg.webp`,alt:V===0?S:`${S} \u2013 view ${V+1}`}));return{id:"p-"+String(m+1).padStart(3,"0"),slug:x,title:S,brand:d,categoryId:De(M),condition:y,price:h,originalPrice:q,shipping:L,inventory:c,bestDeal:se,published:!0,noShipping:x==="live-bonsai-tree",upc:Ae(m+11),description:Z,images:E,createdAt:new Date(Ie-m*36e5*7).toISOString()}});var nt={products:[],categories:[],settings:{...ve}};function k(n,m=""){let x=g(".toasts");x||(x=document.createElement("div"),x.className="toasts",x.setAttribute("role","status"),x.setAttribute("aria-live","polite"),document.body.appendChild(x));let S=document.createElement("div");S.className="toast "+m,S.innerHTML=`${p(m==="err"?"close":"check","icon-sm")}<span>${i(n)}</span>`,x.appendChild(S),setTimeout(()=>{S.style.opacity="0",S.style.transition="opacity .3s",setTimeout(()=>S.remove(),320)},2600)}var _e=[["overview","Overview","chart"],["products","Products","tag"],["orders","Orders","box"],["offers","Offers","cash"],["requests","Requests","calendar"],["messages","Messages","msg"],["categories","Categories","layers"],["settings","Settings","gear"]],Re=["New","Open Box","Like New","Refurbished","Used \u2013 Good","Used \u2013 Fair","Scratch & Dent"],ae=[["new","New"],["processing","Processing"],["ready","Ready for pickup"],["shipped","Shipped"],["completed","Completed"],["cancelled","Cancelled"]],we=[["new","New"],["contacted","Contacted"],["accepted","Accepted"],["declined","Declined"]],Ue=[["new","New"],["read","Read"],["archived","Archived"]];function je(n,{store:m,navigate:x,query:S}){let d=m.admin,M=S.get("tab")||"overview",y=S.get("edit")||null,h={products:[],orders:[],offers:[],inquiries:[]},q={q:"",f:"all"},L=new Set,c=null,se=()=>{let s=new URLSearchParams;M!=="overview"&&s.set("tab",M),y&&s.set("edit",y);let e="/admin"+(s.toString()?"?"+s:"");D.routing==="path"?history.replaceState({},"",j(e)):D.routing==="hash"&&history.replaceState(null,"","#"+e)},Z=s=>(m.category(s)||{}).name||"\u2014",E=()=>{m.refresh(),window.dispatchEvent(new CustomEvent("cart:change"))};async function O(){let[s,e,t,a]=await Promise.all([d.products(),d.orders(),d.offers(),d.inquiries()]);h={products:s,orders:e,offers:t,inquiries:a}}async function V(s){n.innerHTML=`<div class="adm-login"><form class="adm-login-card" novalidate data-login>
    ...
    </form></div>`,setTimeout(()=>{let e=g("#adm-pw");e&&e.focus()},30)}function B(s,e,t=""){let a=(w,I="new")=>h[w].filter(r=>r.status===I).length,o={orders:a("orders"),offers:a("offers"),requests:h.inquiries.filter(w=>w.type==="pickup"&&w.status==="new").length,messages:h.inquiries.filter(w=>w.type!=="pickup"&&w.status==="new").length};return`<div class="admin">
      <aside class="adm-side on-ink">
        <a class="logo" href="${j("/")}" aria-label="View store">${te()}</a>
        <span class="adm-tag">Admin</span>
        <nav aria-label="Admin"><ul>${_e.map(([w,I,r])=>`<li><button type="button" data-tab="${w}" ${M===w?'aria-current="page"':""}>${p(r)}<span>${I}</span>${o[w]?`<b class="adm-badge">${o[w]}</b>`:""}</button></li>`).join("")}</ul></nav>
    ...
    </div>`}async function N(){if(!await d.session()){V();return}if(await O(),se(),M==="products"&&y)return $e();({overview:oe,products:K,orders:X,offers:ce,requests:ne,messages:de,categories:J,settings:xe}[M]||oe)()}function oe(){let s=h.products,e=s.filter(r=>r.published&&r.inventory>0),t=s.filter(r=>r.inventory<=0),a=s.filter(r=>r.inventory>0&&r.inventory<=2).slice(0,6),o=Date.now()-30*864e5,w=h.orders.filter(r=>r.status!=="cancelled"&&new Date(r.createdAt)>o).reduce((r,C)=>r+C.total,0),I=[["Live listings",e.length,"Published and in stock"],["Units in stock",s.reduce((r,C)=>r+Math.max(0,C.inventory),0),`${t.length} sold out (hidden)`],["New orders",h.orders.filter(r=>r.status==="new").length,`${h.orders.length} total`],["New offers",h.offers.filter(r=>r.status==="new").length,`${h.offers.length} total`],["Sales \xB7 30 days",P(w),"Excludes cancelled"],["Messages",h.inquiries.filter(r=>r.status==="new").length,"Contact + wholesale"]];n.innerHTML=B(`
      <div class="adm-tiles">${I.map(([r,C,l])=>`<div class="adm-tile"><span class="eyebrow">${r}</span><b class="tabnum">${C}</b><span class="muted">${l}</span></div>`).join("")}</div>
      <div class="adm-cols">
      ...
      <div class="adm-toolbar">
        ...
        <div class="select"><label class="sr-only" for="pf-f">Show</label><select id="pf-f" data-pf-f>${[["all","All products"],["live","Live"],["hidden","Unpublished"],["out","Sold out"],["deal","Best deals"]].map(([a,o])=>`<option value="${a}" ${t===a?"selected":""}>${o}</option>`).join("")}</select>${p("down")}</div>
      ...
      <div class="table-wrap"><table class="tbl adm-table"><thead><tr>
        ...
        ${e.map(a=>`<tr data-row="${a.id}" class="${a.inventory<=0||!a.published?"dim":""}">
          ...
          <td><label class="switch"><input type="checkbox" data-pub="${a.id}" ${a.published?"checked":""}><span>${a.published?"Published":"Hidden"}</span></label>
              <label class="switch"><input type="checkbox" data-deal="${a.id}" ${a.bestDeal?"checked":""}><span>Best deal</span></label></td>
      ...
      </figcaption></figure>`).join(""):'<p class="muted" style="font-size:14px">No photos yet.</p>'}function ke(){let s=g("[data-editor]"),e=l=>s.elements[l],t=()=>{g("[data-photos]").innerHTML=le(c)},a=()=>{["title","description","brand","upc","slug","condition","categoryId"].forEach(l=>{c[l]=e(l).value}),c.price=e("price").value,c.originalPrice=e("originalPrice").value,c.shipping=e("shipping").value,c.inventory=e("inventory").value,c.published=e("published").checked,c.bestDeal=e("bestDeal").checked,c.noShipping=e("noShipping").checked};s.addEventListener("input",l=>{let b=l.target.name,v=parseFloat(e("originalPrice").value),A=parseFloat(e("pct").value),u=parseFloat(e("price").value);b==="pct"&&v>0&&A>=0&&(e("price").value=U(v*(1-A/100)).toFixed(2)),b==="price"&&v>0&&u>=0&&(e("pct").value=u<v?Math.round((1-u/v)*100):""),b==="originalPrice"&&v>0&&A>0&&(e("price").value=U(v*(1-A/100)).toFixed(2)),(b==="title"||b==="slug")&&(g("[data-slug-preview]").textContent=Y(e("slug").value||e("title").value)||"product-name"),a()}),s.addEventListener("change",a),s.addEventListener("click",async l=>{let b=l.target.closest("[data-ed-inv]");b&&(e("inventory").value=Math.max(0,(parseInt(e("inventory").value,10)||0)+Number(b.dataset.edInv)),a());let v=l.target.closest("[data-ph-move]");if(v){let u=Number(v.closest("[data-ph]").dataset.ph),f=u+Number(v.dataset.phMove);[c.images[u],c.images[f]]=[c.images[f],c.images[u]],t()}let A=l.target.closest("[data-ph-del]");if(A){let u=Number(A.closest("[data-ph]").dataset.ph),[f]=c.images.splice(u,1);d.removeImage&&d.removeImage(f).catch(()=>{}),t()}l.target.closest("[data-delete-product]")&&(g("[data-confirm]").hidden=!1),l.target.closest("[data-confirm-cancel]")&&(g("[data-confirm]").hidden=!0),l.target.closest("[data-confirm-delete]")&&(await d.deleteProduct(y),E(),k("Product deleted"),y=null,c=null,N())});let o=null,w=g("[data-photos]");w.addEventListener("dragstart",l=>{let b=l.target.closest("[data-ph]");b&&(o=Number(b.dataset.ph),l.dataTransfer.effectAllowed="move",b.classList.add("dragging"))}),w.addEventListener("dragover",l=>{o!==null&&l.preventDefault()}),w.addEventListener("drop",l=>{let b=l.target.closest("[data-ph]");if(o===null||!b)return;l.preventDefault();let v=Number(b.dataset.ph),[A]=c.images.splice(o,1);c.images.splice(v,0,A),o=null,t()}),w.addEventListener("dragend",()=>{o=null,_(".dragging").forEach(l=>l.classList.remove("dragging"))});let I=g("[data-upload-status]"),r=async l=>{if(l=Array.from(l).filter(b=>b.type.startsWith("image/")),!!l.length){for(let b=0;b<l.length;b++){I.textContent=`Uploading ${b+1} of ${l.length}\u2026`;try{let v=await d.uploadImage(l[b]);c.images.push({...v,alt:""}),t()}catch(v){k(`Couldn\u2019t upload ${l[b].name}: ${v.message}`,"err")}}I.textContent=`${l.length} photo${l.length>1?"s":""} added \u2014 remember to save.`}};g("[data-files]").addEventListener("change",l=>{r(l.target.files),l.target.value=""});let C=g("[data-drop]");C.addEventListener("dragover",l=>{l.dataTransfer.types.includes("Files")&&(l.preventDefault(),C.classList.add("over"))}),C.addEventListener("dragleave",()=>C.classList.remove("over")),C.addEventListener("drop",l=>{l.dataTransfer.files.length&&(l.preventDefault(),C.classList.remove("over"),r(l.dataTransfer.files))}),s.addEventListener("submit",async l=>{l.preventDefault(),a();let b=g("[data-err]"),v=parseFloat(c.price),A=parseFloat(c.originalPrice),u=parseFloat(c.shipping||0),f=parseInt(c.inventory,10),$=String(c.upc||"").replace(/\D/g,""),T=[];if(c.title.trim()||T.push(["ed-title","Add a title."]),v>0||T.push(["ed-price","Enter a sale price greater than $0."]),f>=0||T.push(["ed-inv","Inventory must be 0 or more."]),u>=0||T.push(["ed-ship","Shipping must be 0 or more."]),$&&($.length<8||$.length>14)&&T.push(["ed-upc","UPC should be 8\u201314 digits."]),_("[aria-invalid]",s).forEach(H=>H.removeAttribute("aria-invalid")),T.length){T.forEach(([H])=>g("#"+H).setAttribute("aria-invalid","true")),b.textContent=T.map(H=>H[1]).join(" "),b.hidden=!1,g("#"+T[0][0]).focus();return}let z={id:y==="new"?void 0:y,title:c.title.trim(),slug:c.slug.trim(),brand:c.brand.trim(),upc:$,condition:c.condition,categoryId:c.categoryId||null,description:c.description.trim(),price:U(v),originalPrice:A>v?U(A):null,shipping:U(u),inventory:f,published:c.published,bestDeal:c.bestDeal,noShipping:!!c.noShipping,images:c.images},F=g('button[type="submit"]',s);F.disabled=!0,F.textContent="Saving\u2026";try{let H=await d.saveProduct(z);return E(),k(y==="new"?"Product created":"Changes saved"),y=null,c=null,M="products",N(),H}catch(H){b.textContent=H.message,b.hidden=!1,F.disabled=!1,F.textContent="Save changes"}})}let Q="all";function X(){let s=h.orders.filter(e=>Q==="all"||e.status===Q);n.innerHTML=B(`
      ...
      </div>`,t.showModal()}function ce(){n.innerHTML=B(h.offers.length?`<div class="table-wrap"><table class="tbl"><thead><tr><th>Date</th><th>Product</th><th>List price</th><th>Offer</th><th>Customer</th><th>Status</th><th></th></tr></thead><tbody>
    ...
    </article>`).join("")}</div>`:'<div class="empty"><h2>No messages</h2><p class="muted">Contact form and wholesale requests appear here.</p></div>',"Messages")}function J(){let s=m.categories(),e={};h.products.forEach(t=>{e[t.categoryId]=(e[t.categoryId]||0)+1}),n.innerHTML=B(`<div class="adm-card"><div class="adm-card-body">
    ...
    </div></div>`,"Categories")}function xe(){let s=m.settings(),e=(t,a,o="")=>`<div class="field"><label for="st-${t}">${a}</label><input id="st-${t}" name="${t}" value="${i(s[t]||"")}" ${o}></div>`;n.innerHTML=B(`<div class="adm-cols two">
    ...
    </div>`,"Settings")}let ue=async s=>{let e=s.target,t=e.closest("[data-tab]");if(t){M=t.dataset.tab,y=null,c=null,N();return}if(e.closest("[data-logout]")){await d.logout(),V();return}if(e.closest("[data-new]")){M="products",y="new",c=null,N();return}let a=e.closest("[data-edit]");if(a){M="products",y=a.dataset.edit,c=null,N();return}if(e.closest("[data-cancel-edit]")){y=null,c=null,M="products",N();return}let o=e.closest("[data-inv]");if(o){let u=await d.adjustInventory(o.dataset.id,Number(o.dataset.inv));if(E(),u){let f=o.closest("tr");g("b",o.parentElement).textContent=u.inventory,f.classList.toggle("dim",u.inventory<=0||!u.published)}return}let w=e.closest("[data-open-order]");if(w){Se(w.dataset.openOrder);return}if(e.closest("[data-close-dlg]")||e.matches("dialog.modal")){let u=e.closest("dialog");u&&u.close();return}let I=e.closest("[data-cancel-order]");if(I){await d.updateOrder(I.dataset.cancelOrder,{status:"cancelled",restock:!0}),E(),k("Order cancelled and items restocked");let u=e.closest("dialog");u&&u.close(),await O(),X();return}let r=e.closest("[data-accept-price]");if(r){let u=h.offers.find($=>$.id===r.dataset.acceptPrice),f=h.products.find($=>$.id===u.productId);f&&(await d.saveProduct({...f,originalPrice:f.originalPrice||f.price,price:u.amount}),await d.updateOffer(u.id,{status:"accepted"}),E(),k(`Price set to ${P(u.amount)} and offer accepted`),await O(),ce());return}let C=e.closest("[data-bulk]");if(C){let u=[...L],f=C.dataset.bulk;if(f==="delete"&&!C.dataset.sure){C.dataset.sure="1",C.textContent=`Confirm delete ${u.length}`,C.classList.add("adm-danger");return}for(let $ of u){let T=h.products.find(z=>z.id===$);if(T){if(f==="discount"){let z=Math.min(95,Math.max(1,parseInt(g("[data-bulk-pct]").value,10)||0)),F=T.originalPrice||T.price;await d.saveProduct({...T,originalPrice:F,price:U(F*(1-z/100))})}f==="publish"&&await d.patchProduct($,{published:!0}),f==="unpublish"&&await d.patchProduct($,{published:!1}),f==="delete"&&await d.deleteProduct($)}}L.clear(),E(),k("Updated "+u.length+" product"+(u.length>1?"s":"")),await O(),K();return}let l=e.closest("[data-cat-move]");if(l){let u=l.closest("[data-cat]"),f=m.categories().map(z=>z.id),$=f.indexOf(u.dataset.cat),T=$+Number(l.dataset.catMove);[f[$],f[T]]=[f[T],f[$]],await d.reorderCategories(f),E(),J();return}let b=e.closest("[data-cat-save]");if(b){let u=b.closest("[data-cat]"),f=g("[data-cat-name]",u).value.trim();if(!f)return;await d.saveCategory({id:u.dataset.cat,name:f}),E(),k("Category saved"),J();return}let v=e.closest("[data-cat-del]");if(v){if(!v.dataset.sure){v.dataset.sure="1",v.textContent="Confirm";return}await d.deleteCategory(v.closest("[data-cat]").dataset.cat),E(),k("Category deleted"),await O(),J();return}let A=e.closest("[data-msg-del]");if(A){await d.deleteInquiry(A.dataset.msgDel),await O(),(A.dataset.back==="requests"?ne:de)(),k("Deleted");return}if(e.closest("[data-export]")){let u=await d.exportData(),f=new Blob([u],{type:"application/json"}),$=document.createElement("a");$.href=URL.createObjectURL(f),$.download=`mlgroup-backup-${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild($),$.click(),$.remove(),k("Backup downloaded");return}if(e.closest("[data-reset]")){g("[data-reset-confirm]").hidden=!1;return}if(e.closest("[data-reset-no]")){g("[data-reset-confirm]").hidden=!0;return}e.closest("[data-reset-yes]")&&(await d.reset(),E(),k("Demo data restored"),N())},pe=async s=>{let e=s.target;if(e.matches("[data-pf-f]")&&(q.f=e.value,K()),e.matches("[data-sel]")){e.checked?L.add(e.dataset.sel):L.delete(e.dataset.sel);let t=g("[data-bulk]");t.hidden=!L.size,g("[data-bulk-n]").textContent=`${L.size} selected`}if(e.matches("[data-sel-all]")){_("[data-sel]").forEach(a=>{a.checked=e.checked,e.checked?L.add(a.dataset.sel):L.delete(a.dataset.sel)});let t=g("[data-bulk]");t.hidden=!L.size,g("[data-bulk-n]").textContent=`${L.size} selected`}if(e.matches("[data-pub]")&&(await d.patchProduct(e.dataset.pub,{published:e.checked}),E(),e.nextElementSibling.textContent=e.checked?"Published":"Hidden",k(e.checked?"Published":"Hidden from store")),e.matches("[data-deal]")&&(await d.patchProduct(e.dataset.deal,{bestDeal:e.checked}),E(),k(e.checked?"Added to Best Deals":"Removed from Best Deals")),e.matches("[data-of]")&&(Q=e.value,X()),e.matches("[data-order-status]")&&(await d.updateOrder(e.dataset.orderStatus,{status:e.value}),k("Order status updated"),await O()),e.matches("[data-offer-status]")&&(await d.updateOffer(e.dataset.offerStatus,{status:e.value}),k("Offer updated"),await O()),e.matches("[data-msg-status]")&&(await d.updateInquiry(e.dataset.msgStatus,{status:e.value}),await O(),k("Updated"),M==="requests"&&ne()),e.matches("[data-import]")&&e.files[0])try{await d.importData(await e.files[0].text()),E(),k("Backup imported"),N()}catch(t){k(t.message,"err")}},me=s=>{if(s.target.matches("[data-pf-q]")){q.q=s.target.value;let e=s.target.selectionStart;K();let t=g("[data-pf-q]");t.focus(),t.setSelectionRange(e,e)}},he=async s=>{let e=s.target;if(e.matches("[data-login]")){s.preventDefault();let t=g('button[type="submit"]',e);t.disabled=!0;try{await d.login(e.elements.pw.value),N()}catch(a){V(a.message||"Sign in failed.")}}if(e.matches("[data-settings]")){s.preventDefault();let t=Object.fromEntries(new FormData(e));t.state=(t.state||"").toUpperCase(),await d.saveSettings(t),E(),k("Store details saved")}if(e.matches("[data-password]")){s.preventDefault();let t=Object.fromEntries(new FormData(e)),a=g("[data-err]",e);if(t.pw1.length<6){a.textContent="Use at least 6 characters.",a.hidden=!1;return}if(t.pw1!==t.pw2){a.textContent="The passwords don\u2019t match.",a.hidden=!1;return}try{await d.changePassword(t.pw1),e.reset(),a.hidden=!0,k("Password changed")}catch(o){a.textContent=o.message,a.hidden=!1}}if(e.matches("[data-cat-add]")){s.preventDefault();let t=e.elements.name.value.trim();if(!t)return;await d.saveCategory({name:t}),E(),k("Category added"),J()}},Me=()=>_("table.tbl",n).forEach(s=>{let e=_("thead th",s).map(t=>t.textContent.trim());_("tbody tr",s).forEach(t=>_("td",t).forEach((a,o)=>{a.hasAttribute("data-label")||a.setAttribute("data-label",e[o]||"")}))}),fe=new MutationObserver(Me);return fe.observe(n,{childList:!0,subtree:!0}),n.addEventListener("click",ue),n.addEventListener("change",pe),n.addEventListener("input",me),n.addEventListener("submit",he),N(),()=>{fe.disconnect(),n.removeEventListener("click",ue),n.removeEventListener("change",pe),n.removeEventListener("input",me),n.removeEventListener("submit",he)}}window.MLAdmin={mount:je};})();
