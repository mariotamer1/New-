...
\u2022 6\u20138 years old
\u2022 10" tall`]],Ie=Date.UTC(2026,8,20),ve=De.map((n,p)=>{let[S,$,d,C,y,m,B,M,c,se,Z]=n,E=(be[S]||[]).map((I,W)=>({sm:`img/p/${S}-${I}-sm.webp`,lg:`img/p/${S}-${I}-lg.webp`,alt:W===0?$:`${$} \u2013 view ${W+1}`}));return{id:"p-"+String(p+1).padStart(3,"0"),slug:S,title:$,brand:d,categoryId:Ae(C),condition:y,price:m,originalPrice:B,shipping:M,inventory:c,bestDeal:se,published:!0,noShipping:S==="live-bonsai-tree",upc:Te(p+11),description:Z,images:E,createdAt:new Date(Ie-p*36e5*7).toISOString()}});var st={products:[],categories:[],settings:{...ge}};function k(n,p=""){let S=g(".toasts");S||(S=document.createElement("div"),S.className="toasts",S.setAttribute("role","status"),S.setAttribute("aria-live","polite"),document.body.appendChild(S));let $=document.createElement("div");$.className="toast "+p,$.innerHTML=`${h(p==="err"?"close":"check","icon-sm")}<span>${o(n)}</span>`,S.appendChild($),setTimeout(()=>{$.style.opacity="0",$.style.transition="opacity .3s",setTimeout(()=>$.remove(),320)},2600)}var qe=[["overview","Overview","chart"],["products","Products","tag"],["orders","Orders","box"],["offers","Offers","cash"],["messages","Messages","msg"],["categories","Categories","layers"],["settings","Settings","gear"]],_e=["New","Open Box","Like New","Refurbished","Used \u2013 Good","Used \u2013 Fair","Scratch & Dent"],ae=[["new","New"],["processing","Processing"],["ready","Ready for pickup"],["shipped","Shipped"],["completed","Completed"],["cancelled","Cancelled"]],ye=[["new","New"],["contacted","Contacted"],["accepted","Accepted"],["declined","Declined"]],Re=[["new","New"],["read","Read"],["archived","Archived"]];function Ue(n,{store:p,navigate:S,query:$}){let d=p.admin,C=$.get("tab")||"overview",y=$.get("edit")||null,m={products:[],orders:[],offers:[],inquiries:[]},B={q:"",f:"all"},M=new Set,c=null,se=()=>{let a=new URLSearchParams;C!=="overview"&&a.set("tab",C),y&&a.set("edit",y);let e="/admin"+(a.toString()?"?"+a:"");D.routing==="path"?history.replaceState({},"",U(e)):D.routing==="hash"&&history.replaceState(null,"","#"+e)},Z=a=>(p.category(a)||{}).name||"\u2014",E=()=>{p.refresh(),window.dispatchEvent(new CustomEvent("cart:change"))};async function I(){let[a,e,t,s]=await Promise.all([d.products(),d.orders(),d.offers(),d.inquiries()]);m={products:a,orders:e,offers:t,inquiries:s}}async function W(a){n.innerHTML=`<div class="adm-login"><form class="adm-login-card" novalidate data-login>
    ...
    </form></div>`,setTimeout(()=>{let e=g("#adm-pw");e&&e.focus()},30)}function _(a,e,t=""){let s=(L,O="new")=>m[L].filter(i=>i.status===O).length,r={orders:s("orders"),offers:s("offers"),messages:s("inquiries")};return`<div class="admin">
      <aside class="adm-side on-ink">
        <a class="logo" href="${U("/")}" aria-label="View store">${te()}</a>
        <span class="adm-tag">Admin</span>
        <nav aria-label="Admin"><ul>${qe.map(([L,O,i])=>`<li><button type="button" data-tab="${L}" ${C===L?'aria-current="page"':""}>${h(i)}<span>${O}</span>${r[L]?`<b class="adm-badge">${r[L]}</b>`:""}</button></li>`).join("")}</ul></nav>
    ...
    </div>`}async function N(){if(!await d.session()){W();return}if(await I(),se(),C==="products"&&y)return we();({overview:oe,products:V,orders:X,offers:le,messages:ce,categories:K,settings:Se}[C]||oe)()}function oe(){let a=m.products,e=a.filter(i=>i.published&&i.inventory>0),t=a.filter(i=>i.inventory<=0),s=a.filter(i=>i.inventory>0&&i.inventory<=2).slice(0,6),r=Date.now()-30*864e5,L=m.orders.filter(i=>i.status!=="cancelled"&&new Date(i.createdAt)>r).reduce((i,x)=>i+x.total,0),O=[["Live listings",e.length,"Published and in stock"],["Units in stock",a.reduce((i,x)=>i+Math.max(0,x.inventory),0),`${t.length} sold out (hidden)`],["New orders",m.orders.filter(i=>i.status==="new").length,`${m.orders.length} total`],["New offers",m.offers.filter(i=>i.status==="new").length,`${m.offers.length} total`],["Sales \xB7 30 days",P(L),"Excludes cancelled"],["Messages",m.inquiries.filter(i=>i.status==="new").length,"Contact + wholesale"]];n.innerHTML=_(`
      <div class="adm-tiles">${O.map(([i,x,l])=>`<div class="adm-tile"><span class="eyebrow">${i}</span><b class="tabnum">${x}</b><span class="muted">${l}</span></div>`).join("")}</div>
      <div class="adm-cols">
      ...
      </figcaption></figure>`).join(""):'<p class="muted" style="font-size:14px">No photos yet.</p>'}function $e(){let a=g("[data-editor]"),e=l=>a.elements[l],t=()=>{g("[data-photos]").innerHTML=re(c)},s=()=>{["title","description","brand","upc","slug","condition","categoryId"].forEach(l=>{c[l]=e(l).value}),c.price=e("price").value,c.originalPrice=e("originalPrice").value,c.shipping=e("shipping").value,c.inventory=e("inventory").value,c.published=e("published").checked,c.bestDeal=e("bestDeal").checked,c.noShipping=e("noShipping").checked};a.addEventListener("input",l=>{let b=l.target.name,v=parseFloat(e("originalPrice").value),A=parseFloat(e("pct").value),u=parseFloat(e("price").value);b==="pct"&&v>0&&A>=0&&(e("price").value=R(v*(1-A/100)).toFixed(2)),b==="price"&&v>0&&u>=0&&(e("pct").value=u<v?Math.round((1-u/v)*100):""),b==="originalPrice"&&v>0&&A>0&&(e("price").value=R(v*(1-A/100)).toFixed(2)),(b==="title"||b==="slug")&&(g("[data-slug-preview]").textContent=J(e("slug").value||e("title").value)||"product-name"),s()}),a.addEventListener("change",s),a.addEventListener("click",async l=>{let b=l.target.closest("[data-ed-inv]");b&&(e("inventory").value=Math.max(0,(parseInt(e("inventory").value,10)||0)+Number(b.dataset.edInv)),s());let v=l.target.closest("[data-ph-move]");if(v){let u=Number(v.closest("[data-ph]").dataset.ph),f=u+Number(v.dataset.phMove);[c.images[u],c.images[f]]=[c.images[f],c.images[u]],t()}let A=l.target.closest("[data-ph-del]");if(A){let u=Number(A.closest("[data-ph]").dataset.ph),[f]=c.images.splice(u,1);d.removeImage&&d.removeImage(f).catch(()=>{}),t()}l.target.closest("[data-delete-product]")&&(g("[data-confirm]").hidden=!1),l.target.closest("[data-confirm-cancel]")&&(g("[data-confirm]").hidden=!0),l.target.closest("[data-confirm-delete]")&&(await d.deleteProduct(y),E(),k("Product deleted"),y=null,c=null,N())});let r=null,L=g("[data-photos]");L.addEventListener("dragstart",l=>{let b=l.target.closest("[data-ph]");b&&(r=Number(b.dataset.ph),l.dataTransfer.effectAllowed="move",b.classList.add("dragging"))}),L.addEventListener("dragover",l=>{r!==null&&l.preventDefault()}),L.addEventListener("drop",l=>{let b=l.target.closest("[data-ph]");if(r===null||!b)return;l.preventDefault();let v=Number(b.dataset.ph),[A]=c.images.splice(r,1);c.images.splice(v,0,A),r=null,t()}),L.addEventListener("dragend",()=>{r=null,q(".dragging").forEach(l=>l.classList.remove("dragging"))});let O=g("[data-upload-status]"),i=async l=>{if(l=Array.from(l).filter(b=>b.type.startsWith("image/")),!!l.length){for(let b=0;b<l.length;b++){O.textContent=`Uploading ${b+1} of ${l.length}\u2026`;try{let v=await d.uploadImage(l[b]);c.images.push({...v,alt:""}),t()}catch(v){k(`Couldn\u2019t upload ${l[b].name}: ${v.message}`,"err")}}O.textContent=`${l.length} photo${l.length>1?"s":""} added \u2014 remember to save.`}};g("[data-files]").addEventListener("change",l=>{i(l.target.files),l.target.value=""});let x=g("[data-drop]");x.addEventListener("dragover",l=>{l.dataTransfer.types.includes("Files")&&(l.preventDefault(),x.classList.add("over"))}),x.addEventListener("dragleave",()=>x.classList.remove("over")),x.addEventListener("drop",l=>{l.dataTransfer.files.length&&(l.preventDefault(),x.classList.remove("over"),i(l.dataTransfer.files))}),a.addEventListener("submit",async l=>{l.preventDefault(),s();let b=g("[data-err]"),v=parseFloat(c.price),A=parseFloat(c.originalPrice),u=parseFloat(c.shipping||0),f=parseInt(c.inventory,10),w=String(c.upc||"").replace(/\D/g,""),T=[];if(c.title.trim()||T.push(["ed-title","Add a title."]),v>0||T.push(["ed-price","Enter a sale price greater than $0."]),f>=0||T.push(["ed-inv","Inventory must be 0 or more."]),u>=0||T.push(["ed-ship","Shipping must be 0 or more."]),w&&(w.length<8||w.length>14)&&T.push(["ed-upc","UPC should be 8\u201314 digits."]),q("[aria-invalid]",a).forEach(H=>H.removeAttribute("aria-invalid")),T.length){T.forEach(([H])=>g("#"+H).setAttribute("aria-invalid","true")),b.textContent=T.map(H=>H[1]).join(" "),b.hidden=!1,g("#"+T[0][0]).focus();return}let j={id:y==="new"?void 0:y,title:c.title.trim(),slug:c.slug.trim(),brand:c.brand.trim(),upc:w,condition:c.condition,categoryId:c.categoryId||null,description:c.description.trim(),price:R(v),originalPrice:A>v?R(A):null,shipping:R(u),inventory:f,published:c.published,bestDeal:c.bestDeal,noShipping:!!c.noShipping,images:c.images},z=g('button[type="submit"]',a);z.disabled=!0,z.textContent="Saving\u2026";try{let H=await d.saveProduct(j);return E(),k(y==="new"?"Product created":"Changes saved"),y=null,c=null,C="products",N(),H}catch(H){b.textContent=H.message,b.hidden=!1,z.disabled=!1,z.textContent="Save changes"}})}let Q="all";function X(){let a=m.orders.filter(e=>Q==="all"||e.status===Q);n.innerHTML=_(`
