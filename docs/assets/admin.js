...
\u2022 6\u20138 years old
\u2022 10" tall`]],Oe=Date.UTC(2026,8,20),ve=De.map((n,p)=>{let[S,$,d,M,y,m,B,C,c,se,Z]=n,E=(be[S]||[]).map((O,W)=>({sm:`img/p/${S}-${O}-sm.webp`,lg:`img/p/${S}-${O}-lg.webp`,alt:W===0?$:`${$} \u2013 view ${W+1}`}));return{id:"p-"+String(p+1).padStart(3,"0"),slug:S,title:$,brand:d,categoryId:Ae(M),condition:y,price:m,originalPrice:B,shipping:C,inventory:c,bestDeal:se,published:!0,noShipping:S==="live-bonsai-tree",upc:Te(p+11),description:Z,images:E,createdAt:new Date(Oe-p*36e5*7).toISOString()}});var st={products:[],categories:[],settings:{...ge}};function k(n,p=""){let S=g(".toasts");S||(S=document.createElement("div"),S.className="toasts",S.setAttribute("role","status"),S.setAttribute("aria-live","polite"),document.body.appendChild(S));let $=document.createElement("div");$.className="toast "+p,$.innerHTML=`${h(p==="err"?"close":"check","icon-sm")}<span>${o(n)}</span>`,S.appendChild($),setTimeout(()=>{$.style.opacity="0",$.style.transition="opacity .3s",setTimeout(()=>$.remove(),320)},2600)}var qe=[["overview","Overview","chart"],["products","Products","tag"],["orders","Orders","box"],["offers","Offers","cash"],["messages","Messages","msg"],["categories","Categories","layers"],["settings","Settings","gear"]],_e=["New","Open Box","Like New","Refurbished","Used \u2013 Good","Used \u2013 Fair","Scratch & Dent"],ae=[["new","New"],["processing","Processing"],["ready","Ready for pickup"],["shipped","Shipped"],["completed","Completed"],["cancelled","Cancelled"]],ye=[["new","New"],["contacted","Contacted"],["accepted","Accepted"],["declined","Declined"]],Re=[["new","New"],["read","Read"],["archived","Archived"]];function Ue(n,{store:p,navigate:S,query:$}){let d=p.admin,M=$.get("tab")||"overview",y=$.get("edit")||null,m={products:[],orders:[],offers:[],inquiries:[]},B={q:"",f:"all"},C=new Set,c=null,se=()=>{let t=new URLSearchParams;M!=="overview"&&t.set("tab",M),y&&t.set("edit",y);let e="/admin"+(t.toString()?"?"+t:"");D.routing==="path"?history.replaceState({},"",U(e)):D.routing==="hash"&&history.replaceState(null,"","#"+e)},Z=t=>(p.category(t)||{}).name||"\u2014",E=()=>{p.refresh(),window.dispatchEvent(new CustomEvent("cart:change"))};async function O(){let[t,e,a,s]=await Promise.all([d.products(),d.orders(),d.offers(),d.inquiries()]);m={products:t,orders:e,offers:a,inquiries:s}}async function W(t){n.innerHTML=`<div class="adm-login"><form class="adm-login-card" novalidate data-login>
      ...
      <p class="form-error" data-err ${t?"":"hidden"}>${o(t||"")}</p>
    ...
    </form></div>`,setTimeout(()=>{let e=g("#adm-pw");e&&e.focus()},30)}function _(t,e,a=""){let s=(L,I="new")=>m[L].filter(i=>i.status===I).length,r={orders:s("orders"),offers:s("offers"),messages:s("inquiries")};return`<div class="admin">
      <aside class="adm-side on-ink">
        <a class="logo" href="${U("/")}" aria-label="View store">${te()}</a>
        <span class="adm-tag">Admin</span>
        <nav aria-label="Admin"><ul>${qe.map(([L,I,i])=>`<li><button type="button" data-tab="${L}" ${M===L?'aria-current="page"':""}>${h(i)}<span>${I}</span>${r[L]?`<b class="adm-badge">${r[L]}</b>`:""}</button></li>`).join("")}</ul></nav>
    ...
    </div>`}async function N(){if(!await d.session()){W();return}if(await O(),se(),M==="products"&&y)return we();({overview:oe,products:V,orders:X,offers:le,messages:ce,categories:K,settings:Se}[M]||oe)()}function oe(){let t=m.products,e=t.filter(i=>i.published&&i.inventory>0),a=t.filter(i=>i.inventory<=0),s=t.filter(i=>i.inventory>0&&i.inventory<=2).slice(0,6),r=Date.now()-30*864e5,L=m.orders.filter(i=>i.status!=="cancelled"&&new Date(i.createdAt)>r).reduce((i,x)=>i+x.total,0),I=[["Live listings",e.length,"Published and in stock"],["Units in stock",t.reduce((i,x)=>i+Math.max(0,x.inventory),0),`${a.length} sold out (hidden)`],["New orders",m.orders.filter(i=>i.status==="new").length,`${m.orders.length} total`],["New offers",m.offers.filter(i=>i.status==="new").length,`${m.offers.length} total`],["Sales \xB7 30 days",P(L),"Excludes cancelled"],["Messages",m.inquiries.filter(i=>i.status==="new").length,"Contact + wholesale"]];n.innerHTML=_(`
      ...
      </div>`,"Overview",`<button class="btn btn-primary" type="button" data-new>${h("plus","icon-sm")} Add product</button>`)}let ie=(t,e)=>(t.find(([a])=>a===e)||[e,e])[1];function V(){let t=B.q.toLowerCase(),e=m.products.filter(s=>!t||`${s.title} ${s.brand||""} ${s.upc||""} ${s.slug}`.toLowerCase().includes(t)),a=B.f;a==="live"&&(e=e.filter(s=>s.published&&s.inventory>0)),a==="hidden"&&(e=e.filter(s=>!s.published)),a==="out"&&(e=e.filter(s=>s.inventory<=0)),a==="deal"&&(e=e.filter(s=>s.bestDeal)),C=new Set([...C].filter(s=>m.products.some(r=>r.id===s))),n.innerHTML=_(`
      ...
      <div class="table-wrap"><table class="tbl adm-table"><thead><tr>
        ...
        ${e.map(s=>`<tr data-row="${s.id}" class="${s.inventory<=0||!s.published?"dim":""}">
          <td><input type="checkbox" aria-label="Select ${o(s.title)}" data-sel="${s.id}" ${C.has(s.id)?"checked":""}></td>
      ...
      </tbody></table></div>`,"Products",`<button class="btn btn-primary" type="button" data-new>${h("plus","icon-sm")} Add product</button>`)}function we(){let t=y==="new",e=t?null:m.products.find(r=>r.id===y);if(!t&&!e)return y=null,V();(!c||c._for!==y)&&(c=e?JSON.parse(JSON.stringify(e)):{title:"",slug:"",brand:"",upc:"",condition:"New",categoryId:(p.categories()[0]||{}).id||null,description:"",price:"",originalPrice:"",shipping:9.99,inventory:1,published:!0,bestDeal:!1,images:[]},c._for=y);let a=c,s=a.originalPrice&&a.price&&Number(a.originalPrice)>Number(a.price)?Math.round((1-a.price/a.originalPrice)*100):"";n.innerHTML=_(`
      <form class="adm-editor" novalidate data-editor>
        <div class="adm-ed-main">
          <section class="adm-card"><div class="adm-card-head"><h2>Details</h2></div><div class="adm-card-body form">
            <div class="field"><label for="ed-title">Title</label><input id="ed-title" name="title" required maxlength="140" value="${o(a.title)}" placeholder="e.g. 55&quot; 4K Smart TV \u2013 Open Box"></div>
            <div class="field"><label for="ed-desc">Description</label><textarea id="ed-desc" name="description" rows="8" placeholder="What it is, condition notes, what\u2019s included. Start lines with \u2022 for bullet points.">${o(a.description)}</textarea></div>
            <div class="form-row two">
              <div class="field"><label for="ed-brand">Brand <span class="opt">(optional)</span></label><input id="ed-brand" name="brand" value="${o(a.brand||"")}"></div>
              <div class="field"><label for="ed-upc">UPC <span class="opt">(optional)</span></label><input id="ed-upc" name="upc" inputmode="numeric" maxlength="14" value="${o(a.upc||"")}"><span class="hint">8\u201314 digits from the barcode.</span></div>
            </div>
          </div></section>
        ...
        <div class="adm-ed-side">
          ...
          <section class="adm-card"><div class="adm-card-head"><h2>Pricing</h2></div><div class="adm-card-body form">
            <div class="field"><label for="ed-orig">Original / retail price</label><div class="money-input"><span>$</span><input id="ed-orig" name="originalPrice" type="number" inputmode="decimal" min="0" step="0.01" value="${o(a.originalPrice??"")}"></div></div>
            <div class="form-row two">
              ...
              <div class="field"><label for="ed-price">Sale price</label><div class="money-input"><span>$</span><input id="ed-price" name="price" type="number" inputmode="decimal" min="0" step="0.01" required value="${o(a.price??"")}"></div></div>
            </div>
            <p class="hint" data-price-hint>Enter a discount % to calculate the sale price automatically.</p>
            <div class="field"><label for="ed-ship">Shipping price</label><div class="money-input"><span>$</span><input id="ed-ship" name="shipping" type="number" inputmode="decimal" min="0" step="0.01" value="${o(a.shipping??0)}"></div><span class="hint">Local pickup is always free.</span></div>
          ...
          <section class="adm-card"><div class="adm-card-head"><h2>Inventory</h2></div><div class="adm-card-body form">
            <div class="field"><label for="ed-inv">Quantity in stock</label><div class="qty" style="height:46px"><button type="button" aria-label="Decrease" data-ed-inv="-1">${h("minus","icon-sm")}</button><input id="ed-inv" name="inventory" type="number" inputmode="numeric" min="0" step="1" value="${o(a.inventory)}" style="width:80px"><button type="button" aria-label="Increase" data-ed-inv="1">${h("plus","icon-sm")}</button></div></div>
          </div></section>
          <section class="adm-card"><div class="adm-card-head"><h2>Organize</h2></div><div class="adm-card-body form">
            <div class="field"><label for="ed-cat">Category</label><select id="ed-cat" name="categoryId">${p.categories().map(r=>`<option value="${r.id}" ${a.categoryId===r.id?"selected":""}>${o(r.name)}</option>`).join("")}<option value="" ${a.categoryId?"":"selected"}>Uncategorized</option></select></div>
            <div class="field"><label for="ed-cond">Condition</label><select id="ed-cond" name="condition">${_e.map(r=>`<option ${a.condition===r?"selected":""}>${r}</option>`).join("")}</select></div>
            <div class="field"><label for="ed-slug">Web address</label><input id="ed-slug" name="slug" value="${o(a.slug)}" placeholder="auto from title"><span class="hint">/products/<b data-slug-preview>${o(a.slug||J(a.title)||"product-name")}</b></span></div>
          </div></section>
      ...
      </form>`,t?"New product":"Edit product",`<button class="btn" type="button" data-cancel-edit>${h("left","icon-sm")} All products</button>`),$e()}function re(t){return t.images.length?t.images.map((e,a)=>`<figure class="adm-photo" draggable="true" data-ph="${a}">
      <img src="${o(F(e.sm||e.lg))}" alt="Photo ${a+1}">
      ...
      </figcaption></figure>`).join(""):'<p class="muted" style="font-size:14px">No photos yet.</p>'}function $e(){let t=g("[data-editor]"),e=l=>t.elements[l],a=()=>{g("[data-photos]").innerHTML=re(c)},s=()=>{["title","description","brand","upc","slug","condition","categoryId"].forEach(l=>{c[l]=e(l).value}),c.price=e("price").value,c.originalPrice=e("originalPrice").value,c.shipping=e("shipping").value,c.inventory=e("inventory").value,c.published=e("published").checked,c.bestDeal=e("bestDeal").checked,c.noShipping=e("noShipping").checked};t.addEventListener("input",l=>{let b=l.target.name,v=parseFloat(e("originalPrice").value),A=parseFloat(e("pct").value),u=parseFloat(e("price").value);b==="pct"&&v>0&&A>=0&&(e("price").value=R(v*(1-A/100)).toFixed(2)),b==="price"&&v>0&&u>=0&&(e("pct").value=u<v?Math.round((1-u/v)*100):""),b==="originalPrice"&&v>0&&A>0&&(e("price").value=R(v*(1-A/100)).toFixed(2)),(b==="title"||b==="slug")&&(g("[data-slug-preview]").textContent=J(e("slug").value||e("title").value)||"product-name"),s()}),t.addEventListener("change",s),t.addEventListener("click",async l=>{let b=l.target.closest("[data-ed-inv]");b&&(e("inventory").value=Math.max(0,(parseInt(e("inventory").value,10)||0)+Number(b.dataset.edInv)),s());let v=l.target.closest("[data-ph-move]");if(v){let u=Number(v.closest("[data-ph]").dataset.ph),f=u+Number(v.dataset.phMove);[c.images[u],c.images[f]]=[c.images[f],c.images[u]],a()}let A=l.target.closest("[data-ph-del]");if(A){let u=Number(A.closest("[data-ph]").dataset.ph),[f]=c.images.splice(u,1);d.removeImage&&d.removeImage(f).catch(()=>{}),a()}l.target.closest("[data-delete-product]")&&(g("[data-confirm]").hidden=!1),l.target.closest("[data-confirm-cancel]")&&(g("[data-confirm]").hidden=!0),l.target.closest("[data-confirm-delete]")&&(await d.deleteProduct(y),E(),k("Product deleted"),y=null,c=null,N())});let r=null,L=g("[data-photos]");L.addEventListener("dragstart",l=>{let b=l.target.closest("[data-ph]");b&&(r=Number(b.dataset.ph),l.dataTransfer.effectAllowed="move",b.classList.add("dragging"))}),L.addEventListener("dragover",l=>{r!==null&&l.preventDefault()}),L.addEventListener("drop",l=>{let b=l.target.closest("[data-ph]");if(r===null||!b)return;l.preventDefault();let v=Number(b.dataset.ph),[A]=c.images.splice(r,1);c.images.splice(v,0,A),r=null,a()}),L.addEventListener("dragend",()=>{r=null,q(".dragging").forEach(l=>l.classList.remove("dragging"))});let I=g("[data-upload-status]"),i=async l=>{if(l=Array.from(l).filter(b=>b.type.startsWith("image/")),!!l.length){for(let b=0;b<l.length;b++){I.textContent=`Uploading ${b+1} of ${l.length}\u2026`;try{let v=await d.uploadImage(l[b]);c.images.push({...v,alt:""}),a()}catch(v){k(`Couldn\u2019t upload ${l[b].name}: ${v.message}`,"err")}}I.textContent=`${l.length} photo${l.length>1?"s":""} added \u2014 remember to save.`}};g("[data-files]").addEventListener("change",l=>{i(l.target.files),l.target.value=""});let x=g("[data-drop]");x.addEventListener("dragover",l=>{l.dataTransfer.types.includes("Files")&&(l.preventDefault(),x.classList.add("over"))}),x.addEventListener("dragleave",()=>x.classList.remove("over")),x.addEventListener("drop",l=>{l.dataTransfer.files.length&&(l.preventDefault(),x.classList.remove("over"),i(l.dataTransfer.files))}),t.addEventListener("submit",async l=>{l.preventDefault(),s();let b=g("[data-err]"),v=parseFloat(c.price),A=parseFloat(c.originalPrice),u=parseFloat(c.shipping||0),f=parseInt(c.inventory,10),w=String(c.upc||"").replace(/\D/g,""),T=[];if(c.title.trim()||T.push(["ed-title","Add a title."]),v>0||T.push(["ed-price","Enter a sale price greater than $0."]),f>=0||T.push(["ed-inv","Inventory must be 0 or more."]),u>=0||T.push(["ed-ship","Shipping must be 0 or more."]),w&&(w.length<8||w.length>14)&&T.push(["ed-upc","UPC should be 8\u201314 digits."]),q("[aria-invalid]",t).forEach(H=>H.removeAttribute("aria-invalid")),T.length){T.forEach(([H])=>g("#"+H).setAttribute("aria-invalid","true")),b.textContent=T.map(H=>H[1]).join(" "),b.hidden=!1,g("#"+T[0][0]).focus();return}let j={id:y==="new"?void 0:y,title:c.title.trim(),slug:c.slug.trim(),brand:c.brand.trim(),upc:w,condition:c.condition,categoryId:c.categoryId||null,description:c.description.trim(),price:R(v),originalPrice:A>v?R(A):null,shipping:R(u),inventory:f,published:c.published,bestDeal:c.bestDeal,noShipping:!!c.noShipping,images:c.images},z=g('button[type="submit"]',t);z.disabled=!0,z.textContent="Saving\u2026";try{let H=await d.saveProduct(j);return E(),k(y==="new"?"Product created":"Changes saved"),y=null,c=null,M="products",N(),H}catch(H){b.textContent=H.message,b.hidden=!1,z.disabled=!1,z.textContent="Save changes"}})}let Q="all";function X(){let t=m.orders.filter(e=>Q==="all"||e.status===Q);n.innerHTML=_(`
      ...
      ${t.map(e=>`<tr><td class="mono"><b>#${e.number}</b></td><td>${G(e.createdAt,!0)}</td><td>${o(e.customer.name)}<small>${o(e.customer.phone)}</small></td><td>${e.items.reduce((a,s)=>a+s.qty,0)}</td><td>${e.fulfillment==="pickup"?"Pickup":"Ship"}</td><td class="tabnum"><b>${P(e.total)}</b></td>
      ...
      <dialog class="modal adm-order-dlg" id="order-dlg" aria-labelledby="od-title"></dialog>`,"Orders")}function ke(t){let e=m.orders.find(r=>r.id===t);if(!e)return;g("#order-dlg")||(M="orders",X());let a=g("#order-dlg"),s=e.address?`${o(e.address.line1)}${e.address.line2?", "+o(e.address.line2):""}<br>${o(e.address.city)}, ${o(e.address.state)} ${o(e.address.zip)}`:"Local pickup";a.innerHTML=`<div class="modal-head"><h2 id="od-title">Order #${e.number}</h2><button class="icon-btn" type="button" aria-label="Close" data-close-dlg>${h("close")}</button></div>
      ...
      </div>`,a.showModal()}function le(){n.innerHTML=_(m.offers.length?`<div class="table-wrap"><table class="tbl"><thead><tr><th>Date</th><th>Product</th><th>List price</th><th>Offer</th><th>Customer</th><th>Status</th><th></th></tr></thead><tbody>
      ${m.offers.map(t=>{let e=m.products.find(s=>s.id===t.productId),a=t.listPrice?Math.round(t.amount/t.listPrice*100):0;return`<tr>
        <td>${G(t.createdAt,!0)}</td>
        <td><div class="adm-prod">${t.productImage?`<img src="${o(F(t.productImage.sm))}" alt="" width="48" height="48">`:""}<div>${e?`<button class="adm-title" type="button" data-edit="${e.id}">${o(t.productTitle)}</button>`:o(t.productTitle)}<small>${e?`${e.inventory} in stock`:"Deleted"}</small></div></div></td>
        <td class="tabnum">${P(t.listPrice)}</td>
        <td class="tabnum"><b>${P(t.amount)}</b><small>${a}% of list</small></td>
        <td>${o(t.name)}<small><a href="${Y(t.phone)}">${o(t.phone)}</a> \xB7 <a href="sms:${o(t.phone.replace(/[^\d+]/g,""))}">Text</a></small></td>
        <td><label class="sr-only" for="ofs-${t.id}">Status</label><select id="ofs-${t.id}" class="adm-status" data-offer-status="${t.id}">${ye.map(([s,r])=>`<option value="${s}" ${t.status===s?"selected":""}>${r}</option>`).join("")}</select></td>
      ...
      </tbody></table></div>`:'<div class="empty"><h2>No offers yet</h2><p class="muted">When a customer taps \u201CMake Offer\u201D, their name, phone and amount appear here.</p></div>',"Offers")}function ce(){n.innerHTML=_(m.inquiries.length?`<div class="adm-msgs">${m.inquiries.map(t=>`<article class="adm-card adm-msg ${t.status==="new"?"unread":""}">
      <div class="adm-card-head"><h2>${o(t.name)} <span class="status">${t.type==="wholesale"?"Wholesale":t.type==="pickup"?"Pickup request":"Contact"}</span></h2><span class="muted" style="font-size:13px">${G(t.createdAt,!0)}</span></div>
      <div class="adm-card-body"><p class="adm-msg-meta"><a href="${Y(t.phone)}">${o(t.phone)}</a> \xB7 <a href="sms:${o(String(t.phone).replace(/[^\d+]/g,""))}">Text</a>${t.email?` \xB7 <a href="mailto:${o(t.email)}">${o(t.email)}</a>`:""}${t.company?" \xB7 "+o(t.company):""}</p><p class="adm-msg-text">${o(t.message)}</p>
      <div class="adm-msg-actions"><label class="sr-only" for="ms-${t.id}">Status</label><select id="ms-${t.id}" class="adm-status" data-msg-status="${t.id}">${Re.map(([e,a])=>`<option value="${e}" ${t.status===e?"selected":""}>${a}</option>`).join("")}</select><button class="btn btn-sm" type="button" data-msg-del="${t.id}">${h("trash","icon-sm")} Delete</button></div></div>
    </article>`).join("")}</div>`:'<div class="empty"><h2>No messages</h2><p class="muted">Contact form and wholesale requests appear here.</p></div>',"Messages")}function K(){let t=p.categories(),e={};m.products.forEach(a=>{e[a.categoryId]=(e[a.categoryId]||0)+1}),n.innerHTML=_(`<div class="adm-card"><div class="adm-card-body">
      <ul class="adm-cats">${t.map((a,s)=>`<li data-cat="${a.id}">
        <label class="sr-only" for="cn-${a.id}">Category name</label><input id="cn-${a.id}" value="${o(a.name)}" data-cat-name>
    ...
    </div></div>`,"Categories")}function Se(){let t=p.settings(),e=(a,s,r="")=>`<div class="field"><label for="st-${a}">${s}</label><input id="st-${a}" name="${a}" value="${o(t[a]||"")}" ${r}></div>`;n.innerHTML=_(`<div class="adm-cols two">
    ...
    </div>`,"Settings")}let de=async t=>{let e=t.target,a=e.closest("[data-tab]");if(a){M=a.dataset.tab,y=null,c=null,N();return}if(e.closest("[data-logout]")){await d.logout(),W();return}if(e.closest("[data-new]")){M="products",y="new",c=null,N();return}let s=e.closest("[data-edit]");if(s){M="products",y=s.dataset.edit,c=null,N();return}if(e.closest("[data-cancel-edit]")){y=null,c=null,M="products",N();return}let r=e.closest("[data-inv]");if(r){let u=await d.adjustInventory(r.dataset.id,Number(r.dataset.inv));if(E(),u){let f=r.closest("tr");g("b",r.parentElement).textContent=u.inventory,f.classList.toggle("dim",u.inventory<=0||!u.published)}return}let L=e.closest("[data-open-order]");if(L){ke(L.dataset.openOrder);return}if(e.closest("[data-close-dlg]")||e.matches("dialog.modal")){let u=e.closest("dialog");u&&u.close();return}let I=e.closest("[data-cancel-order]");if(I){await d.updateOrder(I.dataset.cancelOrder,{status:"cancelled",restock:!0}),E(),k("Order cancelled and items restocked");let u=e.closest("dialog");u&&u.close(),await O(),X();return}let i=e.closest("[data-accept-price]");if(i){let u=m.offers.find(w=>w.id===i.dataset.acceptPrice),f=m.products.find(w=>w.id===u.productId);f&&(await d.saveProduct({...f,originalPrice:f.originalPrice||f.price,price:u.amount}),await d.updateOffer(u.id,{status:"accepted"}),E(),k(`Price set to ${P(u.amount)} and offer accepted`),await O(),le());return}let x=e.closest("[data-bulk]");if(x){let u=[...C],f=x.dataset.bulk;if(f==="delete"&&!x.dataset.sure){x.dataset.sure="1",x.textContent=`Confirm delete ${u.length}`,x.classList.add("adm-danger");return}for(let w of u){let T=m.products.find(j=>j.id===w);if(T){if(f==="discount"){let j=Math.min(95,Math.max(1,parseInt(g("[data-bulk-pct]").value,10)||0)),z=T.originalPrice||T.price;await d.saveProduct({...T,originalPrice:z,price:R(z*(1-j/100))})}f==="publish"&&await d.patchProduct(w,{published:!0}),f==="unpublish"&&await d.patchProduct(w,{published:!1}),f==="delete"&&await d.deleteProduct(w)}}C.clear(),E(),k("Updated "+u.length+" product"+(u.length>1?"s":"")),await O(),V();return}let l=e.closest("[data-cat-move]");if(l){let u=l.closest("[data-cat]"),f=p.categories().map(j=>j.id),w=f.indexOf(u.dataset.cat),T=w+Number(l.dataset.catMove);[f[w],f[T]]=[f[T],f[w]],await d.reorderCategories(f),E(),K();return}let b=e.closest("[data-cat-save]");if(b){let u=b.closest("[data-cat]"),f=g("[data-cat-name]",u).value.trim();if(!f)return;await d.saveCategory({id:u.dataset.cat,name:f}),E(),k("Category saved"),K();return}let v=e.closest("[data-cat-del]");if(v){if(!v.dataset.sure){v.dataset.sure="1",v.textContent="Confirm";return}await d.deleteCategory(v.closest("[data-cat]").dataset.cat),E(),k("Category deleted"),await O(),K();return}let A=e.closest("[data-msg-del]");if(A){await d.deleteInquiry(A.dataset.msgDel),await O(),ce();return}if(e.closest("[data-export]")){let u=await d.exportData(),f=new Blob([u],{type:"application/json"}),w=document.createElement("a");w.href=URL.createObjectURL(f),w.download=`mlgroup-backup-${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(w),w.click(),w.remove(),k("Backup downloaded");return}if(e.closest("[data-reset]")){g("[data-reset-confirm]").hidden=!1;return}if(e.closest("[data-reset-no]")){g("[data-reset-confirm]").hidden=!0;return}e.closest("[data-reset-yes]")&&(await d.reset(),E(),k("Demo data restored"),N())},ue=async t=>{let e=t.target;if(e.matches("[data-pf-f]")&&(B.f=e.value,V()),e.matches("[data-sel]")){e.checked?C.add(e.dataset.sel):C.delete(e.dataset.sel);let a=g("[data-bulk]");a.hidden=!C.size,g("[data-bulk-n]").textContent=`${C.size} selected`}if(e.matches("[data-sel-all]")){q("[data-sel]").forEach(s=>{s.checked=e.checked,e.checked?C.add(s.dataset.sel):C.delete(s.dataset.sel)});let a=g("[data-bulk]");a.hidden=!C.size,g("[data-bulk-n]").textContent=`${C.size} selected`}if(e.matches("[data-pub]")&&(await d.patchProduct(e.dataset.pub,{published:e.checked}),E(),e.nextElementSibling.textContent=e.checked?"Published":"Hidden",k(e.checked?"Published":"Hidden from store")),e.matches("[data-deal]")&&(await d.patchProduct(e.dataset.deal,{bestDeal:e.checked}),E(),k(e.checked?"Added to Best Deals":"Removed from Best Deals")),e.matches("[data-of]")&&(Q=e.value,X()),e.matches("[data-order-status]")&&(await d.updateOrder(e.dataset.orderStatus,{status:e.value}),k("Order status updated"),await O()),e.matches("[data-offer-status]")&&(await d.updateOffer(e.dataset.offerStatus,{status:e.value}),k("Offer updated"),await O()),e.matches("[data-msg-status]")&&(await d.updateInquiry(e.dataset.msgStatus,{status:e.value}),await O()),e.matches("[data-import]")&&e.files[0])try{await d.importData(await e.files[0].text()),E(),k("Backup imported"),N()}catch(a){k(a.message,"err")}},pe=t=>{if(t.target.matches("[data-pf-q]")){B.q=t.target.value;let e=t.target.selectionStart;V();let a=g("[data-pf-q]");a.focus(),a.setSelectionRange(e,e)}},me=async t=>{let e=t.target;if(e.matches("[data-login]")){t.preventDefault();let a=g('button[type="submit"]',e);a.disabled=!0;try{await d.login(e.elements.pw.value),N()}catch(s){W(s.message||"Sign in failed.")}}if(e.matches("[data-settings]")){t.preventDefault();let a=Object.fromEntries(new FormData(e));a.state=(a.state||"").toUpperCase(),await d.saveSettings(a),E(),k("Store details saved")}if(e.matches("[data-password]")){t.preventDefault();let a=Object.fromEntries(new FormData(e)),s=g("[data-err]",e);if(a.pw1.length<6){s.textContent="Use at least 6 characters.",s.hidden=!1;return}if(a.pw1!==a.pw2){s.textContent="The passwords don\u2019t match.",s.hidden=!1;return}try{await d.changePassword(a.pw1),e.reset(),s.hidden=!0,k("Password changed")}catch(r){s.textContent=r.message,s.hidden=!1}}if(e.matches("[data-cat-add]")){t.preventDefault();let a=e.elements.name.value.trim();if(!a)return;await d.saveCategory({name:a}),E(),k("Category added"),K()}},xe=()=>q("table.tbl",n).forEach(t=>{let e=q("thead th",t).map(a=>a.textContent.trim());q("tbody tr",t).forEach(a=>q("td",a).forEach((s,r)=>{s.hasAttribute("data-label")||s.setAttribute("data-label",e[r]||"")}))}),he=new MutationObserver(xe);return he.observe(n,{childList:!0,subtree:!0}),n.addEventListener("click",de),n.addEventListener("change",ue),n.addEventListener("input",pe),n.addEventListener("submit",me),N(),()=>{he.disconnect(),n.removeEventListener("click",de),n.removeEventListener("change",ue),n.removeEventListener("input",pe),n.removeEventListener("submit",me)}}window.MLAdmin={mount:Ue};})();
