...
\u2022 6\u20138 years old
\u2022 10" tall`]],Oe=Date.UTC(2026,8,20),ve=De.map((n,p)=>{let[S,$,d,C,y,m,B,M,c,se,Z]=n,E=(be[S]||[]).map((O,W)=>({sm:`img/p/${S}-${O}-sm.webp`,lg:`img/p/${S}-${O}-lg.webp`,alt:W===0?$:`${$} \u2013 view ${W+1}`}));return{id:"p-"+String(p+1).padStart(3,"0"),slug:S,title:$,brand:d,categoryId:Te(C),condition:y,price:m,originalPrice:B,shipping:M,inventory:c,bestDeal:se,published:!0,noShipping:S==="live-bonsai-tree",upc:Ae(p+11),description:Z,images:E,createdAt:new Date(Oe-p*36e5*7).toISOString()}});var st={products:[],categories:[],settings:{...ge}};function k(n,p=""){let S=g(".toasts");S||(S=document.createElement("div"),S.className="toasts",S.setAttribute("role","status"),S.setAttribute("aria-live","polite"),document.body.appendChild(S));let $=document.createElement("div");$.className="toast "+p,$.innerHTML=`${h(p==="err"?"close":"check","icon-sm")}<span>${o(n)}</span>`,S.appendChild($),setTimeout(()=>{$.style.opacity="0",$.style.transition="opacity .3s",setTimeout(()=>$.remove(),320)},2600)}var qe=[["overview","Overview","chart"],["products","Products","tag"],["orders","Orders","box"],["offers","Offers","cash"],["messages","Messages","msg"],["categories","Categories","layers"],["settings","Settings","gear"]],_e=["New","Open Box","Like New","Refurbished","Used \u2013 Good","Used \u2013 Fair","Scratch & Dent"],ae=[["new","New"],["processing","Processing"],["ready","Ready for pickup"],["shipped","Shipped"],["completed","Completed"],["cancelled","Cancelled"]],ye=[["new","New"],["contacted","Contacted"],["accepted","Accepted"],["declined","Declined"]],Re=[["new","New"],["read","Read"],["archived","Archived"]];function Ue(n,{store:p,navigate:S,query:$}){let d=p.admin,C=$.get("tab")||"overview",y=$.get("edit")||null,m={products:[],orders:[],offers:[],inquiries:[]},B={q:"",f:"all"},M=new Set,c=null,se=()=>{let a=new URLSearchParams;C!=="overview"&&a.set("tab",C),y&&a.set("edit",y);let e="/admin"+(a.toString()?"?"+a:"");D.routing==="path"?history.replaceState({},"",U(e)):D.routing==="hash"&&history.replaceState(null,"","#"+e)},Z=a=>(p.category(a)||{}).name||"\u2014",E=()=>{p.refresh(),window.dispatchEvent(new CustomEvent("cart:change"))};async function O(){let[a,e,t,s]=await Promise.all([d.products(),d.orders(),d.offers(),d.inquiries()]);m={products:a,orders:e,offers:t,inquiries:s}}async function W(a){n.innerHTML=`<div class="adm-login"><form class="adm-login-card" novalidate data-login>
    ...
    </form></div>`,setTimeout(()=>{let e=g("#adm-pw");e&&e.focus()},30)}function _(a,e,t=""){let s=(L,I="new")=>m[L].filter(i=>i.status===I).length,r={orders:s("orders"),offers:s("offers"),messages:s("inquiries")};return`<div class="admin">
      <aside class="adm-side on-ink">
        <a class="logo" href="${U("/")}" aria-label="View store">${te()}</a>
        <span class="adm-tag">Admin</span>
        <nav aria-label="Admin"><ul>${qe.map(([L,I,i])=>`<li><button type="button" data-tab="${L}" ${C===L?'aria-current="page"':""}>${h(i)}<span>${I}</span>${r[L]?`<b class="adm-badge">${r[L]}</b>`:""}</button></li>`).join("")}</ul></nav>
        <div class="adm-side-foot"><a href="${U("/")}">${h("eye","icon-sm")} View store</a><button type="button" data-logout>${h("logout","icon-sm")} Sign out</button></div>
      </aside>
    ...
    </div>`}async function N(){if(!await d.session()){W();return}if(await O(),se(),C==="products"&&y)return we();({overview:oe,products:V,orders:X,offers:le,messages:ce,categories:K,settings:Se}[C]||oe)()}function oe(){let a=m.products,e=a.filter(i=>i.published&&i.inventory>0),t=a.filter(i=>i.inventory<=0),s=a.filter(i=>i.inventory>0&&i.inventory<=2).slice(0,6),r=Date.now()-30*864e5,L=m.orders.filter(i=>i.status!=="cancelled"&&new Date(i.createdAt)>r).reduce((i,x)=>i+x.total,0),I=[["Live listings",e.length,"Published and in stock"],["Units in stock",a.reduce((i,x)=>i+Math.max(0,x.inventory),0),`${t.length} sold out (hidden)`],["New orders",m.orders.filter(i=>i.status==="new").length,`${m.orders.length} total`],["New offers",m.offers.filter(i=>i.status==="new").length,`${m.offers.length} total`],["Sales \xB7 30 days",P(L),"Excludes cancelled"],["Messages",m.inquiries.filter(i=>i.status==="new").length,"Contact + wholesale"]];n.innerHTML=_(`
      <div class="adm-tiles">${I.map(([i,x,l])=>`<div class="adm-tile"><span class="eyebrow">${i}</span><b class="tabnum">${x}</b><span class="muted">${l}</span></div>`).join("")}</div>
      <div class="adm-cols">
        <section class="adm-card"><div class="adm-card-head"><h2>Recent orders</h2><button class="link" type="button" data-tab="orders">All orders</button></div>
          ${m.orders.length?`<ul class="adm-list">${m.orders.slice(0,6).map(i=>`<li><button type="button" data-open-order="${i.id}"><span><b class="mono">#${i.number}</b> ${o(i.customer.name)}<small>${G(i.createdAt,!0)} \xB7 ${i.items.length} item${i.items.length>1?"s":""} \xB7 ${i.fulfillment==="pickup"?"Pickup":"Ship"}</small></span><span class="tabnum">${P(i.total)}</span><span class="status s-${i.status}">${ie(ae,i.status)}</span></button></li>`).join("")}</ul>`:'<p class="muted adm-empty">No orders yet. They appear here as soon as a customer checks out.</p>'}
        </section>
        <section class="adm-card"><div class="adm-card-head"><h2>Latest offers</h2><button class="link" type="button" data-tab="offers">All offers</button></div>
          ${m.offers.length?`<ul class="adm-list">${m.offers.slice(0,6).map(i=>`<li><button type="button" data-tab="offers"><span><b>${P(i.amount)}</b> for ${o(i.productTitle)}<small>${o(i.name)} \xB7 ${o(i.phone)} \xB7 ${G(i.createdAt,!0)}</small></span><span class="status s-${i.status}">${ie(ye,i.status)}</span></button></li>`).join("")}</ul>`:'<p class="muted adm-empty">No offers yet. Offers from the \u201CMake Offer\u201D button land here.</p>'}
        </section>
        <section class="adm-card"><div class="adm-card-head"><h2>Low stock</h2><button class="link" type="button" data-tab="products">Inventory</button></div>
          ${s.length?`<ul class="adm-list">${s.map(i=>`<li><button type="button" data-edit="${i.id}"><span>${o(i.title)}<small>${Z(i.categoryId)}</small></span><b class="tabnum">${i.inventory} left</b></button></li>`).join("")}</ul>`:'<p class="muted adm-empty">Nothing is running low.</p>'}
        </section>
      </div>`,"Overview",`<button class="btn btn-primary" type="button" data-new>${h("plus","icon-sm")} Add product</button>`)}let ie=(a,e)=>(a.find(([t])=>t===e)||[e,e])[1];function V(){let a=B.q.toLowerCase(),e=m.products.filter(s=>!a||`${s.title} ${s.brand||""} ${s.upc||""} ${s.slug}`.toLowerCase().includes(a)),t=B.f;t==="live"&&(e=e.filter(s=>s.published&&s.inventory>0)),t==="hidden"&&(e=e.filter(s=>!s.published)),t==="out"&&(e=e.filter(s=>s.inventory<=0)),t==="deal"&&(e=e.filter(s=>s.bestDeal)),M=new Set([...M].filter(s=>m.products.some(r=>r.id===s))),n.innerHTML=_(`
      <div class="adm-toolbar">
        <div class="search-field adm-search">${h("search")}<label class="sr-only" for="pf-q">Search products</label><input id="pf-q" type="search" placeholder="Search title, brand, UPC" value="${o(B.q)}" data-pf-q></div>
        <div class="select"><label class="sr-only" for="pf-f">Show</label><select id="pf-f" data-pf-f>${[["all","All products"],["live","Live"],["hidden","Unpublished"],["out","Sold out"],["deal","Best deals"]].map(([s,r])=>`<option value="${s}" ${t===s?"selected":""}>${r}</option>`).join("")}</select>${h("down")}</div>
      ...
      <div class="table-wrap"><table class="tbl adm-table"><thead><tr>
        ...
        ${e.map(s=>`<tr data-row="${s.id}" class="${s.inventory<=0||!s.published?"dim":""}">
          <td><input type="checkbox" aria-label="Select ${o(s.title)}" data-sel="${s.id}" ${M.has(s.id)?"checked":""}></td>
          <td><div class="adm-prod">${s.images[0]?`<img src="${o(F(s.images[0].sm))}" alt="" width="48" height="48" loading="lazy">`:'<span class="adm-noimg"></span>'}<div><button class="adm-title" type="button" data-edit="${s.id}">${o(s.title)}</button><small>${o(Z(s.categoryId))} \xB7 ${o(s.condition)}${s.noShipping?" \xB7 Pick up only":""}${s.upc?" \xB7 "+o(s.upc):""}</small></div></div></td>
          ...
          <td class="adm-row-actions"><button class="btn btn-sm" type="button" data-edit="${s.id}">${h("edit","icon-sm")} Edit</button>${s.published&&s.inventory>0?`<a class="btn btn-sm" href="${U("/products/"+s.slug)}" target="_blank" rel="noopener" aria-label="View ${o(s.title)} on the store">${h("eye","icon-sm")}</a>`:""}</td>
        </tr>`).join("")||'<tr><td colspan="6" class="muted">No products match.</td></tr>'}
      </tbody></table></div>`,"Products",`<button class="btn btn-primary" type="button" data-new>${h("plus","icon-sm")} Add product</button>`)}function we(){let a=y==="new",e=a?null:m.products.find(r=>r.id===y);if(!a&&!e)return y=null,V();(!c||c._for!==y)&&(c=e?JSON.parse(JSON.stringify(e)):{title:"",slug:"",brand:"",upc:"",condition:"New",categoryId:(p.categories()[0]||{}).id||null,description:"",price:"",originalPrice:"",shipping:9.99,inventory:1,published:!0,bestDeal:!1,images:[]},c._for=y);let t=c,s=t.originalPrice&&t.price&&Number(t.originalPrice)>Number(t.price)?Math.round((1-t.price/t.originalPrice)*100):"";n.innerHTML=_(`
      <form class="adm-editor" novalidate data-editor>
        <div class="adm-ed-main">
          ...
          <section class="adm-card"><div class="adm-card-head"><h2>Photos</h2><span class="muted" style="font-size:13px">First photo is the cover. Drag to reorder.</span></div><div class="adm-card-body">
            <div class="adm-photos" data-photos>${re(t)}</div>
        ...
        <div class="adm-ed-side">
          <section class="adm-card"><div class="adm-card-head"><h2>Visibility</h2></div><div class="adm-card-body form">
            ...
            <label class="switch big"><input type="checkbox" name="bestDeal" ${t.bestDeal?"checked":""}><span>Show in Best Deals</span></label>
          ...
          <section class="adm-card"><div class="adm-card-head"><h2>Inventory</h2></div><div class="adm-card-body form">
            <div class="field"><label for="ed-inv">Quantity in stock</label><div class="qty" style="height:46px"><button type="button" aria-label="Decrease" data-ed-inv="-1">${h("minus","icon-sm")}</button><input id="ed-inv" name="inventory" type="number" inputmode="numeric" min="0" step="1" value="${o(t.inventory)}" style="width:80px"><button type="button" aria-label="Increase" data-ed-inv="1">${h("plus","icon-sm")}</button></div></div>
          </div></section>
          <section class="adm-card"><div class="adm-card-head"><h2>Organize</h2></div><div class="adm-card-body form">
            <div class="field"><label for="ed-cat">Category</label><select id="ed-cat" name="categoryId">${p.categories().map(r=>`<option value="${r.id}" ${t.categoryId===r.id?"selected":""}>${o(r.name)}</option>`).join("")}<option value="" ${t.categoryId?"":"selected"}>Uncategorized</option></select></div>
            <div class="field"><label for="ed-cond">Condition</label><select id="ed-cond" name="condition">${_e.map(r=>`<option ${t.condition===r?"selected":""}>${r}</option>`).join("")}</select></div>
            <div class="field"><label for="ed-slug">Web address</label><input id="ed-slug" name="slug" value="${o(t.slug)}" placeholder="auto from title"><span class="hint">/products/<b data-slug-preview>${o(t.slug||J(t.title)||"product-name")}</b></span></div>
          </div></section>
          ...
          <div class="adm-ed-actions">
            ...
            ${a?"":`<button class="btn btn-lg adm-danger" type="button" data-delete-product>${h("trash","icon-sm")} Delete</button>`}
          </div>
      ...
      </form>`,a?"New product":"Edit product",`<button class="btn" type="button" data-cancel-edit>${h("left","icon-sm")} All products</button>`),$e()}function re(a){return a.images.length?a.images.map((e,t)=>`<figure class="adm-photo" draggable="true" data-ph="${t}">
      <img src="${o(F(e.sm||e.lg))}" alt="Photo ${t+1}">
      ${t===0?'<span class="adm-cover">Cover</span>':""}
      <figcaption>
        <button type="button" aria-label="Move photo ${t+1} left" data-ph-move="-1" ${t===0?"disabled":""}>${h("left","icon-sm")}</button>
        <span class="mono">${t+1}</span>
        <button type="button" aria-label="Move photo ${t+1} right" data-ph-move="1" ${t===a.images.length-1?"disabled":""}>${h("right","icon-sm")}</button>
        <button type="button" aria-label="Delete photo ${t+1}" data-ph-del>${h("trash","icon-sm")}</button>
      </figcaption></figure>`).join(""):'<p class="muted" style="font-size:14px">No photos yet.</p>'}function $e(){let a=g("[data-editor]"),e=l=>a.elements[l],t=()=>{g("[data-photos]").innerHTML=re(c)},s=()=>{["title","description","brand","upc","slug","condition","categoryId"].forEach(l=>{c[l]=e(l).value}),c.price=e("price").value,c.originalPrice=e("originalPrice").value,c.shipping=e("shipping").value,c.inventory=e("inventory").value,c.published=e("published").checked,c.bestDeal=e("bestDeal").checked,c.noShipping=e("noShipping").checked};a.addEventListener("input",l=>{let b=l.target.name,v=parseFloat(e("originalPrice").value),T=parseFloat(e("pct").value),u=parseFloat(e("price").value);b==="pct"&&v>0&&T>=0&&(e("price").value=R(v*(1-T/100)).toFixed(2)),b==="price"&&v>0&&u>=0&&(e("pct").value=u<v?Math.round((1-u/v)*100):""),b==="originalPrice"&&v>0&&T>0&&(e("price").value=R(v*(1-T/100)).toFixed(2)),(b==="title"||b==="slug")&&(g("[data-slug-preview]").textContent=J(e("slug").value||e("title").value)||"product-name"),s()}),a.addEventListener("change",s),a.addEventListener("click",async l=>{let b=l.target.closest("[data-ed-inv]");b&&(e("inventory").value=Math.max(0,(parseInt(e("inventory").value,10)||0)+Number(b.dataset.edInv)),s());let v=l.target.closest("[data-ph-move]");if(v){let u=Number(v.closest("[data-ph]").dataset.ph),f=u+Number(v.dataset.phMove);[c.images[u],c.images[f]]=[c.images[f],c.images[u]],t()}let T=l.target.closest("[data-ph-del]");if(T){let u=Number(T.closest("[data-ph]").dataset.ph),[f]=c.images.splice(u,1);d.removeImage&&d.removeImage(f).catch(()=>{}),t()}l.target.closest("[data-delete-product]")&&(g("[data-confirm]").hidden=!1),l.target.closest("[data-confirm-cancel]")&&(g("[data-confirm]").hidden=!0),l.target.closest("[data-confirm-delete]")&&(await d.deleteProduct(y),E(),k("Product deleted"),y=null,c=null,N())});let r=null,L=g("[data-photos]");L.addEventListener("dragstart",l=>{let b=l.target.closest("[data-ph]");b&&(r=Number(b.dataset.ph),l.dataTransfer.effectAllowed="move",b.classList.add("dragging"))}),L.addEventListener("dragover",l=>{r!==null&&l.preventDefault()}),L.addEventListener("drop",l=>{let b=l.target.closest("[data-ph]");if(r===null||!b)return;l.preventDefault();let v=Number(b.dataset.ph),[T]=c.images.splice(r,1);c.images.splice(v,0,T),r=null,t()}),L.addEventListener("dragend",()=>{r=null,q(".dragging").forEach(l=>l.classList.remove("dragging"))});let I=g("[data-upload-status]"),i=async l=>{if(l=Array.from(l).filter(b=>b.type.startsWith("image/")),!!l.length){for(let b=0;b<l.length;b++){I.textContent=`Uploading ${b+1} of ${l.length}\u2026`;try{let v=await d.uploadImage(l[b]);c.images.push({...v,alt:""}),t()}catch(v){k(`Couldn\u2019t upload ${l[b].name}: ${v.message}`,"err")}}I.textContent=`${l.length} photo${l.length>1?"s":""} added \u2014 remember to save.`}};g("[data-files]").addEventListener("change",l=>{i(l.target.files),l.target.value=""});let x=g("[data-drop]");x.addEventListener("dragover",l=>{l.dataTransfer.types.includes("Files")&&(l.preventDefault(),x.classList.add("over"))}),x.addEventListener("dragleave",()=>x.classList.remove("over")),x.addEventListener("drop",l=>{l.dataTransfer.files.length&&(l.preventDefault(),x.classList.remove("over"),i(l.dataTransfer.files))}),a.addEventListener("submit",async l=>{l.preventDefault(),s();let b=g("[data-err]"),v=parseFloat(c.price),T=parseFloat(c.originalPrice),u=parseFloat(c.shipping||0),f=parseInt(c.inventory,10),w=String(c.upc||"").replace(/\D/g,""),A=[];if(c.title.trim()||A.push(["ed-title","Add a title."]),v>0||A.push(["ed-price","Enter a sale price greater than $0."]),f>=0||A.push(["ed-inv","Inventory must be 0 or more."]),u>=0||A.push(["ed-ship","Shipping must be 0 or more."]),w&&(w.length<8||w.length>14)&&A.push(["ed-upc","UPC should be 8\u201314 digits."]),q("[aria-invalid]",a).forEach(H=>H.removeAttribute("aria-invalid")),A.length){A.forEach(([H])=>g("#"+H).setAttribute("aria-invalid","true")),b.textContent=A.map(H=>H[1]).join(" "),b.hidden=!1,g("#"+A[0][0]).focus();return}let j={id:y==="new"?void 0:y,title:c.title.trim(),slug:c.slug.trim(),brand:c.brand.trim(),upc:w,condition:c.condition,categoryId:c.categoryId||null,description:c.description.trim(),price:R(v),originalPrice:T>v?R(T):null,shipping:R(u),inventory:f,published:c.published,bestDeal:c.bestDeal,noShipping:!!c.noShipping,images:c.images},z=g('button[type="submit"]',a);z.disabled=!0,z.textContent="Saving\u2026";try{let H=await d.saveProduct(j);return E(),k(y==="new"?"Product created":"Changes saved"),y=null,c=null,C="products",N(),H}catch(H){b.textContent=H.message,b.hidden=!1,z.disabled=!1,z.textContent="Save changes"}})}let Q="all";function X(){let a=m.orders.filter(e=>Q==="all"||e.status===Q);n.innerHTML=_(`
      <div class="adm-toolbar"><div class="select"><label class="sr-only" for="of-f">Status</label><select id="of-f" data-of>${[["all","All orders"],...ae].map(([e,t])=>`<option value="${e}" ${Q===e?"selected":""}>${t}</option>`).join("")}</select>${h("down")}</div><span class="muted" style="font-size:14px">${a.length} order${a.length===1?"":"s"}</span></div>
      ...
      ${a.map(e=>`<tr><td class="mono"><b>#${e.number}</b></td><td>${G(e.createdAt,!0)}</td><td>${o(e.customer.name)}<small>${o(e.customer.phone)}</small></td><td>${e.items.reduce((t,s)=>t+s.qty,0)}</td><td>${e.fulfillment==="pickup"?"Pickup":"Ship"}</td><td class="tabnum"><b>${P(e.total)}</b></td>
        <td><label class="sr-only" for="os-${e.id}">Status</label><select id="os-${e.id}" class="adm-status" data-order-status="${e.id}">${ae.map(([t,s])=>`<option value="${t}" ${e.status===t?"selected":""}>${s}</option>`).join("")}</select></td>
      ...
      <dialog class="modal adm-order-dlg" id="order-dlg" aria-labelledby="od-title"></dialog>`,"Orders")}function ke(a){let e=m.orders.find(r=>r.id===a);if(!e)return;g("#order-dlg")||(C="orders",X());let t=g("#order-dlg"),s=e.address?`${o(e.address.line1)}${e.address.line2?", "+o(e.address.line2):""}<br>${o(e.address.city)}, ${o(e.address.state)} ${o(e.address.zip)}`:"Local pickup";t.innerHTML=`<div class="modal-head"><h2 id="od-title">Order #${e.number}</h2><button class="icon-btn" type="button" aria-label="Close" data-close-dlg>${h("close")}</button></div>
      <div class="modal-body">
        <dl class="kv"><div><dt>Placed</dt><dd>${G(e.createdAt,!0)}</dd></div><div><dt>Delivery</dt><dd>${e.fulfillment==="pickup"?"Local pickup":"Shipping"}</dd></div><div><dt>Payment</dt><dd>${e.payment==="pickup"?"Pay at pickup":"Send payment link"}</dd></div><div><dt>Total</dt><dd class="tabnum">${P(e.total)}</dd></div></dl>
      ...
      </div>`,t.showModal()}function le(){n.innerHTML=_(m.offers.length?`<div class="table-wrap"><table class="tbl"><thead><tr><th>Date</th><th>Product</th><th>List price</th><th>Offer</th><th>Customer</th><th>Status</th><th></th></tr></thead><tbody>
      ${m.offers.map(a=>{let e=m.products.find(s=>s.id===a.productId),t=a.listPrice?Math.round(a.amount/a.listPrice*100):0;return`<tr>
        <td>${G(a.createdAt,!0)}</td>
        <td><div class="adm-prod">${a.productImage?`<img src="${o(F(a.productImage.sm))}" alt="" width="48" height="48">`:""}<div>${e?`<button class="adm-title" type="button" data-edit="${e.id}">${o(a.productTitle)}</button>`:o(a.productTitle)}<small>${e?`${e.inventory} in stock`:"Deleted"}</small></div></div></td>
        <td class="tabnum">${P(a.listPrice)}</td>
        <td class="tabnum"><b>${P(a.amount)}</b><small>${t}% of list</small></td>
        <td>${o(a.name)}<small><a href="${Y(a.phone)}">${o(a.phone)}</a> \xB7 <a href="sms:${o(a.phone.replace(/[^\d+]/g,""))}">Text</a></small></td>
        <td><label class="sr-only" for="ofs-${a.id}">Status</label><select id="ofs-${a.id}" class="adm-status" data-offer-status="${a.id}">${ye.map(([s,r])=>`<option value="${s}" ${a.status===s?"selected":""}>${r}</option>`).join("")}</select></td>
        <td>${e&&a.status!=="accepted"?`<button class="btn btn-sm" type="button" data-accept-price="${a.id}" title="Set the product price to this offer">Use as price</button>`:""}</td></tr>`}).join("")}
      </tbody></table></div>`:'<div class="empty"><h2>No offers yet</h2><p class="muted">When a customer taps \u201CMake Offer\u201D, their name, phone and amount appear here.</p></div>',"Offers")}function ce(){n.innerHTML=_(m.inquiries.length?`<div class="adm-msgs">${m.inquiries.map(a=>`<article class="adm-card adm-msg ${a.status==="new"?"unread":""}">
      <div class="adm-card-head"><h2>${o(a.name)} <span class="status">${a.type==="wholesale"?"Wholesale":"Contact"}</span></h2><span class="muted" style="font-size:13px">${G(a.createdAt,!0)}</span></div>
      <div class="adm-card-body"><p class="adm-msg-meta"><a href="${Y(a.phone)}">${o(a.phone)}</a> \xB7 <a href="mailto:${o(a.email)}">${o(a.email)}</a>${a.company?" \xB7 "+o(a.company):""}</p><p class="adm-msg-text">${o(a.message)}</p>
      <div class="adm-msg-actions"><label class="sr-only" for="ms-${a.id}">Status</label><select id="ms-${a.id}" class="adm-status" data-msg-status="${a.id}">${Re.map(([e,t])=>`<option value="${e}" ${a.status===e?"selected":""}>${t}</option>`).join("")}</select><button class="btn btn-sm" type="button" data-msg-del="${a.id}">${h("trash","icon-sm")} Delete</button></div></div>
    </article>`).join("")}</div>`:'<div class="empty"><h2>No messages</h2><p class="muted">Contact form and wholesale requests appear here.</p></div>',"Messages")}function K(){let a=p.categories(),e={};m.products.forEach(t=>{e[t.categoryId]=(e[t.categoryId]||0)+1}),n.innerHTML=_(`<div class="adm-card"><div class="adm-card-body">
      <ul class="adm-cats">${a.map((t,s)=>`<li data-cat="${t.id}">
        ...
        <button class="icon-btn" type="button" aria-label="Move down" data-cat-move="1" ${s===a.length-1?"disabled":""}>${h("right","icon-sm")}</button>
    ...
    </div></div>`,"Categories")}function Se(){let a=p.settings(),e=(t,s,r="")=>`<div class="field"><label for="st-${t}">${s}</label><input id="st-${t}" name="${t}" value="${o(a[t]||"")}" ${r}></div>`;n.innerHTML=_(`<div class="adm-cols two">
    ...
    </div>`,"Settings")}let de=async a=>{let e=a.target,t=e.closest("[data-tab]");if(t){C=t.dataset.tab,y=null,c=null,N();return}if(e.closest("[data-logout]")){await d.logout(),W();return}if(e.closest("[data-new]")){C="products",y="new",c=null,N();return}let s=e.closest("[data-edit]");if(s){C="products",y=s.dataset.edit,c=null,N();return}if(e.closest("[data-cancel-edit]")){y=null,c=null,C="products",N();return}let r=e.closest("[data-inv]");if(r){let u=await d.adjustInventory(r.dataset.id,Number(r.dataset.inv));if(E(),u){let f=r.closest("tr");g("b",r.parentElement).textContent=u.inventory,f.classList.toggle("dim",u.inventory<=0||!u.published)}return}let L=e.closest("[data-open-order]");if(L){ke(L.dataset.openOrder);return}if(e.closest("[data-close-dlg]")||e.matches("dialog.modal")){let u=e.closest("dialog");u&&u.close();return}let I=e.closest("[data-cancel-order]");if(I){await d.updateOrder(I.dataset.cancelOrder,{status:"cancelled",restock:!0}),E(),k("Order cancelled and items restocked");let u=e.closest("dialog");u&&u.close(),await O(),X();return}let i=e.closest("[data-accept-price]");if(i){let u=m.offers.find(w=>w.id===i.dataset.acceptPrice),f=m.products.find(w=>w.id===u.productId);f&&(await d.saveProduct({...f,originalPrice:f.originalPrice||f.price,price:u.amount}),await d.updateOffer(u.id,{status:"accepted"}),E(),k(`Price set to ${P(u.amount)} and offer accepted`),await O(),le());return}let x=e.closest("[data-bulk]");if(x){let u=[...M],f=x.dataset.bulk;if(f==="delete"&&!x.dataset.sure){x.dataset.sure="1",x.textContent=`Confirm delete ${u.length}`,x.classList.add("adm-danger");return}for(let w of u){let A=m.products.find(j=>j.id===w);if(A){if(f==="discount"){let j=Math.min(95,Math.max(1,parseInt(g("[data-bulk-pct]").value,10)||0)),z=A.originalPrice||A.price;await d.saveProduct({...A,originalPrice:z,price:R(z*(1-j/100))})}f==="publish"&&await d.patchProduct(w,{published:!0}),f==="unpublish"&&await d.patchProduct(w,{published:!1}),f==="delete"&&await d.deleteProduct(w)}}M.clear(),E(),k("Updated "+u.length+" product"+(u.length>1?"s":"")),await O(),V();return}let l=e.closest("[data-cat-move]");if(l){let u=l.closest("[data-cat]"),f=p.categories().map(j=>j.id),w=f.indexOf(u.dataset.cat),A=w+Number(l.dataset.catMove);[f[w],f[A]]=[f[A],f[w]],await d.reorderCategories(f),E(),K();return}let b=e.closest("[data-cat-save]");if(b){let u=b.closest("[data-cat]"),f=g("[data-cat-name]",u).value.trim();if(!f)return;await d.saveCategory({id:u.dataset.cat,name:f}),E(),k("Category saved"),K();return}let v=e.closest("[data-cat-del]");if(v){if(!v.dataset.sure){v.dataset.sure="1",v.textContent="Confirm";return}await d.deleteCategory(v.closest("[data-cat]").dataset.cat),E(),k("Category deleted"),await O(),K();return}let T=e.closest("[data-msg-del]");if(T){await d.deleteInquiry(T.dataset.msgDel),await O(),ce();return}if(e.closest("[data-export]")){let u=await d.exportData(),f=new Blob([u],{type:"application/json"}),w=document.createElement("a");w.href=URL.createObjectURL(f),w.download=`mlgroup-backup-${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(w),w.click(),w.remove(),k("Backup downloaded");return}if(e.closest("[data-reset]")){g("[data-reset-confirm]").hidden=!1;return}if(e.closest("[data-reset-no]")){g("[data-reset-confirm]").hidden=!0;return}e.closest("[data-reset-yes]")&&(await d.reset(),E(),k("Demo data restored"),N())},ue=async a=>{let e=a.target;if(e.matches("[data-pf-f]")&&(B.f=e.value,V()),e.matches("[data-sel]")){e.checked?M.add(e.dataset.sel):M.delete(e.dataset.sel);let t=g("[data-bulk]");t.hidden=!M.size,g("[data-bulk-n]").textContent=`${M.size} selected`}if(e.matches("[data-sel-all]")){q("[data-sel]").forEach(s=>{s.checked=e.checked,e.checked?M.add(s.dataset.sel):M.delete(s.dataset.sel)});let t=g("[data-bulk]");t.hidden=!M.size,g("[data-bulk-n]").textContent=`${M.size} selected`}if(e.matches("[data-pub]")&&(await d.patchProduct(e.dataset.pub,{published:e.checked}),E(),e.nextElementSibling.textContent=e.checked?"Published":"Hidden",k(e.checked?"Published":"Hidden from store")),e.matches("[data-deal]")&&(await d.patchProduct(e.dataset.deal,{bestDeal:e.checked}),E(),k(e.checked?"Added to Best Deals":"Removed from Best Deals")),e.matches("[data-of]")&&(Q=e.value,X()),e.matches("[data-order-status]")&&(await d.updateOrder(e.dataset.orderStatus,{status:e.value}),k("Order status updated"),await O()),e.matches("[data-offer-status]")&&(await d.updateOffer(e.dataset.offerStatus,{status:e.value}),k("Offer updated"),await O()),e.matches("[data-msg-status]")&&(await d.updateInquiry(e.dataset.msgStatus,{status:e.value}),await O()),e.matches("[data-import]")&&e.files[0])try{await d.importData(await e.files[0].text()),E(),k("Backup imported"),N()}catch(t){k(t.message,"err")}},pe=a=>{if(a.target.matches("[data-pf-q]")){B.q=a.target.value;let e=a.target.selectionStart;V();let t=g("[data-pf-q]");t.focus(),t.setSelectionRange(e,e)}},me=async a=>{let e=a.target;if(e.matches("[data-login]")){a.preventDefault();let t=g('button[type="submit"]',e);t.disabled=!0;try{await d.login(e.elements.pw.value),N()}catch(s){W(s.message||"Sign in failed.")}}if(e.matches("[data-settings]")){a.preventDefault();let t=Object.fromEntries(new FormData(e));t.state=(t.state||"").toUpperCase(),await d.saveSettings(t),E(),k("Store details saved")}if(e.matches("[data-password]")){a.preventDefault();let t=Object.fromEntries(new FormData(e)),s=g("[data-err]",e);if(t.pw1.length<6){s.textContent="Use at least 6 characters.",s.hidden=!1;return}if(t.pw1!==t.pw2){s.textContent="The passwords don\u2019t match.",s.hidden=!1;return}try{await d.changePassword(t.pw1),e.reset(),s.hidden=!0,k("Password changed")}catch(r){s.textContent=r.message,s.hidden=!1}}if(e.matches("[data-cat-add]")){a.preventDefault();let t=e.elements.name.value.trim();if(!t)return;await d.saveCategory({name:t}),E(),k("Category added"),K()}},xe=()=>q("table.tbl",n).forEach(a=>{let e=q("thead th",a).map(t=>t.textContent.trim());q("tbody tr",a).forEach(t=>q("td",t).forEach((s,r)=>{s.hasAttribute("data-label")||s.setAttribute("data-label",e[r]||"")}))}),he=new MutationObserver(xe);return he.observe(n,{childList:!0,subtree:!0}),n.addEventListener("click",de),n.addEventListener("change",ue),n.addEventListener("input",pe),n.addEventListener("submit",me),N(),()=>{he.disconnect(),n.removeEventListener("click",de),n.removeEventListener("change",ue),n.removeEventListener("input",pe),n.removeEventListener("submit",me)}}window.MLAdmin={mount:Ue};})();
