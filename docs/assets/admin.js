...
\u2022 6\u20138 years old
\u2022 10" tall`]],Oe=Date.UTC(2026,8,20),ye=Ie.map((n,p)=>{let[x,S,d,C,w,m,B,P,c,se,Z]=n,T=(ge[x]||[]).map((I,V)=>({sm:`img/p/${x}-${I}-sm.webp`,lg:`img/p/${x}-${I}-lg.webp`,alt:V===0?S:`${S} \u2013 view ${V+1}`}));return{id:"p-"+String(p+1).padStart(3,"0"),slug:x,title:S,brand:d,categoryId:De(C),condition:w,price:m,originalPrice:B,shipping:P,inventory:c,bestDeal:se,published:!0,noShipping:x==="live-bonsai-tree",upc:Ae(p+11),description:Z,images:T,createdAt:new Date(Oe-p*36e5*7).toISOString()}});var nt={products:[],categories:[],settings:{...ve}};function k(n,p=""){let x=b(".toasts");x||(x=document.createElement("div"),x.className="toasts",x.setAttribute("role","status"),x.setAttribute("aria-live","polite"),document.body.appendChild(x));let S=document.createElement("div");S.className="toast "+p,S.innerHTML=`${u(p==="err"?"close":"check","icon-sm")}<span>${i(n)}</span>`,x.appendChild(S),setTimeout(()=>{S.style.opacity="0",S.style.transition="opacity .3s",setTimeout(()=>S.remove(),320)},2600)}var _e=[["overview","Overview","chart"],["products","Products","tag"],["orders","Orders","box"],["offers","Offers","cash"],["requests","Requests","calendar"],["messages","Messages","msg"],["categories","Categories","layers"],["settings","Settings","gear"]],Re=["New","Open Box","Like New","Refurbished","Used \u2013 Good","Used \u2013 Fair","Scratch & Dent"],ae=[["new","New"],["processing","Processing"],["ready","Ready for pickup"],["shipped","Shipped"],["completed","Completed"],["cancelled","Cancelled"]],we=[["new","New"],["contacted","Contacted"],["accepted","Accepted"],["declined","Declined"]],Ue=[["new","New"],["read","Read"],["archived","Archived"]];function je(n,{store:p,navigate:x,query:S}){let d=p.admin,C=S.get("tab")||"overview",w=S.get("edit")||null,m={products:[],orders:[],offers:[],inquiries:[]},B={q:"",f:"all"},P=new Set,c=null,se=()=>{let s=new URLSearchParams;C!=="overview"&&s.set("tab",C),w&&s.set("edit",w);let e="/admin"+(s.toString()?"?"+s:"");D.routing==="path"?history.replaceState({},"",F(e)):D.routing==="hash"&&history.replaceState(null,"","#"+e)},Z=s=>(p.category(s)||{}).name||"\u2014",T=()=>{p.refresh(),window.dispatchEvent(new CustomEvent("cart:change"))};async function I(){let[s,e,t,a]=await Promise.all([d.products(),d.orders(),d.offers(),d.inquiries()]);m={products:s,orders:e,offers:t,inquiries:a}}async function V(s){n.innerHTML=`<div class="adm-login"><form class="adm-login-card" novalidate data-login>
    ...
    </form></div>`,setTimeout(()=>{let e=b("#adm-pw");e&&e.focus()},30)}function _(s,e,t=""){let a=($,N="new")=>m[$].filter(r=>r.status===N).length,o={orders:a("orders"),offers:a("offers"),requests:m.inquiries.filter($=>$.type==="pickup"&&$.status==="new").length,messages:m.inquiries.filter($=>$.type!=="pickup"&&$.status==="new").length};return`<div class="admin">
      <aside class="adm-side on-ink">
        <a class="logo" href="${F("/")}" aria-label="View store">${te()}</a>
        <span class="adm-tag">Admin</span>
        <nav aria-label="Admin"><ul>${_e.map(([$,N,r])=>`<li><button type="button" data-tab="${$}" ${C===$?'aria-current="page"':""}>${u(r)}<span>${N}</span>${o[$]?`<b class="adm-badge">${o[$]}</b>`:""}</button></li>`).join("")}</ul></nav>
        <div class="adm-side-foot"><a href="${F("/")}">${u("eye","icon-sm")} View store</a><button type="button" data-logout>${u("logout","icon-sm")} Sign out</button></div>
      </aside>
    ...
    </div>`}async function q(){if(!await d.session()){V();return}if(await I(),se(),C==="products"&&w)return $e();({overview:re,products:K,orders:X,offers:ne,requests:ie,messages:de,categories:J,settings:xe}[C]||re)()}function re(){let s=m.products,e=s.filter(r=>r.published&&r.inventory>0),t=s.filter(r=>r.inventory<=0),a=s.filter(r=>r.inventory>0&&r.inventory<=2).slice(0,6),o=Date.now()-30*864e5,$=m.orders.filter(r=>r.status!=="cancelled"&&new Date(r.createdAt)>o).reduce((r,M)=>r+M.total,0),N=[["Live listings",e.length,"Published and in stock"],["Units in stock",s.reduce((r,M)=>r+Math.max(0,M.inventory),0),`${t.length} sold out (hidden)`],["New orders",m.orders.filter(r=>r.status==="new").length,`${m.orders.length} total`],["New offers",m.offers.filter(r=>r.status==="new").length,`${m.offers.length} total`],["Sales \xB7 30 days",E($),"Excludes cancelled"],["Messages",m.inquiries.filter(r=>r.status==="new").length,"Contact + wholesale"]];n.innerHTML=_(`
      <div class="adm-tiles">${N.map(([r,M,l])=>`<div class="adm-tile"><span class="eyebrow">${r}</span><b class="tabnum">${M}</b><span class="muted">${l}</span></div>`).join("")}</div>
      <div class="adm-cols">
        <section class="adm-card"><div class="adm-card-head"><h2>Recent orders</h2><button class="link" type="button" data-tab="orders">All orders</button></div>
          ${m.orders.length?`<ul class="adm-list">${m.orders.slice(0,6).map(r=>`<li><button type="button" data-open-order="${r.id}"><span><b class="mono">#${r.number}</b> ${i(r.customer.name)}<small>${j(r.createdAt,!0)} \xB7 ${r.items.length} item${r.items.length>1?"s":""} \xB7 ${r.fulfillment==="pickup"?"Pickup":"Ship"}</small></span><span class="tabnum">${E(r.total)}</span><span class="status s-${r.status}">${le(ae,r.status)}</span></button></li>`).join("")}</ul>`:'<p class="muted adm-empty">No orders yet. They appear here as soon as a customer checks out.</p>'}
        </section>
        <section class="adm-card"><div class="adm-card-head"><h2>Latest offers</h2><button class="link" type="button" data-tab="offers">All offers</button></div>
          ${m.offers.length?`<ul class="adm-list">${m.offers.slice(0,6).map(r=>`<li><button type="button" data-tab="offers"><span><b>${E(r.amount)}</b> for ${i(r.productTitle)}<small>${i(r.name)} \xB7 ${i(r.phone)} \xB7 ${j(r.createdAt,!0)}</small></span><span class="status s-${r.status}">${le(we,r.status)}</span></button></li>`).join("")}</ul>`:'<p class="muted adm-empty">No offers yet. Offers from the \u201CMake Offer\u201D button land here.</p>'}
        </section>
      ...
      </div>`,"Overview",`<button class="btn btn-primary" type="button" data-new>${u("plus","icon-sm")} Add product</button>`)}let le=(s,e)=>(s.find(([t])=>t===e)||[e,e])[1];function K(){let s=B.q.toLowerCase(),e=m.products.filter(a=>!s||`${a.title} ${a.brand||""} ${a.upc||""} ${a.slug}`.toLowerCase().includes(s)),t=B.f;t==="live"&&(e=e.filter(a=>a.published&&a.inventory>0)),t==="hidden"&&(e=e.filter(a=>!a.published)),t==="out"&&(e=e.filter(a=>a.inventory<=0)),P=new Set([...P].filter(a=>m.products.some(o=>o.id===a))),n.innerHTML=_(`
      <div class="adm-toolbar">
        <div class="search-field adm-search">${u("search")}<label class="sr-only" for="pf-q">Search products</label><input id="pf-q" type="search" placeholder="Search title, brand, UPC" value="${i(B.q)}" data-pf-q></div>
        <div class="select"><label class="sr-only" for="pf-f">Show</label><select id="pf-f" data-pf-f>${[["all","All products"],["live","Live"],["hidden","Unpublished"],["out","Sold out"]].map(([a,o])=>`<option value="${a}" ${t===a?"selected":""}>${o}</option>`).join("")}</select>${u("down")}</div>
      ...
      <div class="table-wrap"><table class="tbl adm-table"><thead><tr>
        ...
        ${e.map(a=>`<tr data-row="${a.id}" class="${a.inventory<=0||!a.published?"dim":""}">
          <td><input type="checkbox" aria-label="Select ${i(a.title)}" data-sel="${a.id}" ${P.has(a.id)?"checked":""}></td>
          ...
          <td class="tabnum"><b>${E(a.price)}</b>${ee(a)?`<small><s>${E(a.originalPrice)}</s> \xB7 -${ee(a)}%</small>`:""}</td>
          ...
          <td class="adm-row-actions"><button class="btn btn-sm" type="button" data-edit="${a.id}">${u("edit","icon-sm")} Edit</button>${a.published&&a.inventory>0?`<a class="btn btn-sm" href="${F("/products/"+a.slug)}" target="_blank" rel="noopener" aria-label="View ${i(a.title)} on the store">${u("eye","icon-sm")}</a>`:""}</td>
        </tr>`).join("")||'<tr><td colspan="6" class="muted">No products match.</td></tr>'}
      </tbody></table></div>`,"Products",`<button class="btn btn-primary" type="button" data-new>${u("plus","icon-sm")} Add product</button>`)}function $e(){let s=w==="new",e=s?null:m.products.find(o=>o.id===w);if(!s&&!e)return w=null,K();(!c||c._for!==w)&&(c=e?JSON.parse(JSON.stringify(e)):{title:"",slug:"",brand:"",upc:"",condition:"New",categoryId:(p.categories()[0]||{}).id||null,description:"",price:"",originalPrice:"",shipping:9.99,inventory:1,published:!0,bestDeal:!1,images:[]},c._for=w);let t=c,a=t.originalPrice&&t.price&&Number(t.originalPrice)>Number(t.price)?Math.round((1-t.price/t.originalPrice)*100):"";n.innerHTML=_(`
      <form class="adm-editor" novalidate data-editor>
        <div class="adm-ed-main">
          ...
          <section class="adm-card"><div class="adm-card-head"><h2>Photos</h2><span class="muted" style="font-size:13px">First photo is the cover. Drag to reorder.</span></div><div class="adm-card-body">
            <div class="adm-photos" data-photos>${ce(t)}</div>
        ...
        <div class="adm-ed-side">
          ...
          <section class="adm-card"><div class="adm-card-head"><h2>Inventory</h2></div><div class="adm-card-body form">
            <div class="field"><label for="ed-inv">Quantity in stock</label><div class="qty" style="height:46px"><button type="button" aria-label="Decrease" data-ed-inv="-1">${u("minus","icon-sm")}</button><input id="ed-inv" name="inventory" type="number" inputmode="numeric" min="0" step="1" value="${i(t.inventory)}" style="width:80px"><button type="button" aria-label="Increase" data-ed-inv="1">${u("plus","icon-sm")}</button></div></div>
          </div></section>
          <section class="adm-card"><div class="adm-card-head"><h2>Organize</h2></div><div class="adm-card-body form">
            <div class="field"><label for="ed-cat">Category</label><select id="ed-cat" name="categoryId">${p.categories().map(o=>`<option value="${o.id}" ${t.categoryId===o.id?"selected":""}>${i(o.name)}</option>`).join("")}<option value="" ${t.categoryId?"":"selected"}>Uncategorized</option></select></div>
      ...
      </form>`,s?"New product":"Edit product",`<button class="btn" type="button" data-cancel-edit>${u("left","icon-sm")} All products</button>`),ke()}function ce(s){return s.images.length?s.images.map((e,t)=>`<figure class="adm-photo" draggable="true" data-ph="${t}">
      ...
      </figcaption></figure>`).join(""):'<p class="muted" style="font-size:14px">No photos yet.</p>'}function ke(){let s=b("[data-editor]"),e=l=>s.elements[l],t=()=>{b("[data-photos]").innerHTML=ce(c)},a=()=>{["title","description","brand","upc","slug","condition","categoryId"].forEach(l=>{c[l]=e(l).value}),c.price=e("price").value,c.originalPrice=e("originalPrice").value,c.shipping=e("shipping").value,c.inventory=e("inventory").value,c.published=e("published").checked,c.noShipping=e("noShipping").checked};s.addEventListener("input",l=>{let f=l.target.name,y=parseFloat(e("originalPrice").value),L=parseFloat(e("pct").value),A=parseFloat(e("price").value);f==="pct"&&y>0&&L>=0&&(e("price").value=z(y*(1-L/100)).toFixed(2)),f==="price"&&y>0&&A>=0&&(e("pct").value=A<y?Math.round((1-A/y)*100):""),f==="originalPrice"&&y>0&&L>0&&(e("price").value=z(y*(1-L/100)).toFixed(2)),(f==="title"||f==="slug")&&(b("[data-slug-preview]").textContent=Y(e("slug").value||e("title").value)||"product-name"),a()}),s.addEventListener("change",a),s.addEventListener("click",async l=>{let f=l.target.closest("[data-ed-inv]");f&&(e("inventory").value=Math.max(0,(parseInt(e("inventory").value,10)||0)+Number(f.dataset.edInv)),a());let y=l.target.closest("[data-ph-move]");if(y){let A=Number(y.closest("[data-ph]").dataset.ph),h=A+Number(y.dataset.phMove);[c.images[A],c.images[h]]=[c.images[h],c.images[A]],t()}let L=l.target.closest("[data-ph-del]");if(L){let A=Number(L.closest("[data-ph]").dataset.ph),[h]=c.images.splice(A,1);d.removeImage&&d.removeImage(h).catch(()=>{}),t()}l.target.closest("[data-delete-product]")&&(b("[data-confirm]").hidden=!1),l.target.closest("[data-confirm-cancel]")&&(b("[data-confirm]").hidden=!0),l.target.closest("[data-confirm-delete]")&&(await d.deleteProduct(w),T(),k("Product deleted"),w=null,c=null,q())});let o=null,$=b("[data-photos]");$.addEventListener("dragstart",l=>{let f=l.target.closest("[data-ph]");f&&(o=Number(f.dataset.ph),l.dataTransfer.effectAllowed="move",f.classList.add("dragging"))}),$.addEventListener("dragover",l=>{o!==null&&l.preventDefault()}),$.addEventListener("drop",l=>{let f=l.target.closest("[data-ph]");if(o===null||!f)return;l.preventDefault();let y=Number(f.dataset.ph),[L]=c.images.splice(o,1);c.images.splice(y,0,L),o=null,t()}),$.addEventListener("dragend",()=>{o=null,U(".dragging").forEach(l=>l.classList.remove("dragging"))});let N=b("[data-upload-status]"),r=async l=>{if(l=Array.from(l).filter(f=>f.type.startsWith("image/")),!!l.length){for(let f=0;f<l.length;f++){N.textContent=`Uploading ${f+1} of ${l.length}\u2026`;try{let y=await d.uploadImage(l[f]);c.images.push({...y,alt:""}),t()}catch(y){k(`Couldn\u2019t upload ${l[f].name}: ${y.message}`,"err")}}N.textContent=`${l.length} photo${l.length>1?"s":""} added \u2014 remember to save.`}};b("[data-files]").addEventListener("change",l=>{r(l.target.files),l.target.value=""});let M=b("[data-drop]");M.addEventListener("dragover",l=>{l.dataTransfer.types.includes("Files")&&(l.preventDefault(),M.classList.add("over"))}),M.addEventListener("dragleave",()=>M.classList.remove("over")),M.addEventListener("drop",l=>{l.dataTransfer.files.length&&(l.preventDefault(),M.classList.remove("over"),r(l.dataTransfer.files))}),s.addEventListener("submit",async l=>{l.preventDefault(),a();let f=b("[data-err]"),y=parseFloat(c.price),L=parseFloat(c.originalPrice),A=parseFloat(c.shipping||0),h=parseInt(c.inventory,10),g=String(c.upc||"").replace(/\D/g,""),v=[];if(c.title.trim()||v.push(["ed-title","Add a title."]),y>0||v.push(["ed-price","Enter a sale price greater than $0."]),h>=0||v.push(["ed-inv","Inventory must be 0 or more."]),A>=0||v.push(["ed-ship","Shipping must be 0 or more."]),g&&(g.length<8||g.length>14)&&v.push(["ed-upc","UPC should be 8\u201314 digits."]),U("[aria-invalid]",s).forEach(O=>O.removeAttribute("aria-invalid")),v.length){v.forEach(([O])=>b("#"+O).setAttribute("aria-invalid","true")),f.textContent=v.map(O=>O[1]).join(" "),f.hidden=!1,b("#"+v[0][0]).focus();return}let R={id:w==="new"?void 0:w,title:c.title.trim(),slug:c.slug.trim(),brand:c.brand.trim(),upc:g,condition:c.condition,categoryId:c.categoryId||null,description:c.description.trim(),price:z(y),originalPrice:L>y?z(L):null,shipping:z(A),inventory:h,published:c.published,bestDeal:!1,noShipping:!!c.noShipping,images:c.images},H=b('button[type="submit"]',s);H.disabled=!0,H.textContent="Saving\u2026";try{let O=await d.saveProduct(R);return T(),k(w==="new"?"Product created":"Changes saved"),w=null,c=null,C="products",q(),O}catch(O){f.textContent=O.message,f.hidden=!1,H.disabled=!1,H.textContent="Save changes"}})}let Q="all";function X(){let s=m.orders.filter(e=>Q==="all"||e.status===Q);n.innerHTML=_(`
      <div class="adm-toolbar"><div class="select"><label class="sr-only" for="of-f">Status</label><select id="of-f" data-of>${[["all","All orders"],...ae].map(([e,t])=>`<option value="${e}" ${Q===e?"selected":""}>${t}</option>`).join("")}</select>${u("down")}</div><span class="muted" style="font-size:14px">${s.length} order${s.length===1?"":"s"}</span></div>
      ...
      ${s.map(e=>`<tr><td class="mono"><b>#${e.number}</b></td><td>${j(e.createdAt,!0)}</td><td>${i(e.customer.name)}<small>${i(e.customer.phone)}</small></td><td>${e.items.reduce((t,a)=>t+a.qty,0)}</td><td>${e.fulfillment==="pickup"?"Pickup":"Ship"}</td><td class="tabnum"><b>${E(e.total)}</b></td>
      ...
      <dialog class="modal adm-order-dlg" id="order-dlg" aria-labelledby="od-title"></dialog>`,"Orders")}function Se(s){let e=m.orders.find(o=>o.id===s);if(!e)return;b("#order-dlg")||(C="orders",X());let t=b("#order-dlg"),a=e.address?`${i(e.address.line1)}${e.address.line2?", "+i(e.address.line2):""}<br>${i(e.address.city)}, ${i(e.address.state)} ${i(e.address.zip)}`:"Local pickup";t.innerHTML=`<div class="modal-head"><h2 id="od-title">Order #${e.number}</h2><button class="icon-btn" type="button" aria-label="Close" data-close-dlg>${u("close")}</button></div>
      <div class="modal-body">
        <dl class="kv"><div><dt>Placed</dt><dd>${j(e.createdAt,!0)}</dd></div><div><dt>Delivery</dt><dd>${e.fulfillment==="pickup"?"Local pickup":"Shipping"}</dd></div><div><dt>Payment</dt><dd>${e.payment==="paypal"?e.paidAt?`<b>PAID</b> \xB7 PayPal${e.paypalCaptureId?`<small class="mono">${i(e.paypalCaptureId)}</small>`:""}`:"PayPal \u2014 not paid":e.payment==="pickup"?"Pay at pickup":"Send payment link"}</dd></div><div><dt>Total</dt><dd class="tabnum">${E(e.total)}</dd></div></dl>
        ...
        <ul class="sum-items" style="max-height:none;border:1px solid var(--line)">${e.items.map(o=>`<li class="sum-item"><span class="th">${oe(o.image,{alt:"",sizes:"56px"})}<span class="q">${o.qty}</span></span><span class="t">${i(o.title)}<small>${o.qty} \xD7 ${E(o.price)}${o.shipping?" \xB7 ship "+E(o.shipping):""}</small></span><span class="p tabnum">${E(o.price*o.qty)}</span></li>`).join("")}</ul>
        <div class="sum-totals" style="padding:0"><div class="row"><span>Subtotal</span><span class="tabnum">${E(e.subtotal)}</span></div><div class="row"><span>Shipping</span><span class="tabnum">${E(e.shippingTotal)}</span></div><div class="row total"><span>Total</span><span class="tabnum">${E(e.total)}</span></div></div>
        <div class="field"><label for="od-status">Status</label><select id="od-status" data-order-status="${e.id}">${ae.map(([o,$])=>`<option value="${o}" ${e.status===o?"selected":""}>${$}</option>`).join("")}</select></div>
      ...
      </div>`,t.showModal()}function ne(){n.innerHTML=_(m.offers.length?`<div class="table-wrap"><table class="tbl"><thead><tr><th>Date</th><th>Product</th><th>List price</th><th>Offer</th><th>Customer</th><th>Status</th><th></th></tr></thead><tbody>
      ${m.offers.map(s=>{let e=m.products.find(a=>a.id===s.productId),t=s.listPrice?Math.round(s.amount/s.listPrice*100):0;return`<tr>
        <td>${j(s.createdAt,!0)}</td>
        ...
        <td class="tabnum">${E(s.listPrice)}</td>
        <td class="tabnum"><b>${E(s.amount)}</b><small>${t}% of list</small></td>
      ...
      </tbody></table></div>`:'<div class="empty"><h2>No offers yet</h2><p class="muted">When a customer taps \u201CMake Offer\u201D, their name, phone and amount appear here.</p></div>',"Offers")}function ie(){let s=m.inquiries.filter(t=>t.type==="pickup"),e=t=>{let a=((t.message||"").match(/Pickup request:\s*(.+)/)||[])[1]||"Pickup request",o=((t.message||"").match(/When:\s*(.+)/)||[])[1]||"";return{item:a.trim(),when:o.trim()}};n.innerHTML=_(s.length?`<div class="adm-reqs">${s.map(t=>{let a=e(t),o=String(t.phone||"").replace(/[^\d+]/g,"");return`<details class="adm-req ${t.status==="new"?"unread":""}">
      <summary><span class="adm-req-title"><b>${i(a.item)}</b><small>${u("calendar","icon-sm")} ${i(a.when||j(t.createdAt,!0))}</small></span>${t.status==="new"?'<span class="status s-new">New</span>':`<span class="status s-${t.status}">${t.status==="read"?"Confirmed":"Done"}</span>`}${u("down")}</summary>
      <div class="adm-req-body">
        <dl class="kv"><div><dt>Name</dt><dd>${i(t.name)}</dd></div><div><dt>Phone</dt><dd><a href="${W(t.phone)}">${i(t.phone)}</a></dd></div><div><dt>Pickup time</dt><dd>${i(a.when||"\u2014")}</dd></div><div><dt>Requested</dt><dd>${j(t.createdAt,!0)}</dd></div></dl>
        <div class="adm-req-actions">
          <a class="btn btn-primary" href="sms:${i(o)}">${u("msg","icon-sm")} Text</a>
          <a class="btn" href="${W(t.phone)}">${u("phone","icon-sm")} Call</a>
      ...
      </div></details>`}).join("")}</div>`:'<div class="empty"><h2>No pickup requests yet</h2><p class="muted">When a customer taps \u201CChoose a date &amp; time\u201D on a product, their request shows up here.</p></div>',"Requests")}function de(){let s=m.inquiries.filter(e=>e.type!=="pickup");n.innerHTML=_(s.length?`<div class="adm-msgs">${s.map(e=>`<article class="adm-card adm-msg ${e.status==="new"?"unread":""}">
      <div class="adm-card-head"><h2>${i(e.name)} <span class="status">${e.type==="wholesale"?"Wholesale":e.type==="pickup"?"Pickup request":"Contact"}</span></h2><span class="muted" style="font-size:13px">${j(e.createdAt,!0)}</span></div>
      ...
      <div class="adm-msg-actions"><label class="sr-only" for="ms-${e.id}">Status</label><select id="ms-${e.id}" class="adm-status" data-msg-status="${e.id}">${Ue.map(([t,a])=>`<option value="${t}" ${e.status===t?"selected":""}>${a}</option>`).join("")}</select><button class="btn btn-sm" type="button" data-msg-del="${e.id}">${u("trash","icon-sm")} Delete</button></div></div>
    </article>`).join("")}</div>`:'<div class="empty"><h2>No messages</h2><p class="muted">Contact form and wholesale requests appear here.</p></div>',"Messages")}function J(){let s=p.categories(),e={};m.products.forEach(t=>{e[t.categoryId]=(e[t.categoryId]||0)+1}),n.innerHTML=_(`<div class="adm-card"><div class="adm-card-body">
    ...
    </div></div>`,"Categories")}function xe(){let s=p.settings(),e=(t,a,o="")=>`<div class="field"><label for="st-${t}">${a}</label><input id="st-${t}" name="${t}" value="${i(s[t]||"")}" ${o}></div>`;n.innerHTML=_(`<div class="adm-cols two">
    ...
    </div>`,"Settings")}let ue=async s=>{let e=s.target,t=e.closest("[data-tab]");if(t){C=t.dataset.tab,w=null,c=null,q();return}if(e.closest("[data-logout]")){await d.logout(),V();return}if(e.closest("[data-new]")){C="products",w="new",c=null,q();return}let a=e.closest("[data-edit]");if(a){C="products",w=a.dataset.edit,c=null,q();return}if(e.closest("[data-cancel-edit]")){w=null,c=null,C="products",q();return}let o=e.closest("[data-inv]");if(o){let h=await d.adjustInventory(o.dataset.id,Number(o.dataset.inv));if(T(),h){let g=o.closest("tr");b("b",o.parentElement).textContent=h.inventory,g.classList.toggle("dim",h.inventory<=0||!h.published)}return}let $=e.closest("[data-open-order]");if($){Se($.dataset.openOrder);return}if(e.closest("[data-close-dlg]")||e.matches("dialog.modal")){let h=e.closest("dialog");h&&h.close();return}let N=e.closest("[data-cancel-order]");if(N){await d.updateOrder(N.dataset.cancelOrder,{status:"cancelled",restock:!0}),T(),k("Order cancelled and items restocked");let h=e.closest("dialog");h&&h.close(),await I(),X();return}let r=e.closest("[data-accept-price]");if(r){let h=m.offers.find(v=>v.id===r.dataset.acceptPrice),g=m.products.find(v=>v.id===h.productId);g&&(await d.saveProduct({...g,originalPrice:g.originalPrice||g.price,price:h.amount}),await d.updateOffer(h.id,{status:"accepted"}),T(),k(`Price set to ${E(h.amount)} and offer accepted`),await I(),ne());return}let M=e.closest("[data-bulk]");if(M){let h=[...P],g=M.dataset.bulk;if(g==="delete"&&!M.dataset.sure){M.dataset.sure="1",M.textContent=`Confirm delete ${h.length}`,M.classList.add("adm-danger");return}for(let v of h){let R=m.products.find(H=>H.id===v);if(R){if(g==="discount"){let H=Math.min(95,Math.max(1,parseInt(b("[data-bulk-pct]").value,10)||0)),O=R.originalPrice||R.price;await d.saveProduct({...R,originalPrice:O,price:z(O*(1-H/100))})}g==="publish"&&await d.patchProduct(v,{published:!0}),g==="unpublish"&&await d.patchProduct(v,{published:!1}),g==="delete"&&await d.deleteProduct(v)}}P.clear(),T(),k("Updated "+h.length+" product"+(h.length>1?"s":"")),await I(),K();return}let l=e.closest("[data-cat-move]");if(l){let h=l.closest("[data-cat]"),g=p.categories().map(H=>H.id),v=g.indexOf(h.dataset.cat),R=v+Number(l.dataset.catMove);[g[v],g[R]]=[g[R],g[v]],await d.reorderCategories(g),T(),J();return}let f=e.closest("[data-cat-save]");if(f){let h=f.closest("[data-cat]"),g=b("[data-cat-name]",h).value.trim();if(!g)return;await d.saveCategory({id:h.dataset.cat,name:g}),T(),k("Category saved"),J();return}let y=e.closest("[data-cat-del]");if(y){if(!y.dataset.sure){y.dataset.sure="1",y.textContent="Confirm";return}await d.deleteCategory(y.closest("[data-cat]").dataset.cat),T(),k("Category deleted"),await I(),J();return}let L=e.closest("[data-offer-del]");if(L){if(!L.dataset.sure){L.dataset.sure="1",L.textContent="Tap again to delete";return}await d.deleteOffer(L.dataset.offerDel),await I(),ne(),k("Offer deleted");return}let A=e.closest("[data-msg-del]");if(A){await d.deleteInquiry(A.dataset.msgDel),await I(),(A.dataset.back==="requests"?ie:de)(),k("Deleted");return}if(e.closest("[data-export]")){let h=await d.exportData(),g=new Blob([h],{type:"application/json"}),v=document.createElement("a");v.href=URL.createObjectURL(g),v.download=`mlgroup-backup-${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(v),v.click(),v.remove(),k("Backup downloaded");return}if(e.closest("[data-reset]")){b("[data-reset-confirm]").hidden=!1;return}if(e.closest("[data-reset-no]")){b("[data-reset-confirm]").hidden=!0;return}e.closest("[data-reset-yes]")&&(await d.reset(),T(),k("Demo data restored"),q())},pe=async s=>{let e=s.target;if(e.matches("[data-pf-f]")&&(B.f=e.value,K()),e.matches("[data-sel]")){e.checked?P.add(e.dataset.sel):P.delete(e.dataset.sel);let t=b("[data-bulk]");t.hidden=!P.size,b("[data-bulk-n]").textContent=`${P.size} selected`}if(e.matches("[data-sel-all]")){U("[data-sel]").forEach(a=>{a.checked=e.checked,e.checked?P.add(a.dataset.sel):P.delete(a.dataset.sel)});let t=b("[data-bulk]");t.hidden=!P.size,b("[data-bulk-n]").textContent=`${P.size} selected`}if(e.matches("[data-pub]")&&(await d.patchProduct(e.dataset.pub,{published:e.checked}),T(),e.nextElementSibling.textContent=e.checked?"Published":"Hidden",k(e.checked?"Published":"Hidden from store")),e.matches("[data-of]")&&(Q=e.value,X()),e.matches("[data-order-status]")&&(await d.updateOrder(e.dataset.orderStatus,{status:e.value}),k("Order status updated"),await I()),e.matches("[data-offer-status]")&&(await d.updateOffer(e.dataset.offerStatus,{status:e.value}),k("Offer updated"),await I()),e.matches("[data-msg-status]")&&(await d.updateInquiry(e.dataset.msgStatus,{status:e.value}),await I(),k("Updated"),C==="requests"&&ie()),e.matches("[data-import]")&&e.files[0])try{await d.importData(await e.files[0].text()),T(),k("Backup imported"),q()}catch(t){k(t.message,"err")}},me=s=>{if(s.target.matches("[data-pf-q]")){B.q=s.target.value;let e=s.target.selectionStart;K();let t=b("[data-pf-q]");t.focus(),t.setSelectionRange(e,e)}},he=async s=>{let e=s.target;if(e.matches("[data-login]")){s.preventDefault();let t=b('button[type="submit"]',e);t.disabled=!0;try{await d.login(e.elements.pw.value),q()}catch(a){V(a.message||"Sign in failed.")}}if(e.matches("[data-settings]")){s.preventDefault();let t=Object.fromEntries(new FormData(e));t.state=(t.state||"").toUpperCase(),await d.saveSettings(t),T(),k("Store details saved")}if(e.matches("[data-password]")){s.preventDefault();let t=Object.fromEntries(new FormData(e)),a=b("[data-err]",e);if(t.pw1.length<6){a.textContent="Use at least 6 characters.",a.hidden=!1;return}if(t.pw1!==t.pw2){a.textContent="The passwords don\u2019t match.",a.hidden=!1;return}try{await d.changePassword(t.pw1),e.reset(),a.hidden=!0,k("Password changed")}catch(o){a.textContent=o.message,a.hidden=!1}}if(e.matches("[data-cat-add]")){s.preventDefault();let t=e.elements.name.value.trim();if(!t)return;await d.saveCategory({name:t}),T(),k("Category added"),J()}},Ce=()=>U("table.tbl",n).forEach(s=>{let e=U("thead th",s).map(t=>t.textContent.trim());U("tbody tr",s).forEach(t=>U("td",t).forEach((a,o)=>{a.hasAttribute("data-label")||a.setAttribute("data-label",e[o]||"")}))}),fe=new MutationObserver(Ce);return fe.observe(n,{childList:!0,subtree:!0}),n.addEventListener("click",ue),n.addEventListener("change",pe),n.addEventListener("input",me),n.addEventListener("submit",he),q(),()=>{fe.disconnect(),n.removeEventListener("click",ue),n.removeEventListener("change",pe),n.removeEventListener("input",me),n.removeEventListener("submit",he)}}window.MLAdmin={mount:je};})();
