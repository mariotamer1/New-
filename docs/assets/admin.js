...
\u2022 6\u20138 years old
\u2022 10" tall`]],Ie=Date.UTC(2026,8,20),ye=Oe.map((n,p)=>{let[x,S,d,M,w,m,B,L,c,se,Z]=n,T=(ge[x]||[]).map((O,V)=>({sm:`img/p/${x}-${O}-sm.webp`,lg:`img/p/${x}-${O}-lg.webp`,alt:V===0?S:`${S} \u2013 view ${V+1}`}));return{id:"p-"+String(p+1).padStart(3,"0"),slug:x,title:S,brand:d,categoryId:De(M),condition:w,price:m,originalPrice:B,shipping:L,inventory:c,bestDeal:se,published:!0,noShipping:x==="live-bonsai-tree",upc:Ae(p+11),description:Z,images:T,createdAt:new Date(Ie-p*36e5*7).toISOString()}});var nt={products:[],categories:[],settings:{...ve}};function k(n,p=""){let x=b(".toasts");x||(x=document.createElement("div"),x.className="toasts",x.setAttribute("role","status"),x.setAttribute("aria-live","polite"),document.body.appendChild(x));let S=document.createElement("div");S.className="toast "+p,S.innerHTML=`${u(p==="err"?"close":"check","icon-sm")}<span>${i(n)}</span>`,x.appendChild(S),setTimeout(()=>{S.style.opacity="0",S.style.transition="opacity .3s",setTimeout(()=>S.remove(),320)},2600)}var _e=[["overview","Overview","chart"],["products","Products","tag"],["orders","Orders","box"],["offers","Offers","cash"],["requests","Requests","calendar"],["messages","Messages","msg"],["categories","Categories","layers"],["settings","Settings","gear"]],Re=["New","Open Box","Like New","Refurbished","Used \u2013 Good","Used \u2013 Fair","Scratch & Dent"],ae=[["new","New"],["processing","Processing"],["ready","Ready for pickup"],["shipped","Shipped"],["completed","Completed"],["cancelled","Cancelled"]],we=[["new","New"],["contacted","Contacted"],["accepted","Accepted"],["declined","Declined"]],Ue=[["new","New"],["read","Read"],["archived","Archived"]];function je(n,{store:p,navigate:x,query:S}){let d=p.admin,M=S.get("tab")||"overview",w=S.get("edit")||null,m={products:[],orders:[],offers:[],inquiries:[]},B={q:"",f:"all"},L=new Set,c=null,se=()=>{let s=new URLSearchParams;M!=="overview"&&s.set("tab",M),w&&s.set("edit",w);let e="/admin"+(s.toString()?"?"+s:"");D.routing==="path"?history.replaceState({},"",F(e)):D.routing==="hash"&&history.replaceState(null,"","#"+e)},Z=s=>(p.category(s)||{}).name||"\u2014",T=()=>{p.refresh(),window.dispatchEvent(new CustomEvent("cart:change"))};async function O(){let[s,e,t,a]=await Promise.all([d.products(),d.orders(),d.offers(),d.inquiries()]);m={products:s,orders:e,offers:t,inquiries:a}}async function V(s){n.innerHTML=`<div class="adm-login"><form class="adm-login-card" novalidate data-login>
      ...
      <aside class="adm-side on-ink">
        ...
        <nav aria-label="Admin"><ul>${_e.map(([$,N,r])=>`<li><button type="button" data-tab="${$}" ${M===$?'aria-current="page"':""}>${u(r)}<span>${N}</span>${o[$]?`<b class="adm-badge">${o[$]}</b>`:""}</button></li>`).join("")}</ul></nav>
    ...
    </div>`}async function q(){if(!await d.session()){V();return}if(await O(),se(),M==="products"&&w)return $e();({overview:re,products:K,orders:X,offers:ne,requests:ie,messages:de,categories:J,settings:xe}[M]||re)()}function re(){let s=m.products,e=s.filter(r=>r.published&&r.inventory>0),t=s.filter(r=>r.inventory<=0),a=s.filter(r=>r.inventory>0&&r.inventory<=2).slice(0,6),o=Date.now()-30*864e5,$=m.orders.filter(r=>r.status!=="cancelled"&&new Date(r.createdAt)>o).reduce((r,C)=>r+C.total,0),N=[["Live listings",e.length,"Published and in stock"],["Units in stock",s.reduce((r,C)=>r+Math.max(0,C.inventory),0),`${t.length} sold out (hidden)`],["New orders",m.orders.filter(r=>r.status==="new").length,`${m.orders.length} total`],["New offers",m.offers.filter(r=>r.status==="new").length,`${m.offers.length} total`],["Sales \xB7 30 days",E($),"Excludes cancelled"],["Messages",m.inquiries.filter(r=>r.status==="new").length,"Contact + wholesale"]];n.innerHTML=_(`
      <div class="adm-tiles">${N.map(([r,C,l])=>`<div class="adm-tile"><span class="eyebrow">${r}</span><b class="tabnum">${C}</b><span class="muted">${l}</span></div>`).join("")}</div>
      <div class="adm-cols">
      ...
      </figcaption></figure>`).join(""):'<p class="muted" style="font-size:14px">No photos yet.</p>'}function ke(){let s=b("[data-editor]"),e=l=>s.elements[l],t=()=>{b("[data-photos]").innerHTML=ce(c)},a=()=>{["title","description","brand","upc","slug","condition","categoryId"].forEach(l=>{c[l]=e(l).value}),c.price=e("price").value,c.originalPrice=e("originalPrice").value,c.shipping=e("shipping").value,c.inventory=e("inventory").value,c.published=e("published").checked,c.noShipping=e("noShipping").checked};s.addEventListener("input",l=>{let f=l.target.name,y=parseFloat(e("originalPrice").value),P=parseFloat(e("pct").value),A=parseFloat(e("price").value);f==="pct"&&y>0&&P>=0&&(e("price").value=z(y*(1-P/100)).toFixed(2)),f==="price"&&y>0&&A>=0&&(e("pct").value=A<y?Math.round((1-A/y)*100):""),f==="originalPrice"&&y>0&&P>0&&(e("price").value=z(y*(1-P/100)).toFixed(2)),(f==="title"||f==="slug")&&(b("[data-slug-preview]").textContent=Y(e("slug").value||e("title").value)||"product-name"),a()}),s.addEventListener("change",a),s.addEventListener("click",async l=>{let f=l.target.closest("[data-ed-inv]");f&&(e("inventory").value=Math.max(0,(parseInt(e("inventory").value,10)||0)+Number(f.dataset.edInv)),a());let y=l.target.closest("[data-ph-move]");if(y){let A=Number(y.closest("[data-ph]").dataset.ph),h=A+Number(y.dataset.phMove);[c.images[A],c.images[h]]=[c.images[h],c.images[A]],t()}let P=l.target.closest("[data-ph-del]");if(P){let A=Number(P.closest("[data-ph]").dataset.ph),[h]=c.images.splice(A,1);d.removeImage&&d.removeImage(h).catch(()=>{}),t()}l.target.closest("[data-delete-product]")&&(b("[data-confirm]").hidden=!1),l.target.closest("[data-confirm-cancel]")&&(b("[data-confirm]").hidden=!0),l.target.closest("[data-confirm-delete]")&&(await d.deleteProduct(w),T(),k("Product deleted"),w=null,c=null,q())});let o=null,$=b("[data-photos]");$.addEventListener("dragstart",l=>{let f=l.target.closest("[data-ph]");f&&(o=Number(f.dataset.ph),l.dataTransfer.effectAllowed="move",f.classList.add("dragging"))}),$.addEventListener("dragover",l=>{o!==null&&l.preventDefault()}),$.addEventListener("drop",l=>{let f=l.target.closest("[data-ph]");if(o===null||!f)return;l.preventDefault();let y=Number(f.dataset.ph),[P]=c.images.splice(o,1);c.images.splice(y,0,P),o=null,t()}),$.addEventListener("dragend",()=>{o=null,U(".dragging").forEach(l=>l.classList.remove("dragging"))});let N=b("[data-upload-status]"),r=async l=>{if(l=Array.from(l).filter(f=>f.type.startsWith("image/")),!!l.length){for(let f=0;f<l.length;f++){N.textContent=`Uploading ${f+1} of ${l.length}\u2026`;try{let y=await d.uploadImage(l[f]);c.images.push({...y,alt:""}),t()}catch(y){k(`Couldn\u2019t upload ${l[f].name}: ${y.message}`,"err")}}N.textContent=`${l.length} photo${l.length>1?"s":""} added \u2014 remember to save.`}};b("[data-files]").addEventListener("change",l=>{r(l.target.files),l.target.value=""});let C=b("[data-drop]");C.addEventListener("dragover",l=>{l.dataTransfer.types.includes("Files")&&(l.preventDefault(),C.classList.add("over"))}),C.addEventListener("dragleave",()=>C.classList.remove("over")),C.addEventListener("drop",l=>{l.dataTransfer.files.length&&(l.preventDefault(),C.classList.remove("over"),r(l.dataTransfer.files))}),s.addEventListener("submit",async l=>{l.preventDefault(),a();let f=b("[data-err]"),y=parseFloat(c.price),P=parseFloat(c.originalPrice),A=parseFloat(c.shipping||0),h=parseInt(c.inventory,10),g=String(c.upc||"").replace(/\D/g,""),v=[];if(c.title.trim()||v.push(["ed-title","Add a title."]),y>0||v.push(["ed-price","Enter a sale price greater than $0."]),h>=0||v.push(["ed-inv","Inventory must be 0 or more."]),A>=0||v.push(["ed-ship","Shipping must be 0 or more."]),g&&(g.length<8||g.length>14)&&v.push(["ed-upc","UPC should be 8\u201314 digits."]),U("[aria-invalid]",s).forEach(I=>I.removeAttribute("aria-invalid")),v.length){v.forEach(([I])=>b("#"+I).setAttribute("aria-invalid","true")),f.textContent=v.map(I=>I[1]).join(" "),f.hidden=!1,b("#"+v[0][0]).focus();return}let R={id:w==="new"?void 0:w,title:c.title.trim(),slug:c.slug.trim(),brand:c.brand.trim(),upc:g,condition:c.condition,categoryId:c.categoryId||null,description:c.description.trim(),price:z(y),originalPrice:P>y?z(P):null,shipping:z(A),inventory:h,published:c.published,bestDeal:!1,noShipping:!!c.noShipping,images:c.images},H=b('button[type="submit"]',s);H.disabled=!0,H.textContent="Saving\u2026";try{let I=await d.saveProduct(R);return T(),k(w==="new"?"Product created":"Changes saved"),w=null,c=null,M="products",q(),I}catch(I){f.textContent=I.message,f.hidden=!1,H.disabled=!1,H.textContent="Save changes"}})}let Q="all";function X(){let s=m.orders.filter(e=>Q==="all"||e.status===Q);n.innerHTML=_(`
      ...
      <dialog class="modal adm-order-dlg" id="order-dlg" aria-labelledby="od-title"></dialog>`,"Orders")}function Se(s){let e=m.orders.find(o=>o.id===s);if(!e)return;b("#order-dlg")||(M="orders",X());let t=b("#order-dlg"),a=e.address?`${i(e.address.line1)}${e.address.line2?", "+i(e.address.line2):""}<br>${i(e.address.city)}, ${i(e.address.state)} ${i(e.address.zip)}`:"Local pickup";t.innerHTML=`<div class="modal-head"><h2 id="od-title">Order #${e.number}</h2><button class="icon-btn" type="button" aria-label="Close" data-close-dlg>${u("close")}</button></div>
      <div class="modal-body">
        <dl class="kv"><div><dt>Placed</dt><dd>${j(e.createdAt,!0)}</dd></div><div><dt>Delivery</dt><dd>${e.fulfillment==="pickup"?"Local pickup":"Shipping"}</dd></div><div><dt>Payment</dt><dd>${e.payment==="pickup"?"Pay at pickup":"Send payment link"}</dd></div><div><dt>Total</dt><dd class="tabnum">${E(e.total)}</dd></div></dl>
    ...
    </div>`,"Settings")}let ue=async s=>{let e=s.target,t=e.closest("[data-tab]");if(t){M=t.dataset.tab,w=null,c=null,q();return}if(e.closest("[data-logout]")){await d.logout(),V();return}if(e.closest("[data-new]")){M="products",w="new",c=null,q();return}let a=e.closest("[data-edit]");if(a){M="products",w=a.dataset.edit,c=null,q();return}if(e.closest("[data-cancel-edit]")){w=null,c=null,M="products",q();return}let o=e.closest("[data-inv]");if(o){let h=await d.adjustInventory(o.dataset.id,Number(o.dataset.inv));if(T(),h){let g=o.closest("tr");b("b",o.parentElement).textContent=h.inventory,g.classList.toggle("dim",h.inventory<=0||!h.published)}return}let $=e.closest("[data-open-order]");if($){Se($.dataset.openOrder);return}if(e.closest("[data-close-dlg]")||e.matches("dialog.modal")){let h=e.closest("dialog");h&&h.close();return}let N=e.closest("[data-cancel-order]");if(N){await d.updateOrder(N.dataset.cancelOrder,{status:"cancelled",restock:!0}),T(),k("Order cancelled and items restocked");let h=e.closest("dialog");h&&h.close(),await O(),X();return}let r=e.closest("[data-accept-price]");if(r){let h=m.offers.find(v=>v.id===r.dataset.acceptPrice),g=m.products.find(v=>v.id===h.productId);g&&(await d.saveProduct({...g,originalPrice:g.originalPrice||g.price,price:h.amount}),await d.updateOffer(h.id,{status:"accepted"}),T(),k(`Price set to ${E(h.amount)} and offer accepted`),await O(),ne());return}let C=e.closest("[data-bulk]");if(C){let h=[...L],g=C.dataset.bulk;if(g==="delete"&&!C.dataset.sure){C.dataset.sure="1",C.textContent=`Confirm delete ${h.length}`,C.classList.add("adm-danger");return}for(let v of h){let R=m.products.find(H=>H.id===v);if(R){if(g==="discount"){let H=Math.min(95,Math.max(1,parseInt(b("[data-bulk-pct]").value,10)||0)),I=R.originalPrice||R.price;await d.saveProduct({...R,originalPrice:I,price:z(I*(1-H/100))})}g==="publish"&&await d.patchProduct(v,{published:!0}),g==="unpublish"&&await d.patchProduct(v,{published:!1}),g==="delete"&&await d.deleteProduct(v)}}L.clear(),T(),k("Updated "+h.length+" product"+(h.length>1?"s":"")),await O(),K();return}let l=e.closest("[data-cat-move]");if(l){let h=l.closest("[data-cat]"),g=p.categories().map(H=>H.id),v=g.indexOf(h.dataset.cat),R=v+Number(l.dataset.catMove);[g[v],g[R]]=[g[R],g[v]],await d.reorderCategories(g),T(),J();return}let f=e.closest("[data-cat-save]");if(f){let h=f.closest("[data-cat]"),g=b("[data-cat-name]",h).value.trim();if(!g)return;await d.saveCategory({id:h.dataset.cat,name:g}),T(),k("Category saved"),J();return}let y=e.closest("[data-cat-del]");if(y){if(!y.dataset.sure){y.dataset.sure="1",y.textContent="Confirm";return}await d.deleteCategory(y.closest("[data-cat]").dataset.cat),T(),k("Category deleted"),await O(),J();return}let P=e.closest("[data-offer-del]");if(P){if(!P.dataset.sure){P.dataset.sure="1",P.textContent="Tap again to delete";return}await d.deleteOffer(P.dataset.offerDel),await O(),ne(),k("Offer deleted");return}let A=e.closest("[data-msg-del]");if(A){await d.deleteInquiry(A.dataset.msgDel),await O(),(A.dataset.back==="requests"?ie:de)(),k("Deleted");return}if(e.closest("[data-export]")){let h=await d.exportData(),g=new Blob([h],{type:"application/json"}),v=document.createElement("a");v.href=URL.createObjectURL(g),v.download=`mlgroup-backup-${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(v),v.click(),v.remove(),k("Backup downloaded");return}if(e.closest("[data-reset]")){b("[data-reset-confirm]").hidden=!1;return}if(e.closest("[data-reset-no]")){b("[data-reset-confirm]").hidden=!0;return}e.closest("[data-reset-yes]")&&(await d.reset(),T(),k("Demo data restored"),q())},pe=async s=>{let e=s.target;if(e.matches("[data-pf-f]")&&(B.f=e.value,K()),e.matches("[data-sel]")){e.checked?L.add(e.dataset.sel):L.delete(e.dataset.sel);let t=b("[data-bulk]");t.hidden=!L.size,b("[data-bulk-n]").textContent=`${L.size} selected`}if(e.matches("[data-sel-all]")){U("[data-sel]").forEach(a=>{a.checked=e.checked,e.checked?L.add(a.dataset.sel):L.delete(a.dataset.sel)});let t=b("[data-bulk]");t.hidden=!L.size,b("[data-bulk-n]").textContent=`${L.size} selected`}if(e.matches("[data-pub]")&&(await d.patchProduct(e.dataset.pub,{published:e.checked}),T(),e.nextElementSibling.textContent=e.checked?"Published":"Hidden",k(e.checked?"Published":"Hidden from store")),e.matches("[data-of]")&&(Q=e.value,X()),e.matches("[data-order-status]")&&(await d.updateOrder(e.dataset.orderStatus,{status:e.value}),k("Order status updated"),await O()),e.matches("[data-offer-status]")&&(await d.updateOffer(e.dataset.offerStatus,{status:e.value}),k("Offer updated"),await O()),e.matches("[data-msg-status]")&&(await d.updateInquiry(e.dataset.msgStatus,{status:e.value}),await O(),k("Updated"),M==="requests"&&ie()),e.matches("[data-import]")&&e.files[0])try{await d.importData(await e.files[0].text()),T(),k("Backup imported"),q()}catch(t){k(t.message,"err")}},me=s=>{if(s.target.matches("[data-pf-q]")){B.q=s.target.value;let e=s.target.selectionStart;K();let t=b("[data-pf-q]");t.focus(),t.setSelectionRange(e,e)}},he=async s=>{let e=s.target;if(e.matches("[data-login]")){s.preventDefault();let t=b('button[type="submit"]',e);t.disabled=!0;try{await d.login(e.elements.pw.value),q()}catch(a){V(a.message||"Sign in failed.")}}if(e.matches("[data-settings]")){s.preventDefault();let t=Object.fromEntries(new FormData(e));t.state=(t.state||"").toUpperCase(),await d.saveSettings(t),T(),k("Store details saved")}if(e.matches("[data-password]")){s.preventDefault();let t=Object.fromEntries(new FormData(e)),a=b("[data-err]",e);if(t.pw1.length<6){a.textContent="Use at least 6 characters.",a.hidden=!1;return}if(t.pw1!==t.pw2){a.textContent="The passwords don\u2019t match.",a.hidden=!1;return}try{await d.changePassword(t.pw1),e.reset(),a.hidden=!0,k("Password changed")}catch(o){a.textContent=o.message,a.hidden=!1}}if(e.matches("[data-cat-add]")){s.preventDefault();let t=e.elements.name.value.trim();if(!t)return;await d.saveCategory({name:t}),T(),k("Category added"),J()}},Me=()=>U("table.tbl",n).forEach(s=>{let e=U("thead th",s).map(t=>t.textContent.trim());U("tbody tr",s).forEach(t=>U("td",t).forEach((a,o)=>{a.hasAttribute("data-label")||a.setAttribute("data-label",e[o]||"")}))}),fe=new MutationObserver(Me);return fe.observe(n,{childList:!0,subtree:!0}),n.addEventListener("click",ue),n.addEventListener("change",pe),n.addEventListener("input",me),n.addEventListener("submit",he),q(),()=>{fe.disconnect(),n.removeEventListener("click",ue),n.removeEventListener("change",pe),n.removeEventListener("input",me),n.removeEventListener("submit",he)}}window.MLAdmin={mount:je};})();
